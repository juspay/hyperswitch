...
macro_rules! runners {
    ($($body:tt),*) => {
        ...
        pub fn runner_from_task<T>(task: &storage::ProcessTracker) -> Result<Option<Box<dyn ProcessTrackerWorkflow<T>>>, errors::ProcessTrackerError> where T : Storable + Sync + Send + Clone, PaymentsSyncWorkflow: ProcessTrackerWorkflow<T> {
