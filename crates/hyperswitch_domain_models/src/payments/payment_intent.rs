...
pub struct CustomerData {
    ...
    /// Persisted as an ISO-8601 string: the `router` binary declares time's
    /// `serde-human-readable` feature, so `Date` round-trips through this JSON blob as
    /// `"1970-04-03"` rather than a `(year, ordinal)` tuple.
