...
#[cfg(feature = "v2")]
use crate::{
    address::Address, business_profile, customer, errors, merchant_account, payment_address,
...
pub struct PaymentIntent {
    ...
    #[serde(with = "common_utils::custom_serde::iso8601")]
    pub modified_at: PrimitiveDateTime,
    /// The time at which the order was lasy synced
    pub last_synced: Option<PrimitiveDateTime>,
    ...
    #[encrypt(ty = Value)]
    pub customer_details: Option<Encryptable<GuestCustomerDetails>>,
...
impl PaymentIntent {
    ...
    pub fn create_domain_model_from_request(
        ...
        let customer_id = match request.customer {
            Some(api_models::payments::CustomerDetailsType::Id(customer_id)) => Some(customer_id),
            None | Some(_) => None,
        };
