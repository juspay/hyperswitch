...
async fn cluster_pool() -> Option<RedisConnectionWithContext> {
    ...
    test_connection(&settings).await.ok()
}
...
async fn test_consumer_group_create() {
    let is_invalid_redis_entry_error = tokio::task::spawn_blocking(move || {
        futures::executor::block_on(async {
            let redis_conn = test_connection(&RedisSettings::default())
                .await
...
async fn test_set_key_if_not_exists_and_get_value_concurrent_access() {
    let is_success = tokio::task::spawn_blocking(move || {
        futures::executor::block_on(async {
            ...
            let conn1 = pool.clone();
            let conn2 = pool.clone();
            let key1 = key_name.into();
            let key2 = key_name.into();

            let (result1, result2) = tokio::join!(
                conn1.set_key_if_not_exists_and_get_value(&key1, value1, Some(30)),
                conn2.set_key_if_not_exists_and_get_value(&key2, value2, Some(30))
            );
        ...
        futures::executor::block_on(async {
            ...
            let result = pool
                .set_multiple_keys_if_not_exist(&[
                    (key1.tenant_aware_key(&pool.redis_conn), "val1"),
                    (key2.tenant_aware_key(&pool.redis_conn), "val2"),
                ])
                .await;
...
async fn test_set_multiple_keys_if_not_exist_with_existing_key() {
    let is_success = tokio::task::spawn_blocking(move || {
        futures::executor::block_on(async {
            ...
            let result = pool
                .set_multiple_keys_if_not_exist(&[
                    (key1.tenant_aware_key(&pool.redis_conn), "new1"),
                    (key2.tenant_aware_key(&pool.redis_conn), "new2"),
                ])
                .await;
...
/// `XREADGROUP` over a stream with nothing undelivered must report "no work",
/// and must never report work that is not there.
///
/// The two backends say it differently, which is the point of pinning it:
/// `redis-rs` returns `Ok` with an empty reply, `fred` returns
/// `Err(StreamEmptyOrNotAvailable)`. Both are fine for a caller that checks.
/// A caller that only handles one of them silently gets the other — the
/// scheduler consumer read the `redis-rs` empty reply as a batch and issued
/// `XACK key group` with no IDs, which Redis rejects outright.
///
/// So this asserts the union rather than either shape, and would fail if a
/// backend ever returned entries for an exhausted stream.
#[tokio::test]
async fn test_stream_read_with_options_xreadgroup_empty() {
    let is_success = tokio::task::spawn_blocking(move || {
        futures::executor::block_on(async {
            let pool = test_connection(&RedisSettings::default())
                .await
                .expect("failed to create redis connection pool");
            let uid = unique_test_id();
            let stream: RedisKey = format!("test_xreadgroup_empty_{uid}").into();
            let group = format!("test_grp_{uid}");
            let consumer = format!("test_consumer_{uid}");

            // Append and then read the one entry, so the group exists and has
            // consumed everything — an idle scheduler's resting state, rather
            // than a stream that was never written to.
            let fields: Vec<(&str, &str)> = vec![("task", "process_payment")];
            pool.stream_append_entry(&stream, &RedisEntryId::AutoGeneratedID, fields)
                .await
                .expect("failed to append");
            ...
            .await
            .expect("failed to drain the only entry");
            ...
            match result {
                Ok(reply) => reply.values().all(|entries| entries.is_empty()),
                Err(err) => matches!(
                    err.current_context(),
                    crate::errors::RedisError::StreamEmptyOrNotAvailable
                ),
            }
        })
    })
...
async fn test_on_error_triggers_shutdown_when_redis_unreachable() {
    ...
    let pool = test_connection(&settings).await;
    let pool = match pool {
        Ok(pool) => pool,
