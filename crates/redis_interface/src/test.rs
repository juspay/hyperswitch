...
/// Regression test for a scheduler consumer leak (juspay/hyperswitch#13628): `XREADGROUP`
/// registers its consumer in the group as a side effect of the read, even when there are
/// no new entries to deliver, and Redis never expires these registrations on its own. A
/// caller that generates a fresh consumer name on every poll therefore grows the consumer
/// group without bound. Reusing the same consumer name across repeated reads must not.
#[tokio::test]
async fn test_xreadgroup_reused_consumer_name_does_not_grow_consumer_group() {
    let is_success = tokio::task::spawn_blocking(move || {
        futures::executor::block_on(async {
            let pool = test_connection(&RedisSettings::default())
                .await
                .expect("failed to create redis connection pool");
            let uid = unique_test_id();
            let stream_name = format!("test_stable_consumer_{uid}");
            let stream: RedisKey = stream_name.clone().into();
            let group = format!("test_stable_group_{uid}");
            let consumer = format!("test_stable_consumer_name_{uid}");

            pool.consumer_group_create(&stream, &group, &RedisEntryId::AutoGeneratedID)
                .await
                .expect("failed to create group");
            ...
            let registered_consumers: i64 = pool
