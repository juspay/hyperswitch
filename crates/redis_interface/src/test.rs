...
async fn cluster_pool() -> Option<RedisConnectionPool> {
    ...
    RedisConnectionPool::new(&settings, std::sync::Arc::new(common_utils::external_service::NoOpEventEmitter)).await.ok()
}
...
async fn test_consumer_group_create() {
    let is_invalid_redis_entry_error = tokio::task::spawn_blocking(move || {
        futures::executor::block_on(async {
            let redis_conn = RedisConnectionPool::new(&RedisSettings::default(), std::sync::Arc::new(common_utils::external_service::NoOpEventEmitter))
                .await
...
async fn test_delete_existing_key_success() {
    let is_success = tokio::task::spawn_blocking(move || {
        futures::executor::block_on(async {
            let pool = RedisConnectionPool::new(&RedisSettings::default(), std::sync::Arc::new(common_utils::external_service::NoOpEventEmitter))
                .await
                .expect("failed to create redis connection pool");
...
async fn test_connection_with_custom_config() {
    let is_success = tokio::task::spawn_blocking(move || {
        futures::executor::block_on(async {
            ...
            let pool = RedisConnectionPool::new(&settings, std::sync::Arc::new(common_utils::external_service::NoOpEventEmitter))
                .await
...
async fn test_on_error_triggers_shutdown_when_redis_unreachable() {
    ...
    let pool = RedisConnectionPool::new(&settings, std::sync::Arc::new(common_utils::external_service::NoOpEventEmitter)).await;
    let pool = match pool {
        Ok(pool) => pool,
