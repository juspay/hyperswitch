...
use crate::{
    ...
    logger,
    routes::{alert_manager, alerts_dicts, alerts_info, cloudwatch, health_check, notify},
    state::AppState,
};
        ...
        web::scope("/alerts")
            ...
            .service(
                web::scope("/alerts_manager")
                    ...
                    .service(
                        web::resource("/dicts")
                            .route(web::get().to(alerts_dicts::list))
                            .route(web::post().to(alerts_dicts::create)),
                    )
                    .service(
                        web::resource("/dicts/{id}")
                            .route(web::get().to(alerts_dicts::retrieve))
                            .route(web::delete().to(alerts_dicts::delete)),
                    )
                    .service(
                        web::resource("/notification_reads")
                            .route(web::get().to(alert_manager::notification_reads::retrieve))
                            .route(web::post().to(alert_manager::notification_reads::upsert)),
                    ),
            )
    }
}
