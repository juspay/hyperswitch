...
impl GcmAes256 {
    ...
    pub fn decrypt_aci_webhook_payload(
        &self,
        hex_key: &str,
        hex_iv: &str,
        hex_auth_tag: &str,
        ...
        let nonce_arr: [u8; aead::NONCE_LEN] = iv_bytes
            .as_slice()
            .try_into()
            .map_err(|_| errors::CryptoError::InvalidIvLength)
...
mod crypto_tests {
    ...
    #[test]
    fn test_gcm_aes_256_decrypt_aci_webhook_payload() {
        let key_hex = "000102030405060708090A0B0C0D0E0F000102030405060708090A0B0C0D0E0F";
        let iv_hex = "3D575574536D450F71AC76D8";
        let auth_tag_hex = "19FDD068C6F383C173D3A906F7BD1D83";
        let encrypted_body_hex = "F8E2F759E528CB69375E51DB2AF9B53734E393";
        let expected_plaintext = "{\"type\": \"PAYMENT\"}";

        let algorithm = super::GcmAes256;
        let decrypted_payload = algorithm
            .decrypt_aci_webhook_payload(key_hex, iv_hex, auth_tag_hex, encrypted_body_hex)
            .expect("ACI webhook payload decryption failed");

        assert_eq!(
            String::from_utf8(decrypted_payload)
                .expect("Failed to convert decrypted bytes to string"),
            expected_plaintext
        );

        let wrong_key_hex = "000102030405060708090A0B0C0D0E0F000102030405060708090A0B0C0D0E0E";
        let result_wrong_key = algorithm.decrypt_aci_webhook_payload(
            wrong_key_hex,
            iv_hex,
            auth_tag_hex,
            encrypted_body_hex,
        ...
        let wrong_auth_tag_hex = "19FDD068C6F383C173D3A906F7BD1D82";
        let result_wrong_tag = algorithm.decrypt_aci_webhook_payload(
            key_hex,
            iv_hex,
            wrong_auth_tag_hex,
            encrypted_body_hex,
        ...
        assert!(
            result_wrong_tag.is_err(),
            "Decryption should fail with wrong auth tag"
        );
    }
}
