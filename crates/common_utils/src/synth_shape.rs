...
//! recorded value.
//!
//! [`deja::synth::id`] is the usual answer for a fabricated string, and it is the
//! right one wherever the shape is unconstrained: it carries a visible marker, so
//! an operator reading a diff can see the value was invented. It is the wrong
//! answer here. Its marker is `deja-synth-`, which contains `-`, and its length
//! is fixed — while `consts::ALPHABETS` is 62 characters with no `-` or `_`,
//! `generate_random_numeric_string` promises digits only, and five connectors put
//! a default-alphabet nanoid on the wire as a payment reference.
//!
//! deja states the rule these break: *"a value the service rejects is worse than
//! a stop, because the rejection is attributed to the candidate"*. A generator's
//! alphabet and length are part of what it promises its callers, so a miss has
//! to honour them.
//!
//! What is kept is the property that actually matters. Every value here is a
//! function of the miss alone, so replay gets `same query -> same value, every
//! run`; without that, two replays of one unchanged candidate disagree with each
//! other and a divergence can no longer be attributed to the candidate rather
//! than to the fabrication.
//!
//! What is given up is human visibility: there is no prefix to read. The
//! fabrication is still attributed — the ledger records the outcome as
//! synthesized whatever the value looks like — so this costs a reader looking at
//! a value in isolation, not the scorer.
...
///
/// [`deja::synth::monotonic`] is plain `base + occurrence * step`; the `_ns` in
/// its parameter names is the caller's convention, not arithmetic, so the helper
/// is unit-agnostic and this constant fixes the unit at the one place that
/// chooses it. One millisecond, expressed in nanoseconds.
///
/// Kept small on purpose: a step large enough to put derived values above the
/// timing rule's evidential floor (`MIN_EVIDENTIAL_MS`, juspay/deja#188) would
/// let one coincide with a span's floored
/// duration, and a fabricated value would then read as a measured one.
...
mod tests {
    ...
    fn the_value_honours_the_alphabet_and_the_length() {
        for length in [1_usize, 8, 32, 64] {
            ...
            assert_eq!(value.chars().count(), length, "length is part of the promise");
            assert!(
