...
pub fn create_client(
        ...
        (client_certificate.clone(), client_certificate_key.clone())
    {
        ...
        let client_builder = get_client_builder(proxy_config, merchant_proxy_url.as_ref().map(|url| url.clone()))?;
    ...
    if let Some(ca_pem) = ca_certificate {
        ...
        let client_builder = get_client_builder(proxy_config, merchant_proxy_url.as_ref().map(|url| url.clone()))?.add_root_certificate(cert);
        return client_builder
            .use_rustls_tls()
            .build()
            .change_context(HttpClientError::ClientConstructionFailed)
...
pub fn get_client_builder(
    ...
    } else if let Some(url) = proxy_config.https_url.as_ref() {
        client_builder = client_builder.proxy(
