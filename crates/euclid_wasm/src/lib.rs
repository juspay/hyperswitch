...
#[cfg(test)]
mod webhook_status_config_tests {
    use super::*;

    #[test]
    fn includes_every_event_class_once() {
        let configs: Vec<_> = EventClass::iter().map(webhook_status_config).collect();

        assert_eq!(configs.len(), EventClass::iter().count());
        for event_class in EventClass::iter() {
            assert_eq!(
                configs
                    .iter()
                    .filter(|config| config.event_class == event_class)
                    .count(),
                1
            );
        }
    ...
    #[test]
    fn exposes_api_values_and_normalized_event_types() {
        let payment_config = webhook_status_config(EventClass::Payments);
        assert_eq!(payment_config.api_field, "payment_statuses_enabled");
        assert!(payment_config.statuses.iter().any(|status| {
            status.value == "succeeded" && status.event_type == EventType::PaymentSucceeded
        }));

        let subscription_config = webhook_status_config(EventClass::Subscriptions);
        assert_eq!(subscription_config.api_field, "invoice_statuses_enabled");
        assert_eq!(
            subscription_config
                .statuses
                .first()
                .map(|status| (status.value.as_str(), status.event_type)),
            Some(("invoice_paid", EventType::InvoicePaid))
        );
    }
}
