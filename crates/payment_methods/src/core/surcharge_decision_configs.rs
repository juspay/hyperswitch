...
use crate::{
    ...
    core::{
        domain::{self,types, payment_methods::SurchargeInterface},
        errors::{self, ConditionalConfigError as ConfigError},
    },
    db::StorageInterface,
    surcharge::{conditional_configs::ConditionalConfigResult, SurchargeKey, SurchargeMetadata},
};
use api_models::{
...
use euclid::backend::inputs::BackendInput;
use euclid::{
    backend,
    backend::{inputs as dsl_inputs, EuclidBackend},
};
use router_env::{instrument, logger, tracing};
use scheduler::db::process_tracker::storage;
...
impl SurchargeSource {
    pub fn generate_surcharge_details_and_populate_surcharge_metadata(
        ...
        surcharge_metadata_and_key: (&mut SurchargeMetadata, SurchargeKey),
...
pub async fn perform_surcharge_decision_management_for_payment_method_list(
    ...
    _payment_intent: &domain::PaymentIntent,
                    ...
                    let surcharge_details = surcharge_source
                        .generate_surcharge_details_and_populate_surcharge_metadata(
                            ...
                            (
                                ...
                                SurchargeKey::PaymentMethodData(
...
pub async fn perform_surcharge_decision_management_for_session_flow(
    ...
    payment_attempt: &domain::PaymentAttempt,
    _payment_intent: &domain::PaymentIntent,
    mut backend_input: BackendInput,
    ...
    for customer_payment_method in customer_payment_method_list.iter_mut() {
        ...
        let surcharge_details = surcharge_source
            .generate_surcharge_details_and_populate_surcharge_metadata(
                &backend_input,
                payment_attempt,
                (&mut surcharge_metadata, SurchargeKey::Token(payment_token)),
            )?;
...
//     customer_payment_method_list: &mut [api_models::payment_methods::CustomerPaymentMethod],
// ) -> ConditionalConfigResult<SurchargeMetadata> {
//     // let mut surcharge_metadata = SurchargeMetadata::new(payment_attempt.id.clone());
...
//                 (
//                     &mut surcharge_metadata,
//                     SurchargeKey::Token(payment_token),
//                 ),
//             )?;
//         customer_payment_method.surcharge_details = surcharge_details
//             .map(|surcharge_details| {
