...
impl Refund {
    pub async fn update(self, conn: &PgPooledConn, refund: RefundUpdate) -> StorageResult<Self> {
        ...
        >(
            conn,
            dsl::merchant_reference_id
                .eq(self.merchant_reference_id.to_owned())
                .and(dsl::merchant_id.eq(self.merchant_id.to_owned())),
            RefundUpdateInternal::from(refund),
        )
        .await
    ...
    // This is required to be changed for KV.
    pub async fn find_by_merchant_id_merchant_reference_id(
        conn: &PgPooledConn,
        merchant_id: &common_utils::id_type::MerchantId,
        merchant_reference_id: &common_utils::id_type::RefundReferenceId,
    ) -> StorageResult<Self> {
        generics::generic_find_one::<<Self as HasTable>::Table, _, _>(
            conn,
            dsl::merchant_id
