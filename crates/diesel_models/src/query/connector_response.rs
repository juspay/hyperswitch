...
impl ConnectorResponse {
    ...
    pub async fn update(
        ...
        let connector_response_result = match generics::generic_update_with_unique_predicate_get_result::<
            ...
            Err(err) => match err.current_context() {
                errors::DatabaseError::NotFound => {
                    ...
                    .await
                    {
                        ...
                        Err(err) => {
                            logger::error!(
                                "Error while fetching payment attempt in connector_response flow {:?}",
                                err
                            );
                            Err(err)
                        }
