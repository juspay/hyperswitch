...
use crate::{self as storage_types, cache, CardInfo, enums, EphemeralKeyNew, EphemeralKey, RefundUpdateInternal};
...
pub trait RefundInterface {
    ...
    async fn filter_refund_by_constraints(
        ...
        limit: i64,
        offset: i64,
    ) -> CustomResult<Vec<crate::refund::Refund>, errors::StorageError>;
...
mod storage {
    ...
    use crate::{
        connection,
        Refund, ReverseLookupNew, RefundNew, RefundUpdate,
        db::reverse_lookup::ReverseLookupInterface,
        services::{Store, logger},
        utils::{self, db_utils, storage_partitioning::PartitionKey}, enums, errors, kv,
    };
    #[async_trait::async_trait]
    impl RefundInterface for Store {
        async fn find_refund_by_internal_reference_id_merchant_id(
                ...
                enums::MerchantStorageScheme::RedisKv => {
                        ...
                        .await
                    {
                        ...
                        Ok(HsetnxReply::KeySet) => {
                            ...
                            {
                                reverse_lookups.push(ReverseLookupNew {
                                    sk_id: field.clone(),
                                    lookup_id: format!(
                                        "{}_{}_{}",
                                        created_refund.merchant_id,
                            ...
                            ReverseLookupNew::batch_insert(reverse_lookups, &conn)
                                .await
                            ...
                            self.push_to_drainer_stream::<Refund>(
                                redis_entry,
        ...
        async fn find_refund_by_merchant_id_connector_transaction_id(
            ...
            match storage_scheme {
                enums::MerchantStorageScheme::PostgresOnly => {
                    let conn = connection::pg_connection_read(self).await?;
                    Refund::find_by_merchant_id_connector_transaction_id(
                        &conn,
                        merchant_id,
                        connector_transaction_id,
                    )
        ...
        async fn update_refund(
            ...
            match storage_scheme {
                ...
                enums::MerchantStorageScheme::RedisKv => {
                    ...
                    let redis_value =
                    common_utils::ext_traits::Encode::<Refund>::encode_to_string_of_json(
                            &updated_refund,
                        )
        ...
        async fn find_refund_by_merchant_id_connector_refund_id_connector(
            ...
            let database_call = || async {
                let conn = connection::pg_connection_read(self).await?;
                Refund::find_by_merchant_id_connector_refund_id_connector(
                    &conn,
                    merchant_id,
                    connector_refund_id,
                    connector,
        ...
        async fn find_refund_by_payment_id_merchant_id(
            ...
            match storage_scheme {
                enums::MerchantStorageScheme::PostgresOnly => {
                    let conn = connection::pg_connection_read(self).await?;
                    Refund::find_by_payment_id_merchant_id(
                        &conn,
                        payment_id,
                        merchant_id,
                    )
        ...
        async fn filter_refund_by_constraints(
            ...
            match storage_scheme {
                enums::MerchantStorageScheme::PostgresOnly => {
                    let conn = connection::pg_connection_read(self).await?;
                    <Refund as crate::RefundDbExt>::filter_by_constraints(&conn, merchant_id, refund_details, limit, offset)
                        .await
                        .map_err(Into::into)
                        .into_report()
                }
        ...
        async fn filter_refund_by_meta_constraints(
            ...
            match storage_scheme {
                enums::MerchantStorageScheme::PostgresOnly => {
                    let conn = connection::pg_connection_read(self).await?;
                    <Refund as crate::RefundDbExt>::filter_by_meta_constraints(&conn, merchant_id, refund_details)
                        .await
                        .map_err(Into::into)
                        .into_report()
                }
...
impl RefundInterface for MockDb {
    ...
    async fn filter_refund_by_meta_constraints(
        ...
        let filtered_refunds = refunds
            ...
            .cloned()
            .collect::<Vec<crate::refund::Refund>>();
        ...
        let mut unique_connectors = std::collections::HashSet::new();
        let mut unique_currencies = std::collections::HashSet::new();
