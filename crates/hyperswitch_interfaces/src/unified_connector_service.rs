...
pub fn get_payments_response_from_ucs_webhook_content(
    ...
    match webhook_content.content {
        ...
        Some(unified_connector_service_client::payments::event_response::Content::RefundsResponse(_)) => {
        ...
        Some(unified_connector_service_client::payments::event_response::Content::DisputesResponse(_)) => {
            Err(UnifiedConnectorServiceError::WebhookProcessingFailure)
                .attach_printable("UCS webhook contains disputes response but payments response was expected")?
        },
        Some(unified_connector_service_client::payments::event_response::Content::IncompleteTransformation(_)) => {
            Err(UnifiedConnectorServiceError::WebhookProcessingFailure)
