...
pub struct MerchantAccountCreate {
    ...
    #[serde(default, deserialize_with = "routing_algorithm::deserialize_option")]
...
pub struct MerchantAccountUpdate {
    ...
    /// The routing algorithm to be used for routing payments to desired connectors
    #[schema(value_type = Option<Object>,example = json!({"type": "single", "data": "stripe"}))]
    #[serde(deserialize_with = "routing_algorithm::deserialize_option")]
    pub routing_algorithm: Option<serde_json::Value>,
...
pub mod routing_algorithm {
    ...
    use crate::enums::{RoutableConnectors, RoutablePaymentConnectors};
    ...
    impl<'de> Visitor<'de> for RoutingAlgorithmVisitor {
            ...
            A: de::MapAccess<'de>,
        {
            ...
            while let Some(key) = map.next_key()? {
                match key {
                    ...
                    "data" => {
                        ...
                        output["data"] = serde_json::Value::String(routing_type.to_owned());
                    }
                    f => {
            ...
            match routing_type.as_ref() {
                "single" => {
                    let routable_payment_conn = RoutablePaymentConnectors::from_str(&routing_data);
                    let routable_conn = match routable_payment_conn {
                        Ok(rpc) => Ok(RoutableConnectors::PaymentConnectors(rpc)),
                        Err(_) => Err(de::Error::custom(format!(
                            "Unknown payment connector {routing_data}"
                        ))),
                    }?;
                    Ok(RoutingAlgorithm::Single(routable_conn))
                }
...
pub mod payout_routing_algorithm {
    ...
    use super::RoutingAlgorithm;
    use crate::enums::{RoutableConnectors, RoutablePayoutConnectors};
    struct RoutingAlgorithmVisitor;
    struct OptionalRoutingAlgorithmVisitor;
    ...
    impl<'de> Visitor<'de> for RoutingAlgorithmVisitor {
            ...
            A: de::MapAccess<'de>,
        {
            ...
            match routing_type.as_ref() {
                "single" => {
                    let routable_payment_conn = RoutablePayoutConnectors::from_str(&routing_data);
                    let routable_conn = match routable_payment_conn {
                        Ok(rpc) => Ok(RoutableConnectors::PayoutConnectors(rpc)),
                        Err(_) => Err(de::Error::custom(format!(
