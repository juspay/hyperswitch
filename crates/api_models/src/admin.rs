...
mod tests {
    ...
    #[test]
    fn payment_method_blocking_config_accepts_extended_card_config() {
        let config = serde_json::from_value::<PaymentMethodBlockingConfig>(serde_json::json!({
            "card": {
                "issuing_country": ["US"],
                "card_types": ["credit"],
                "card_subtypes": ["consumer"],
                "issuers": ["issuer_123"],
                "block_if_bin_info_unavailable": true,
                "card_networks": ["Visa"],
                "funding_sources": ["CREDIT"],
                "card_segment_types": ["consumer"],
                "block_virtual_cards": true,
                "block_non_reloadable_prepaid_cards": true,
                "gambling_blocked": true
            }
        }))
        .expect("extended card blocking config should deserialize");

        let card = config.card.expect("card blocking config should be present");
        assert!(card
            .issuing_country
            .is_some_and(|countries| countries.contains(&common_enums::CountryAlpha2::US)));
        assert!(card
            .card_types
            .is_some_and(|card_types| card_types.contains(&common_enums::CardType::Credit)));
        assert!(card
            .card_subtypes
            .is_some_and(|card_subtypes| card_subtypes.contains("consumer")));
        assert!(card
            .issuers
            .is_some_and(|issuers| issuers.contains("issuer_123")));
        assert_eq!(card.block_if_bin_info_unavailable, Some(true));
        assert!(card
            .card_networks
            .is_some_and(|networks| networks.contains(&common_enums::CardNetwork::Visa)));
        assert!(card.funding_sources.is_some_and(|funding_sources| {
            funding_sources.contains(&common_enums::FundingSource::Credit)
        }));
        assert!(card.card_segment_types.is_some_and(|segment_types| {
            segment_types.contains(&common_enums::CardSegmentType::Consumer)
        }));
        assert_eq!(card.block_virtual_cards, Some(true));
        assert_eq!(card.block_non_reloadable_prepaid_cards, Some(true));
        assert_eq!(card.gambling_blocked, Some(true));
    }

    #[test]
    fn payment_method_blocking_config_accepts_legacy_wallet_config() {
        let config = serde_json::from_value::<PaymentMethodBlockingConfig>(serde_json::json!({
            "wallet": {
                "card_types": ["credit"]
            }
        }))
        .expect("legacy wallet blocking config should deserialize");

        let wallet = config
            .wallet
            .expect("legacy wallet blocking config should be present");
        assert!(wallet
            .card_types
            .is_some_and(|card_types| card_types.contains(&common_enums::CardType::Credit)));
        assert!(wallet.apple_pay.is_none());
        assert!(wallet.google_pay.is_none());
    }

    #[test]
    fn payment_method_blocking_config_accepts_per_wallet_config() {
        let config = serde_json::from_value::<PaymentMethodBlockingConfig>(serde_json::json!({
            "wallet": {
                "apple_pay": {
                    "issuing_country": ["US"],
                ...
                "google_pay": {
                    ...
                    "funding_sources": ["DEBIT"],
                    "card_segment_types": ["commercial"],
                    "block_virtual_cards": false,
                    "block_non_reloadable_prepaid_cards": false,
                    "gambling_blocked": true
                }
            }
        }))
        .expect("per-wallet blocking config should deserialize");

        let wallet = config
            .wallet
            .expect("wallet blocking config should be present");
        let apple_pay = wallet
            .apple_pay
            .expect("Apple Pay blocking config should be present");
        assert!(apple_pay
            .issuing_country
            .is_some_and(|countries| countries.contains(&common_enums::CountryAlpha2::US)));
        assert!(apple_pay
        ...
        assert!(apple_pay
            .issuers
            .is_some_and(|issuers| issuers.contains("issuer_123")));
        assert_eq!(apple_pay.block_if_bin_info_unavailable, Some(true));
        assert!(apple_pay
            .card_networks
            .is_some_and(|networks| networks.contains(&common_enums::CardNetwork::Visa)));
        assert!(apple_pay.funding_sources.is_some_and(|funding_sources| {
            funding_sources.contains(&common_enums::FundingSource::Credit)
        }));
        assert!(apple_pay.card_segment_types.is_some_and(|segment_types| {
            segment_types.contains(&common_enums::CardSegmentType::Consumer)
        }));
        assert_eq!(apple_pay.block_virtual_cards, Some(true));
        assert_eq!(apple_pay.block_non_reloadable_prepaid_cards, Some(true));
        assert_eq!(apple_pay.gambling_blocked, Some(false));

        let google_pay = wallet
            .google_pay
            .expect("Google Pay blocking config should be present");
        assert!(google_pay
            .card_networks
            .is_some_and(|networks| networks.contains(&common_enums::CardNetwork::Mastercard)));
        assert!(google_pay.funding_sources.is_some_and(|funding_sources| {
            funding_sources.contains(&common_enums::FundingSource::Debit)
        }));
        assert!(google_pay.card_segment_types.is_some_and(|segment_types| {
            segment_types.contains(&common_enums::CardSegmentType::Commercial)
        }));
        assert_eq!(google_pay.block_virtual_cards, Some(false));
        assert_eq!(google_pay.block_non_reloadable_prepaid_cards, Some(false));
        assert_eq!(google_pay.gambling_blocked, Some(true));
    }
