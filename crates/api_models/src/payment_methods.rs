...
mod card_type_tests {
    ...
    #[test]
    fn serialization_is_uppercase() {
        assert_eq!(
            serde_json::to_string(&CardType::Credit).unwrap(),
            r#""CREDIT""#
        );
        assert_eq!(
    ...
    #[test]
    fn deserialization_accepts_uppercase_and_lowercase() {
        // Lowercase is the previously-serialized form; it must keep deserializing so
        // existing clients and already-persisted JSON are not broken.
        for (input, expected) in [
            ...
            assert_eq!(serde_json::from_str::<CardType>(input).unwrap(), expected);
        }
    }

    #[test]
    fn from_str_is_uppercase_only() {
        // `payments::Card.card_type` is a plain `String` parsed via `FromStr` — serde
        // aliases do not apply here. Only the UPPERCASE form parses, which is what
        // `cards_info.card_type` holds.
        assert_eq!(CardType::from_str("CREDIT").unwrap(), CardType::Credit);
        assert_eq!(CardType::from_str("DEBIT").unwrap(), CardType::Debit);
        assert!(CardType::from_str("credit").is_err());
        assert!(CardType::from_str("Credit").is_err());
    }

    #[test]
    fn display_stays_uppercase() {
