...
pub struct FrmListFilters {
    ...
    pub currency: Vec<enums::Currency>,
    /// The list of available refund status filters
    #[schema(value_type = Vec<FrmStatus>)]
    pub frm_status: Vec<enums::FrmStatus>,
}
...
impl From<enums::FrmStatus> for FrmStatus {
    fn from(status: enums::FrmStatus) -> Self {
        match status {
            // enums::RefundStatus::Failure | enums::RefundStatus::TransactionFailure => Self::Failed,
            // enums::RefundStatus::ManualReview => Self::Review,
            enums::FrmStatus::FrmRejected => Self::FrmRejected,
            enums::FrmStatus::ManualReview => Self::ManualReview,
        }
    }
}
