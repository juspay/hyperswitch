...
pub async fn incoming_webhook_flow(
    ...
    if matches!(
        ...
        // Stripe 不保证 webhook 顺序。在持久化事件版本前，外部事件只能推进到取消终态；
        // active、paused 等可逆状态由同步 API 管理，避免延迟 webhook 覆盖它们。
    ...
    if connector == Connector::Stripebilling {
        ...
        if invoice_status == InvoiceStatus::InvoiceCreated {
            // Stripe Billing 原生订阅会自行续费；`invoice.created` 仅表示账单存在，
            // 此处再次发起 MIT 会导致重复扣款。
            return Ok(WebhookResponseTracker::NoEffect);
        }
        ...
        if let Err(create_error) = create_result {
            // Stripe 可能并发发送 `invoice.paid` 与兼容事件 `invoice.payment_succeeded`。
            // 唯一键冲突后必须重新查询，确认记录已存在后才能按幂等成功处理。
    ...
    if invoice.is_some() {
        // Hyperswitch 会为非原生计费连接器发起 MIT；账单已存在时必须避免重复扣款。
