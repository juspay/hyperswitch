...
pub async fn create_and_confirm_subscription(
    ...
    // Complete the first payment against the billing-platform price before creating an external
    // subscription, avoiding an orphaned subscription when payment fails.
    let estimate = billing_handler
...
pub async fn confirm_subscription(
    ...
    if let Some(connector_subscription_id) = hosted_stripe_subscription_id {
        ...
        // Stripe Checkout `mode=subscription` already created the Stripe subscription. Bind the
        // `sub_*` and update the first invoice instead of creating a duplicate through billing.
...
pub async fn pause_subscription(
    ...
    for _ in 0..3 {
        ...
        // The connector pause succeeded. If a webhook won the race, converge from the latest local
        // state without ever overwriting the terminal cancelled state.
...
pub async fn resume_subscription(
    ...
    for _ in 0..3 {
        ...
        // The connector resume succeeded. If a webhook won the race, converge from the latest local
        // state without ever overwriting the terminal cancelled state.
        expected_status = subscription_entry.subscription.status.clone();
    }
    if !converged {
