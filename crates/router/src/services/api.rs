...
pub fn build_redirection_form(
    ...
    match form {
        ...
        RedirectForm::BankOfAmericaAuthSetup {
            access_token,
            ddc_url,
            reference_id,
        } => {
              ...
              (PreEscaped(format!("<script>
                window.addEventListener(\"message\", function(event) {{
                    if (event.origin === \"https://centinelapistag.cardinalcommerce.com\" || event.origin === \"https://centinelapi.cardinalcommerce.com\") {{
                      window.location.href = window.location.pathname.replace(/payments\\/redirect\\/(\\w+)\\/(\\w+)\\/\\w+/, \"payments/$1/$2/redirect/complete/bankofamerica?referenceId={reference_id}\");
                    }}
                  }}, false);
                </script>
                ")))
        ...
        RedirectForm::BankOfAmericaConsumerAuth {
            access_token,
            step_up_url,
        } => {
            maud::html! {
            ...
            html {
                ...
                // This is the iframe recommended by bankofamerica but the redirection happens inside this iframe once otp
