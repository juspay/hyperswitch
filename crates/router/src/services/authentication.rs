    ...
    A: SessionStateInfo + Sync,
{
    async fn authenticate_and_fetch(
        ...
        let merchant = state
            .store()
            .find_merchant_account_by_merchant_id(state, &stored_api_key.merchant_id, &key_store)
            .await
    ...
    A: SessionStateInfo + Sync,
{
    async fn authenticate_and_fetch(
        ...
        let merchant = state
            .store()
            .find_merchant_account_by_merchant_id(state, self.0.as_ref(), &key_store)
            .await
            .map_err(|e| {
                if e.current_context().is_db_not_found() {
                    e.change_context(errors::ApiErrorResponse::Unauthorized)
    ...
    A: SessionStateInfo + Sync,
{
    async fn authenticate_and_fetch(
        ...
        let merchant = state
            .store()
            .find_merchant_account_by_merchant_id(state, &payload.merchant_id, &key_store)
            .await
            .change_context(errors::ApiErrorResponse::InvalidJwtToken)?;
