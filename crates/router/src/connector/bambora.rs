...
use std::fmt::Debug;
use crate::errors::ConnectorError::RequestEncodingFailedWithReason;
...
use crate::{
    ...
    types::{
        ...
        ErrorResponse, Response, ConnectorAuthType, PaymentsResponseData,
    }
};
...
where
    ...
    fn build_headers(
        ...
        let req = &_req.connector_auth_type;
        
        match req {
            ConnectorAuthType::HeaderKey { api_key } => {Ok(vec![(header::AUTHORIZATION.to_string(),api_key.clone()), (header::CONTENT_TYPE.to_string(), self.get_content_type().to_string())])},
            _ => { Ok(vec![])}
        }
    }
}
    ...
    > for Bambora
{
    ...
    fn get_request_body(
        ...
        if let Some(amount) = req.amount_captured {
            Ok(Some(format!("{{\"amount\":{}}}", amount).to_string()))
        } else {
            Ok(Some("{}".to_string()))
        }
    }
...
impl
    ...
    fn get_url(&self, _req: &types::PaymentsAuthorizeRouterData, _connectors: &settings::Connectors,) -> CustomResult<String,errors::ConnectorError> {
        return Ok(self.base_url(_connectors).to_string()+"payments");
    }
