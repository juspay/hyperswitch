                ...
                match status {
                    true => Ok(Self {
                        ...
                        response: Ok(types::PaymentsResponseData::TransactionResponse {
                            ...
                            redirection_data: None,
                            mandate_reference: response.carrier_id.as_ref().map(
        ...
        match status {
            true => Ok(Self {
                ...
                response: Ok(types::PaymentsResponseData::TransactionResponse {
                    ...
                    redirection_data: None,
                    mandate_reference: response.carrier_id.as_ref().map(|pm| MandateReference {
                        connector_mandate_id: Some(pm.clone().expose()),
                        payment_method_id: None,
                        mandate_metadata: None,
                        connector_mandate_request_reference_id: response
    ...
    fn try_from(
        ...
        Ok(Self {
            ...
            cc_exp_date: get_card_expiry_month_year_2_digit(
                card_mandate_info.card_exp_month.peek().clone(),
                card_mandate_info.card_exp_year.peek().clone(),
            )?,
        })
    }
}
...
fn get_card_expiry_month_year_2_digit(
    ...
    let year_2_digit = card_exp_year
        .get(card_exp_year.len() - 2..)
        .ok_or(errors::ConnectorError::RequestEncodingFailed)?
        .to_string();
    Ok(Secret::new(format!("{}{}", card_exp_month, year_2_digit)))
}
