    ...
    fn try_from(item: NoonRouterData<&types::PaymentsAuthorizeRouterData>) -> Result<Self, Self::Error> {
        let (payment_data, currency, category) = match item.router_data.request.connector_mandate_id() {
            ...
            _ => (
                ...
                Some(item.router_data.request.order_category.clone().ok_or(
        ...
        let name: String = item.router_data
            .get_description()?
            .trim()
            .replace("  ", " ")
            .chars()
...
impl TryFrom<NoonRouterData<&types::PaymentsCaptureRouterData>> for NoonPaymentsActionRequest {
    type Error = error_stack::Report<errors::ConnectorError>;
    fn try_from(item: NoonRouterData<&types::PaymentsCaptureRouterData>) -> Result<Self, Self::Error> {
