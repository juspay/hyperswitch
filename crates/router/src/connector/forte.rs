...
impl Forte {
    ...
    pub fn get_organization_id(&self, auth: &forte::ForteAuthType) -> String {
        let forte::ForteAuthType {
            organization_key, ..
        } = auth;
        if let Some((id, _loc_id)) = organization_key.split_once('@') {
            return format!("org_{}", id);
        } else {
            return format!("org_436912");
        }
    }
    pub fn get_location_id(&self, auth: &forte::ForteAuthType) -> String {
        let forte::ForteAuthType {
            organization_key, ..
        } = auth;
        if let Some((_id, loc_id)) = organization_key.split_once('@') {
    ...
    for Forte
{
    ...
    fn get_url(
        ...
        let transaction_id = _req.request.connector_transaction_id.clone();
    ...
    fn get_request_body(
        ...
        let forte_req = utils::Encode::<forte::FortePaymentsCancelReq>::convert_and_encode(_req)
    ...
    fn handle_response(
        ...
        let response: forte::FortePaymentCancelResponse = res
            .response
...
impl ConnectorIntegration<api::Execute, types::RefundsData, types::RefundsResponseData> for Forte {
    ...
    fn get_url(
        ...
        let location_id = self.get_location_id(&auth);
        Ok(format!(
            "{base_url}/organizations/{organization_id}/locations/{location_id}/transactions"
        ))
    }
