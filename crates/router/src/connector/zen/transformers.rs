...
use crate::{core::errors,types::{self,api, storage::enums}, pii, connector::utils::{PaymentsAuthorizeRequestData, self, RouterData, CardData, RefundsRequestData}};
...
impl TryFrom<&types::PaymentsAuthorizeRouterData> for ZenPaymentsRequest  {
    ...
    fn try_from(item: &types::PaymentsAuthorizeRouterData) -> Result<Self,Self::Error> {
        ...
        let (payment_specific_data, payment_channel) = match item.request.payment_method_data.clone() {
        ...
        Ok(Self { 
            ...
            customer: ZenCustomerDetails { 
                ...
                ip: browser_info.ip_address.ok_or(errors::ConnectorError::MissingRequiredField {field_name: "browser_info.ip_address"})?,
            }
        })
    }
}
...
impl<F,T> TryFrom<types::ResponseRouterData<F, ZenPaymentsResponse, T, types::PaymentsResponseData>> for types::RouterData<F, T, types::PaymentsResponseData> {
    type Error = error_stack::Report<errors::ConnectorError>;
    fn try_from(item: types::ResponseRouterData<F, ZenPaymentsResponse, T, types::PaymentsResponseData>) -> Result<Self,Self::Error> {
        Ok(Self {
