...
use crate::{
    connector::utils::{
        to_connector_meta, AddressDetailsData, CardData, PaymentsAuthorizeRequestData,
    },
    core::errors,
    pii, services,
...
impl TryFrom<&types::RefreshTokenRouterData> for PaypalAuthUpdateRequest {
    ...
    fn try_from(item: &types::RefreshTokenRouterData) -> Result<Self, Self::Error> {
        let paypal_auth_type = PaypalAuthType::try_from(&item.connector_auth_type)?;
        Ok(Self {
            grant_type: "client_credentials".to_string(),
            client_id: paypal_auth_type.key1,
            client_secret: paypal_auth_type.api_key,
        })
    }
}
