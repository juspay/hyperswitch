...
use common_utils::{ext_traits::Encode, id_type, pii, types::MinorUnit};
...
fn get_line_items(item: &AdyenRouterData<&types::PaymentsAuthorizeRouterData>) -> Vec<LineItem> {
    ...
    match order_details {
        Some(od) => od
            ...
            .enumerate()
            .map(|(i, data)| LineItem {
                amount_including_tax: Some(item.amount),
                amount_excluding_tax: Some(item.amount),
    ...
    fn try_from(
        ...
        Ok(Self {
            ...
            payment_method_balance: Some(types::PaymentMethodBalance {
                ...
                amount:item.response.balance.value.get_amount_as_i64(),
            }),
            ..item.data
        })
    }
