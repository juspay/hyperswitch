    ...
    fn try_from(item: &frm_types::FrmSaleRouterData) -> Result<Self, Self::Error> {
        let products = item
            ...
            .map(|order_detail| Products {
                ...
                item_price: order_detail.amount.get_amount_as_i64(), // This should be changed to MiniorUnit when we implement amount conversion for this connector. Additionally, the function get_amount_as_i64() should be avoided in the future.
