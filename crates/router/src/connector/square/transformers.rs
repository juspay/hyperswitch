    ...
    fn try_from(item: &types::TokenizationRouterData) -> Result<Self, Self::Error> {
        match &item.request.payment_method_data.clone() {
            api::PaymentMethodData::BankDebit(bank_debit_data) => {
                Self::try_from((item, bank_debit_data.clone()))
            }
            api::PaymentMethodData::Card(card_data) => Self::try_from((item, card_data.clone())),
            api::PaymentMethodData::Wallet(wallet_data) => {
                Self::try_from((item, wallet_data.clone()))
            }
            api::PaymentMethodData::PayLater(pay_later_data) => {
                Self::try_from((item, pay_later_data.clone()))
            }
