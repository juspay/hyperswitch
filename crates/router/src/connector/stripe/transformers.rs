...
pub struct SourceResponseMetadata {
    pub display_bank_transfer_information: SourceBankTransferData,
}

#[derive(Clone, Debug, Serialize)]
pub struct SourceBankTransferData {
    pub bank_transfer_steps_and_charges_details: StripeSourceResponse,
}
...
pub struct ConnectorMetadata {
    #[serde(skip_serializing_if = "Option::is_none")]
    pub qr_code_information: Option<WechatPayNextInstructions>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub display_bank_transfer_information: Option<BankTransferConnectorMetadata>,
}

#[derive(Clone, Debug, Serialize)]
pub struct BankTransferConnectorMetadata {
    pub bank_transfer_steps_and_charges_details: SepaAndBacsBankTransferInstructions,
}
...
pub fn get_connector_metadata(
    ...
    let qr_code_information =
        next_action.and_then(|next_action_response| match next_action_response {
            StripeNextActionResponse::WechatPayDisplayQrCode(qr_code_data) => {
                Some(WechatPayNextInstructions {
                    image_data_url: qr_code_data.image_data_url.to_owned(),
                })
            }
            _ => None,
        });

    let display_bank_transfer_information =
        next_action.and_then(|next_action_response| match next_action_response {
            StripeNextActionResponse::DisplayBankTransferInstructions(bank_transfer_data) => {
                let bank_instructions = bank_transfer_data.financial_addresses.get(0);
                ...
                Some(BankTransferConnectorMetadata {
                    bank_transfer_steps_and_charges_details: SepaAndBacsBankTransferInstructions {
                        sepa_bank_instructions,
                        bacs_bank_instructions,
                        receiver: SepaAndBacsReceiver {
                            amount_received: amount - bank_transfer_data.amount_remaining,
                            amount_remaining: bank_transfer_data.amount_remaining,
                        },
                    },
                })
            }
    ...
    for types::RouterData<F, T, types::PaymentsResponseData>
{
    ...
    fn try_from(
        ...
        let source_response_metadata = SourceResponseMetadata {
            display_bank_transfer_information: SourceBankTransferData {
                bank_transfer_steps_and_charges_details: connector_source_response.to_owned(),
            },
        };
        let connector_metadata =
            common_utils::ext_traits::Encode::<SourceResponseMetadata>::encode_to_value(
