...
impl TryFrom<&types::PaymentsAuthorizeRouterData> for NetseasyPaymentsRequest  {
    ...
    fn try_from(_item: &types::PaymentsAuthorizeRouterData) -> Result<Self,Self::Error> {
        let check_details = get_checkout_details (_item);
        let order_det : OrderDetails = match _item.request.order_details.clone() {
            Some (x) => Ok(x),
            None => Err(errors::ConnectorError::MissingRequiredField {
                field_name: "product_name"
            })
        }.unwrap();
        let order_req = get_order_details (_item, order_det);
        Ok(Self {
           order : order_req,
...
impl<F,T> TryFrom<types::ResponseRouterData<F, NetseasyPaymentsResponse, T, types::PaymentsResponseData>> for types::RouterData<F, T, types::PaymentsResponseData> {
    type Error = error_stack::Report<errors::ConnectorError>;
    fn try_from(item: types::ResponseRouterData<F, NetseasyPaymentsResponse, T, types::PaymentsResponseData>) -> Result<Self,Self::Error> {
        let redirection_d = {
            let url = Url::parse( &item.response.hosted_payment_page_url).into_report().change_context(
...
fn get_checkout_details(item: &types::PaymentsAuthorizeRouterData) -> NetseasyCheckout {
    NetseasyCheckout {
        url : "".to_string(),
        terms_url : "https://google.com".to_string(),
        integration_type : "HostedPaymentPage".to_string(),
        return_url : "https://google.com".to_string()
    }
}

fn get_order_details(item: &types::PaymentsAuthorizeRouterData, order_details : OrderDetails) -> NetseasyOrder {
    NetseasyOrder {
        amount : item.request.amount,
        currency: item.request.currency.to_string(),
        items : vec![ItemList {
            reference : "something".to_string(),
