...
impl FxExchangeRatesCacheEntry {
    ...
    fn is_expired(&self, data_expiration_delay: u32) -> CustomResult<bool, ForexError> {
        Ok(self.timestamp + i64::try_from(data_expiration_delay)
            .change_context(ForexError::ConversionError)? < date_time::now_unix_timestamp())
    }
}
...
async fn save_forex_data_to_redis(
    ...
    app_state
        ...
        .serialize_and_set_key_with_expiry(
            ...
            i64::try_from(forex_api.redis_ttl_in_seconds).change_context(ForexError::ConversionError)?,
        )
        .await
...
async fn is_redis_expired(
    ...
    data_expiration_delay: u32,
) -> CustomResult<Option<Arc<ExchangeRates>>, ForexError> {
    let delay = i64::try_from(data_expiration_delay)
            .change_context(ForexError::ConversionError)?;
    Ok(redis_cache.and_then(|cache| {
        if cache.timestamp + delay > date_time::now_unix_timestamp() {
