...
use crate::core::payments::{
    helpers::get_payment_id_from_client_secret, PaymentsRedirectResponseData,
};
...
impl ApiEventMetric for Vec<MandateResponse> {}
    ...
    fn get_api_event_type(&self) -> Option<ApiEventsType> {
        Some(ApiEventsType::PaymentMethodList {
            payment_id: self
                .client_secret
                .as_ref()
                .and_then(|cs| get_payment_id_from_client_secret(cs).ok()),
        })
    }
}
