...
pub struct PayloadAchPendingPromotionWorkflow;

#[async_trait::async_trait]
impl ProcessTrackerWorkflow<SessionState> for PayloadAchPendingPromotionWorkflow {
    #[cfg(feature = "v1")]
    async fn execute_workflow<'a>(
        ...
        let handler = PayloadAchPendingPromotionHandler::create(state, tracking_data).await?;

        if handler.check_status()? {
            handler.promote_to_charged().await?;
        } else {
            logger::info!(
                "ACH pending promotion skipped for attempt {}: status is already {:?}",
                handler.tracking_data.attempt_id,
                handler.payment_attempt.status
            );
            handler
...
pub struct PayloadAchPendingPromotionHandler<'a> {
    state: &'a SessionState,
    tracking_data: storage::AchPendingPromotionTrackingData,
...
impl<'a> PayloadAchPendingPromotionHandler<'a> {
    pub async fn create(
        state: &'a SessionState,
        tracking_data: storage::AchPendingPromotionTrackingData,
    ) -> Result<Self, sch_errors::ProcessTrackerError> {
    ...
    pub fn check_status(&self) -> Result<bool, sch_errors::ProcessTrackerError> {
        // Only promote if the payment is still in Pending status.
        Ok(self.payment_attempt.status == enums::AttemptStatus::Pending)
    }

    pub async fn promote_to_charged(&self) -> Result<(), sch_errors::ProcessTrackerError> {
        let db: &dyn StorageInterface = &*self.state.store;
        ...
        let payment_attempt_update =
            hyperswitch_domain_models::payments::payment_attempt::PaymentAttemptUpdate::StatusUpdate
            {
                ...
                updated_by: self.merchant_account.storage_scheme.to_string(),
            };

        db.update_payment_attempt_with_attempt_id(
            self.payment_attempt.clone(),
        ...
        let payment_intent_update =
            hyperswitch_domain_models::payments::payment_intent::PaymentIntentUpdate::PGStatusUpdate
            {
                ...
                feature_metadata: self.payment_intent.feature_metadata.clone().map(masking::Secret::new),
            };

        db.update_payment_intent(
            self.payment_intent.clone(),
