...
/// Workflow that auto-promotes eligible payments from Pending to Charged
/// after a configured time window (e.g., Payload ACH after 3 days).
pub struct PendingToChargedWorkflow;

#[async_trait::async_trait]
impl ProcessTrackerWorkflow<SessionState> for PendingToChargedWorkflow {
    #[cfg(feature = "v1")]
    async fn execute_workflow<'a>(
        ...
        let tracking_data: storage::PendingToChargedWorkflowTrackingData = process
            .tracking_data
            .clone()
            .parse_value("PendingToChargedWorkflowTrackingData")?;
        ...
        if payment_attempt.status != enums::AttemptStatus::Pending {
            logger::info!(
                "Pending promotion skipped for attempt {}: status is already {:?}",
                tracking_data.attempt_id,
        ...
        logger::info!(
            "Pending promotion completed: attempt {} promoted to Charged",
            tracking_data.attempt_id
        );
    ...
    async fn error_handler<'a>(
        ...
        logger::error!(%process.id, "Failed while executing pending promotion workflow");
        Ok(())
    }
}
