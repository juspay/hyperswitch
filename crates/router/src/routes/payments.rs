    ...
    if should_call_proxy_for_payments_core {
        ...
        match req.payment_type.unwrap_or_default() {
            ...
            | api_models::enums::PaymentType::Installment => {
                ...
                // Check if payment method data is VaultCardToken — also route to external vault proxy flow
                let is_vault_card_token = req
                    .payment_method_data
                    .as_ref()
                    .and_then(|pmd| pmd.payment_method_data.as_ref())
                    .map(|data| {
                        matches!(data, api_models::payments::PaymentMethodData::VaultCardToken(_))
                    })
                    .unwrap_or(false);

                if is_vault_card_token {
                    let card_token_data = req
                        .payment_method_data
                        .as_ref()
                        .and_then(|pmd| pmd.payment_method_data.as_ref())
                        .and_then(|data| match data {
                            api_models::payments::PaymentMethodData::VaultCardToken(card_token) => {
                                Some(card_token.clone())
                            }
                            _ => None,
                        })
                        .ok_or_else(|| {
                            report!(errors::ApiErrorResponse::InvalidRequestData {
                                message: "Missing vault card token data".to_string(),
                            })
                        })?;

                    let proxy_pmd = payment_types::ProxyPaymentMethodDataRequest {
                        payment_method_data: Some(
                            payment_types::ProxyPaymentMethodData::CardTokenData(
                                payment_types::CardTokenData {
                    ...
                    let payment_method_type = req
                        ...
                        .ok_or_else(|| {
                            report!(errors::ApiErrorResponse::InvalidRequestData {
                                message: "payment_method is required for VaultCardToken"
                                    .to_string(),
                            })
                        })?;
                    ...
                    let payment_method_subtype = req
                        ...
                        .ok_or_else(|| {
                            report!(errors::ApiErrorResponse::InvalidRequestData {
                                message: "payment_method_type is required for VaultCardToken"
                                    .to_string(),
                            })
                        })?;
