...
pub async fn create_saved_view(
    state: web::Data<AppState>,
    req: HttpRequest,
    json_payload: web::Json<user_api::dashboard_metadata::CreateSavedViewRequest>,
) -> HttpResponse {
    let flow = Flow::CreateSavedView;
    Box::pin(api::server_wrap(
        flow,
        state,
        &req,
        json_payload.into_inner(),
        user_core::dashboard_metadata::create_saved_view,
        &auth::JWTAuth {
            permission: Permission::ProfileAccountWrite,
            allow_connected: true,
...
pub async fn list_saved_views(
    state: web::Data<AppState>,
    req: HttpRequest,
    query: web::Query<user_api::dashboard_metadata::ListSavedViewsRequest>,
) -> HttpResponse {
    let flow = Flow::ListSavedViews;
    Box::pin(api::server_wrap(
        flow,
        state,
        &req,
        query.into_inner(),
        user_core::dashboard_metadata::list_saved_views,
        &auth::JWTAuth {
...
pub async fn update_saved_view(
    state: web::Data<AppState>,
    req: HttpRequest,
    json_payload: web::Json<user_api::dashboard_metadata::UpdateSavedViewRequest>,
) -> HttpResponse {
    let flow = Flow::UpdateSavedView;
    let payload = json_payload.into_inner();
    Box::pin(api::server_wrap(
        flow,
        state,
        &req,
        payload.clone(),
        |state, user, req, req_state| {
            user_core::dashboard_metadata::update_saved_view(state, user, req, req_state)
        },
        &auth::JWTAuth {
            permission: Permission::ProfileAccountWrite,
            allow_connected: true,
...
pub async fn delete_saved_view(
    state: web::Data<AppState>,
    req: HttpRequest,
    json_payload: web::Json<user_api::dashboard_metadata::DeleteSavedViewRequest>,
) -> HttpResponse {
    let flow = Flow::DeleteSavedView;
    let payload = json_payload.into_inner();
    Box::pin(api::server_wrap(
        flow,
        state,
        ...
        payload.clone(),
        |state, user, req, req_state| {
            user_core::dashboard_metadata::delete_saved_view(state, user, req, req_state)
        },
        &auth::JWTAuth {
            permission: Permission::ProfileAccountWrite,
            allow_connected: true,
