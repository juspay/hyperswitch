...
use base64::Engine;
use common_utils::{crypto::{self, SignMessage}, errors::CustomResult, ext_traits::Encode};
...
pub async fn delete_from_blocklist_lookup_db (
    ...
    let fingerprint_lookup = pm_hashes.iter().map(|entry| {
        state.store.find_pm_fingerprint_entry(entry.to_string())}).collect::<Vec<_>>();
    ...
    for((unblocked_from_lookup, unblocked_from_blocklist), data) in unblocked_from_lookup.into_iter().zip(unblocked_from_blocklist.into_iter()).zip(pm_hashes.into_iter()) {
...
pub async fn list_blocked_pm_from_db(
    ...
    let blocked_extended_bins = state
        .store
        .list_all_blocked_pm_for_merchant_by_type(merchant_id.clone(), "extended_cardbin".to_string());

    let blocked_fingerprints = state
        .store
        .list_all_blocked_pm_for_merchant_by_type(merchant_id, "fingerprint".to_string());
    let (blocked_cardbins, blocked_fingerprints, blocked_extended_bins) = tokio::join!(blocked_cardbins, blocked_fingerprints, blocked_extended_bins);

    match (blocked_fingerprints, blocked_cardbins, blocked_extended_bins) {
        (Ok(fingerprint), Ok(cardbin), Ok(extended_bin)) => {
            Ok(pm_blacklist::ListBlockedPmResponse {
                ...
                blocked_extended_cardbins: extended_bin
                    .iter()
                    .map(|extended_bin| (extended_bin.pm_hash.clone(), extended_bin.metadata.clone()))
                    .collect::<Vec<_>>(),
            })
        }
        (_, _, _) => {
            Err(errors::ApiErrorResponse::GenericNotFoundError { message: "Unable to retrieve Blocklist".to_string() }.into())
        },
    }
}
...
pub async fn insert_to_blocklist_lookup_db(
            ...
            match pm_type {
                "cardbin" => {
                    ...
                    // Checking for duplicacy
                        if state
                                .store
                                .find_blocklist_lookup_entry_by_merchant_id_kms_decrypted_hash(merchant_id.clone(), encoded_hash.clone())
                                .await
                                .is_ok() 
                        {
                            ...
                            let fingerprint_id = state.store
                                ...
                                .await
                                .change_context(errors::StorageError::ValueNotFound(card_bin.to_string().clone().to_string()))?
                                .fingerprint_id;
                            let _ = state.store.insert_blocklist_lookup_entry(
                    ...
                    // Checking for duplicacy
                                ...
                                .is_ok() 
                        {
                            ...
                            let fingerprint_id = state.store
                                ...
                                .await
                                .change_context(errors::StorageError::ValueNotFound(extended_bin.to_string().clone()))?
                                .fingerprint_id;
                            let _ = state.store.insert_blocklist_lookup_entry(
                                diesel_models::blocklist_lookup::BlocklistLookupNew {
                                    merchant_id: merchant_id.clone(),
                ...
                _ => {
                    ...
                    let kms_decrypted_hash = state.store.find_pm_fingerprint_entry(pm_hash.clone()).await
                    .change_context(errors::StorageError::ValueNotFound(pm_hash.clone().to_string()))?
                    .kms_hash;

                    if state
                        .store
                        .find_pm_blocklist_entry_by_merchant_id_fingerprint(merchant_id.clone(), pm_hash.clone())
                        .await
                        .is_ok() || state
                            .store
                            .find_blocklist_lookup_entry_by_merchant_id_kms_decrypted_hash(merchant_id.clone(), kms_decrypted_hash.clone())
                            .await
                            .is_ok() 
                    {
                        Err(StorageError::DuplicateValue {
    ...
    blocked_pm_futures.into_iter().for_each(|res| match res {
        Ok(blocked_pm) => {
            fingerprints_blocked.push(blocked_pm.pm_hash)
        },
        Err(e) => {
