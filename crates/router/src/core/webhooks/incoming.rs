    ...
    let webhook_effect = match process_webhook_further {
        true => {
            let business_logic_result = process_webhook_business_logic(
                ...
                &webhook_processing_result.shadow_transform_data,
...
async fn process_shadow_ucs_webhook_transform(
    ...
    match (ucs_result, direct_result) {
        (
            ...
            // Return Direct result as primary with UCS data in shadow_transform_data
            Ok(WebhookProcessingResult {
                event_type: direct_processing_result.event_type,
                source_verified: direct_processing_result.source_verified,
                transform_data: None,
                decoded_body: direct_processing_result.decoded_body,
                shadow_transform_data: Some(Box::new(ucs_transform_data)),
            })
        }
...
pub struct WebhookProcessingResult {
    ...
    shadow_transform_data: Option<Box<unified_connector_service::WebhookTransformData>>,
}
...
fn extract_webhook_event_object(
    ...
    match transform_data {
        Some(transform_data) => {
            match &transform_data.webhook_content {
                Some(webhook_content) => {
                    let serialized_value = match webhook_content.content.clone() {
                        Some(payments_grpc::webhook_response_content::Content::IncompleteTransformation(incomplete_transformation_response)) => {
                            serde_json::from_slice::<serde_json::Value>(&incomplete_transformation_response.resource_object)
                                .change_context(errors::ApiErrorResponse::InternalServerError)
    ...
    let event_object: Box<dyn masking::ErasedMaskSerialize> =
        if let Some(transform_data) = webhook_transform_data {
            ...
            if let Some(webhook_content) = &transform_data.webhook_content {
                match webhook_content.content.clone() {
                    Some(
                        payments_grpc::webhook_response_content::Content::IncompleteTransformation(
                        ...
                        .change_context(errors::ApiErrorResponse::InternalServerError)
                        .attach_printable(
                            "Failed to deserialize resource object from UCS webhook response",
                        )?,
                    ),
                    _ => {
                        // Convert UCS webhook content to appropriate format
...
async fn payments_incoming_webhook_flow(
    ...
    // Determine shadow UCS call connector action based on shadow transform data
    let shadow_ucs_call_connector_action = shadow_transform_data.as_ref().map(|shadow_data| {
        if shadow_data.is_transformation_complete {
