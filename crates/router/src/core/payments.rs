        ...
        | (None, Some(_), None, Some(true), _) => {
            ...
            let retryable_connectors = join_all(connectors.into_iter().map(|connector_routing_data| {
                ...
                async move {
                    ...
                    action_types.into_iter().map(|action_type| {
            ...
            routing_data.routed_through = Some(chosen_connector_routing_data.connector_data.connector_name.to_string());

            routing_data
                .merchant_connector_id
                .clone_from(&chosen_connector_routing_data.connector_data.merchant_connector_id);
...
pub async fn get_all_action_types(
        ...
        if ntid_supported_connectors.contains(&connector.connector_name)
                && network_tokenization_supported_connectors.contains(&connector.connector_name)
            {
                if let Ok((token_exp_month, token_exp_year)) =
                    ...
                    .await
                {
                    let action_type = ActionType::NetworkTokenWithNetworkTransactionId(
                        NTWithNTIRef {
                            token_exp_month,
                            token_exp_year,
                            network_transaction_id,
    ...
    if is_card_with_ntid_flow {
        if let Some(network_transaction_id) = &payment_method_info.network_transaction_id {
            let action_type = ActionType::CardWithNetworkTransactionId(
    ...
    if is_mandate_flow {
        if let Ok(connector_mandate_details) = connector_mandate_details {
            let action_type = ActionType::ConnectorMandate(
