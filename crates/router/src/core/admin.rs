    ...
    if let Some(webhooks_option_vec) = &mut profile_response.webhook_details {
        profile_response.webhook_details = Some(
            webhooks_option_vec
                .iter()
                .filter_map(|webhook_opt| {
                    // Check if webhook is Some and status is not "deprecated"
                    if let Some(webhook) = webhook_opt {
                        if webhook.status
                            != Some(common_enums::OutgoingWebhookEndpointStatus::Deprecated)
                        {
                            return Some(Some(webhook.clone())); // Return cloned WebhookDetails if not deprecated
                        }
                    }
                    None // Exclude if it's deprecated or None
                })
                .collect::<Vec<Option<api_models::admin::WebhookDetails>>>(), // Collect as Vec<Option<WebhookDetails>>
        );
    }
    // Return the response with the filtered webhooks
