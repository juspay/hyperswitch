    ...
    if helpers::is_payout_terminal_state(status) {
        return Err(report!(errors::ApiErrorResponse::InvalidRequestData {
            message: format!(
                ...
                payout_data.payout_create.payout_id, status
            ),
        }));
    }
...
pub async fn payouts_cancel_core(
    ...
    let payout_create = payout_data.payout_create.to_owned();
    let connector_payout_id = payout_create.connector_payout_id.to_owned();
    let status = payout_create.status.foreign_into();
    ...
    // Make local cancellation
        ...
        let updated_payout_create = storage::PayoutCreateUpdate::StatusUpdate {
        ...
        payout_data.payout_create = state
            .store
            .update_payout_create_by_merchant_id_payout_id(
            ...
            .await
            .change_context(errors::ApiErrorResponse::InternalServerError)
            .attach_printable("Error updating payout_create in db")?;
...
pub async fn payouts_fulfill_core(
    ...
    // Trigger fulfillment
    let payout_method_data = helpers::make_payout_method_data(state, &None, &payout_create)
        .await?
...
pub async fn call_connector_payout(
    ...
    if let Some(true) = req.create_payout {
        payout_data.payout_method_data = Some(
            helpers::make_payout_method_data(state, &req.payout_method_data, payout_create)
                .await?
                .get_required_value("payout_method_data")?,
        );
        // Eligibility flow
        if payouts.payout_type == storage_enums::PayoutType::Card
            && payout_create.is_eligible.is_none()
        {
        ...
        if payout_data.payouts.payout_type == storage_enums::PayoutType::Bank
            && payout_data.payout_create.status == storage_enums::PayoutStatus::RequiresCreation
        {
            // Create customer flow
            *payout_data =
                create_recipient(state, merchant_account, req, &connector_data, payout_data)
                    .await
    ...
    let status = payout_data.payout_create.status;
...
pub async fn create_recipient(
    ...
    let (is_eligible, _connector_customer_id, _connector_customer_map) =
        payments::customers::should_call_connector_create_customer(
            state,
            connector_data,
            &customer_details,
        )?;
    ...
    match router_data_resp.response {
        Ok(payout_response_data) => {
            ...
            let status = payout_response_data
                .status
                .unwrap_or(payout_create.status.to_owned());
            ...
            if helpers::is_payout_err_state(status.foreign_into()) {
                return Err(report!(errors::ApiErrorResponse::PayoutFailed {
                    data: Some(
                        serde_json::json!({"payout_status": status.to_string(), "error_message": payout_data.payout_create.error_message.as_ref(), "error_code": payout_data.payout_create.error_code.as_ref()})
                    ),
                }));
            }
        }
...
pub async fn cancel_payout(
    ...
    let db = &*state.store;
    let merchant_id = &merchant_account.merchant_id;
    let payout_id = &payout_data.payout_create.payout_id;
    match router_data_resp.response {
        Ok(payout_response_data) => {
            let status = payout_response_data
                .status
                .unwrap_or(payout_data.payout_create.status.to_owned());
            let updated_payout_create = storage::payout_create::PayoutCreateUpdate::StatusUpdate {
                connector_payout_id: payout_response_data.connector_payout_id,
                status,
                error_code: None,
        ...
        Err(err) => {
            let updated_payouts_create = storage::payout_create::PayoutCreateUpdate::StatusUpdate {
                connector_payout_id: String::default(),
                status: storage_enums::PayoutStatus::Failed,
                error_code: Some(err.code),
                error_message: Some(err.message),
...
pub async fn fulfill_payout(
    ...
    let merchant_id = &merchant_account.merchant_id;
    let payout_create = &payout_data.payout_create;
    let payout_id = &payout_create.payout_id;
    match router_data_resp.response {
        Ok(payout_response_data) => {
            ...
            let updated_payouts = storage::payout_create::PayoutCreateUpdate::StatusUpdate {
                connector_payout_id: payout_create.connector_payout_id.to_owned(),
                status,
                error_code: None,
                error_message: None,
                is_eligible: payout_response_data.payout_eligible,
