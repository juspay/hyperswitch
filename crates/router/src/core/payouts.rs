...
pub async fn trigger_webhook_and_handle_response(
    ...
    utils::trigger_payouts_webhook(state, platform, &response, payout_data.payouts.created_by.as_ref()).await?;
...
pub async fn payout_create_db_entries(
    ...
    let (payout_method_id, payout_type) = match stored_payout_method_data {
        Some(payout_method_data) => (
            ...
            Some(api_enums::PayoutType::foreign_try_from(payout_method_data)?),
        ),
        None => {
            (
                payment_method
