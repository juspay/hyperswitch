...
pub async fn payouts_create_core(
    ...
    let (payout_create, payouts, billing_address) =
...
pub async fn call_connector_payout(
    ...
    billing_address: &Option<storage::address::Address>,
) -> RouterResponse<payouts::PayoutCreateResponse> {
...
pub async fn validate_request_and_form_payout_create(
    ...
    Option<storage::address::Address>,
)> {
    let db = &*state.store;
    let (
        billing_address,
    ...
    let (name, phone, phone_country_code, address) =
        billing_address
            ...
            .map_or((None, None, None, None), |a| {
                let name = format!(
                    "{} {}",
                    a.first_name.to_owned().unwrap_or_default().peek(),
                    a.last_name.to_owned().unwrap_or_default().peek()
                );
                let phone = a
                    .phone_number
                    .to_owned()
