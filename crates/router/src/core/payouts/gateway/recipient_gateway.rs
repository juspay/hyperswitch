    ...
    > {
        ...
        let updated_router_data = Box::pin(unified_connector_service::ucs_logging_wrapper_granular(
            ...
            |mut router_data, granular_payout_create_recipient_request, grpc_headers| async move {
                ...
                .await
                .attach_printable("Failed to create_recipient payout")?;

                let payout_create_recipient_response = response.into_inner();

                let ucs_data = unified_connector_service::handle_unified_connector_service_response_for_payout_create_recipient(
