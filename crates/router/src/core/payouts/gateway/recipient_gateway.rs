...
// =============================================================================
// PayoutGateway Implementation for domain::PoRecipient
// =============================================================================
        ...
        + RouterDataConversion<Self, types::PayoutsData, types::PayoutsResponseData>,
{
    ...
    > {
        ...
        logger::debug!("Granular Gateway: Payout recipient flow");
        let granular_payout_create_recipient_request =
            payments_grpc::PayoutServiceCreateRecipientRequest::foreign_try_from(router_data)
                .change_context(ConnectorError::RequestEncodingFailed)
                .attach_printable("Failed to construct Payout Create Recipient Request")?;
        ...
        let updated_router_data = Box::pin(unified_connector_service::ucs_logging_wrapper_granular(
            ...
            |mut router_data, granular_payout_create_recipient_request, grpc_headers| async move {
                let response = match Box::pin(client.payout_create_recipient(
                    granular_payout_create_recipient_request,
                    ...
                    Err(report) => {
                            ...
                            report.current_context()
                        {
                            ...
                            return Ok((
                                router_data,
                                (),
                                payments_grpc::PayoutServiceCreateRecipientResponse::default(),
                            ));
                        }
                        return Err(report.attach_printable("Failed to create payout recipient"));
                    }
                };

                let payout_create_recipient_response = response.into_inner();

                let ucs_data = types::UcsPayoutCreateRecipientResponseData::foreign_try_from((
                    payout_create_recipient_response.clone(),
                ...
                Ok((router_data, (), payout_create_recipient_response))
            },
        ))
        .await
