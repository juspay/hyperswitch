...
// =============================================================================
// PayoutGateway Implementation for domain::PoCreate
// =============================================================================
    ...
    > {
        ...
        let resource_id = id_type::PayoutResourceId::from_str(router_data.attempt_id.as_str())
            ...
            .inspect_err(|err| {
                logger::warn!(
                    error = ?err,
                    "Invalid Payout Resource Id found"
                )
            })
            .ok()
        ...
        logger::debug!("Granular Gateway: Payout create flow");
        let granular_payout_create_request =
            payments_grpc::PayoutServiceCreateRequest::foreign_try_from(router_data)
                .change_context(ConnectorError::RequestEncodingFailed)
                .attach_printable("Failed to construct Payout Create Request")?;
        ...
        let updated_router_data = Box::pin(unified_connector_service::ucs_logging_wrapper_granular(
            ...
            |mut router_data, granular_payout_create_request, grpc_headers| async move {
                ...
                let ucs_data = unified_connector_service::handle_unified_connector_service_response_for_payout_create(
                    payout_create_response.clone(),
                ...
                Ok((router_data, (), payout_create_response))
            },
        ))
        .await
