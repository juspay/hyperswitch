...
// Qualifying UCS failures seen by the kill switch, emitted whether or not it is armed so the
...
// Scopes actually cut over from UCS to the direct integration.
counter_metric!(UCS_KILL_SWITCH_CUT_OVER, GLOBAL_METER);
