...
) -> CustomResult<(CardNetworkTokenResponsePayload, Option<String>), errors::NetworkTokenizationError>
{
    ...
    let jwt = encryption::encrypt_jwe(payload_bytes, enc_key, EncryptionAlgorithm::A128GCM, Some(key_id.as_str()))
        .await
...
pub async fn get_token_from_tokenization_service(
    ...
    let payload = GetCardToken {
        ...
        customer_id: pm_data.customer_id.clone(),
    };
    ...
    let res: TokenResponse = response
        ...
        .change_context(errors::NetworkTokenizationError::FetchNetworkTokenFailed)
        .map_err(|e| {
            logger::error!(fetch_err=?e);
    ...
    let network_token_data = NetworkTokenData {
        ...
        token_cryptogram: Some(res.authentication_details.cryptogram),
        ...
        token_exp_year: token_decrypted
            ...
            .expiry_year
            .unwrap_or_default(),
        nick_name: token_decrypted.clone().unwrap().card_holder_name,
        card_issuer: None,
