...
use storage_models::{encryption::Encryption, enums as storage_enums, payment_method};
...
use crate::{
    ...
    types::{
        ...
        domain::{
            self,
            types::{self as domain_types, decrypt},
        },
        storage::{self, enums},
...
pub async fn get_payment_method_from_hs_locker<'a>(
    ...
    if !locker.mock_locker {
        ...
        let get_card_resp =
            mock_get_payment_method(&*state.store, payment_method_reference).await?;
...
pub async fn mock_get_payment_method<'a>(
    ...
    let dec_data = if let Some(e) = locker_mock_up.enc_card_data {
        // Fetch key
        let key = domain_types::get_merchant_enc_key(db, locker_mock_up.merchant_id.clone())
            .await
            .change_context(errors::VaultError::ResponseDeserializationFailed)
        ...
        // Decrypt
        async { decrypt(Some(Encryption::new(decoded_bytes)), &key).await }
            .await
            .change_context(errors::VaultError::FetchPaymentMethodFailed)?
            .map_or(
