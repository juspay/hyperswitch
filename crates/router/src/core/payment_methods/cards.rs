...
=======
    ...
    logger::info!(
        "<<>>payload data customer_id{:?} mandate_id {:?}",
        &req.customer_id,
        &req.mandate_id,
    );
    logger::info!(
        "<<>>payment_method_responses v1{:?}",
...
pub async fn filter_payment_methods_v2(
    ...
    for payment_method in payment_methods.iter() {
        ...
        if let Ok(payment_methods_enabled) = parse_result {
            ...
            //TODO: @DG check if we get allowed_payment_method_types from somewhere else and replace here
