...
use payment_methods::client::{
    ...
    CreatePaymentMethod, PaymentMethodClient, RetrievePaymentMethod, UpdatePaymentMethod,
...
pub async fn retrieve_pm_modular_service_call(
    ...
    let pm_response = RetrievePaymentMethod::call(state, &client, payment_method_fetch_req)
        .await
        .map_err(|err| {
...
pub async fn create_pm_modular_service_call(
    ...
    //Modular service call
    let pm_response = CreatePaymentMethod::call(state, &client, payment_method_create_req)
        .await
        .map_err(|err| {
            logger::error!("Error in creating payment method: {:?}", err);
            errors::ApiErrorResponse::InternalServerError
