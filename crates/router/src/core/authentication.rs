...
pub async fn perform_authentication(
    ...
    .await?;
    let authentication =
        utils::update_trackers(state, response.clone(), authentication_data, None, merchant_key_store).await?;
    response
        .response
...
pub async fn perform_post_authentication(
    ...
    payment_id: &common_utils::id_type::PaymentId,
) -> CustomResult<hyperswitch_domain_models::router_request_types::authentication::AuthenticationStore, ApiErrorResponse> {
    ...
    let authentication_update = if !authentication.authentication_status.is_terminal_status() && is_pull_mechanism_enabled {
        let router_data = transformers::construct_post_authentication_router_data( // authenticate
            state,
...
pub async fn perform_pre_authentication(
    ...
    let authentication = if authentication_connector.is_separate_version_call_required() {
        ...
        let updated_authentication =
            utils::update_trackers(state, router_data, authentication, acquirer_details.clone(), key_store)
                .await?;
    ...
    let authentication_update = utils::update_trackers(state, router_data, authentication, acquirer_details, key_store).await?;

    Ok(hyperswitch_domain_models::router_request_types::authentication::AuthenticationStore{
        authentication: authentication_update,
        cavv: None, // since cavv wont be present in pre_authentication step
    })
}
