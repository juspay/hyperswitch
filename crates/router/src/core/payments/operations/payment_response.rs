        ...
        Ok(payments_response) => {
            ...
            match router_data.integrity_check.clone() {
                ...
                Ok(()) => {
                    ...
                    let flow_name = core_utils::get_flow_name::<F>()?;
                    let updated_attempt_status = if flow_name == "PSync"
                        && attempt_status.is_terminal_status()
                    {
                        // For PSync flows, if payment is already in terminal state, don't update the status
                        attempt_status
                    } else {
                        let default_attempt_status = router_data.get_attempt_status_for_db_update(
                            &payment_data,
                        ...
                        payment_data
                            .frm_message
                            .as_ref()
                            .map(|frm| match frm.frm_status {
                                enums::FraudCheckStatus::Fraud
                                | enums::FraudCheckStatus::ManualReview => attempt_status,
                                _ => default_attempt_status,
                            })
                            .unwrap_or(default_attempt_status)
                    };
