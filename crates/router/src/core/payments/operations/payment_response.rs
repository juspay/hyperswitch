...
fn is_valid_stripe_checkout_transaction_id_transition(
    ...
    // Checkout Session 响应包含会话 ID；PaymentIntent webhook 不包含该 ID，
    // 但已通过 metadata.order_id 关联当前尝试，因此只允许从 cs_* 单向切换到 pi_*。
        ...
        Ok(payments_response) => {
            ...
            match router_data.integrity_check.clone() {
                ...
                Ok(()) => {
                    ...
                    // 优惠可能使 Stripe Checkout 实付金额低于原订单金额，
                    // 但已支付会话仍属于全额成功，而非部分支付。
                    ...
                    // Checkout 成功后仍可能收到过期或重复事件，应单调保留成功状态与已收敛交易 ID。
