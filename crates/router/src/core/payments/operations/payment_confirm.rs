...
impl<F: Clone + Send + Sync> Domain<F, api::PaymentsRequest, PaymentData<F>> for PaymentConfirm {
                        ...
                        .and_then(|pmd| pmd.payment_method_data.as_ref())
                    {
                        ...
                        | Some(api_models::payments::PaymentMethodData::Wallet(_)) => {
                            ...
                            if should_create {
                                ...
                                logger::info!("Skipping wallet PM creation - setup_future_usage is not off_session");
                            }
                        }
                        _ => {
    ...
    //     mandate_type: Option<MandateTransactionType>,
    // ) -> CustomResult<(), errors::ApiErrorResponse> {
    //     let authentication_flow = helpers::decide_action_for_unified_authentication_service(
    //         state,
    //         platform.get_processor(),
    //         business_profile,
    //         payment_data,
    ...
    //     if let Some(authentication_flow) = authentication_flow {
    ...
    //         // AuthenticationCreate step: initiate the modular-auth session.
    //         // We run this and pre_authentication back-to-back and persist
    //         // both in a single DB write after pre_authentication completes.
    //         let connector_authentication_id =
    //             uas_utils::types::ModularAuthentication::authentication_create(
    //                 state,
    ...
    //         let mut pre_auth_response = uas_utils::types::ExternalAuthentication::pre_authentication(
    //                     state,
    //                     &payment_data.payment_attempt.merchant_id,
    //                     Some(&payment_data.payment_intent.payment_id),
    ...
    //         if let Ok(unified_authentication_service::UasAuthenticationResponseData::PreAuthentication {
    //             authentication_details,
    //         }) = &mut pre_auth_response.response
    //         {
    //             authentication_details.connector_authentication_id =
    ...
    async fn call_unified_authentication_service_if_eligible<'a>(
        ...
        if let Some(unified_authentication_service_flow) = unified_authentication_service_flow {
            ...
            let acquirer_details = api_models::authentication::AcquirerDetails {
            ...
            let customer_details = api_models::authentication::CustomerDetails {
                email: email,
                shipping: shipping.clone(),
                billing: billing.clone(),
            };  

            let authentication_create_response = uas_utils::types::ExternalAuthentication::authentication_create(
               state,
               authentication_connector_name,
               payment_data.payment_attempt.amount,
               payment_data.payment_intent.currency.ok_or(
                errors::ApiErrorResponse::InternalServerError
               ).attach_printable("currency not found in payment_intent")?,
               Some(return_url),
               force_3ds_challenge,
               psd2_sca_exemption_type,
               authentication
...
impl<F: Clone + Sync> UpdateTracker<F, PaymentData<F>, api::PaymentsRequest> for PaymentConfirm {
    ...
    async fn update_trackers<'b>(
        ...
        _dimensions: &dimension_state::DimensionWithProcessorAndProviderMerchaantId,
    ) -> RouterResult<(
