    ...
    for PaymentExternalVaultProxyConfirm
{
    ...
    async fn get_trackers<'a>(
        ...
        // both proceed from `RequiresCustomerAction`. Gating it (like the standard confirm does)
        // breaks the frictionless resume with IR_16. Only terminal / in-flight states are rejected.
    ...
    for PaymentExternalVaultProxyConfirm
{
    ...
    async fn create_payment_method(
        ...
        // lives in the modular service, so an ineligible org must skip create too, not write a PM
        // that can never be fetched back.
        match feature_config.is_payment_method_modular_allowed {
            true => {
                ...
                // Normalise card_exp_year to 2 digits (YY) before vaulting: Netcetera's cardExpiryDate
                // is YYMM, so a stored 4-digit year yields a 6-char value the AReq rejects. Vault
