...
pub struct WebhookWebhookPaymentStatus;

impl<F: Send + Clone> Operation<F, api::PaymentsRequest> for WebhookPaymentStatus {
...
#[async_trait]
impl<F: Clone + Send> Domain<F, api::PaymentsRequest> for WebhookPaymentStatus {
    #[instrument(skip_all)]
...
#[async_trait]
impl<F: Clone> UpdateTracker<F, PaymentData<F>, api::PaymentsRequest> for WebhookPaymentStatus {
...
#[async_trait]
impl<F: Clone> UpdateTracker<F, PaymentData<F>, api::PaymentsRetrieveRequest> for WebhookPaymentStatus {
    async fn update_trackers<'b>(
        &'b self,
        _db: &dyn StorageInterface,
        payment_data: PaymentData<F>,
    ...
    for WebhookPaymentStatus
{
    ...
    async fn get_trackers<'a>(
        ...
        let (operation, payment_data, customer) = get_tracker_for_sync(
            payment_id,
            merchant_account,
            key_store,
            &*state.store,
        ...
        .await?;
        Ok((Box::new(operation), payment_data, customer))
    }
}
...
impl<F: Send + Clone> ValidateRequest<F, api::PaymentsRetrieveRequest> for WebhookPaymentStatus {
