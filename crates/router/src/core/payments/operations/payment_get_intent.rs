...
impl<F: Send + Clone> Operation<F, PaymentsGetIntentRequest> for &PaymentGetIntent {
    ...
    fn to_validate_request(
        ...
        &(dyn ValidateRequest<F, PaymentsGetIntentRequest, Self::Data> + Send + Sync),
    > {
        Ok(*self)
    }
    fn to_get_tracker(
    ...
    for PaymentGetIntent
{
    ...
    > {
        ...
        let payment_intent = db.find_payment_intent_by_id(key_manager_state, &request.id, key_store, storage_scheme).await.change_context(errors::ApiErrorResponse::InternalServerError).attach_printable("Code fatt gaya")?;
