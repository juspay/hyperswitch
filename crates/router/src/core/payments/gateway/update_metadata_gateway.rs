...
use crate::{
    core::{
        ...
        unified_connector_service::{
            self, handle_unified_connector_service_response_for_update_metadata,
        },
    },
    routes::SessionState,
    services::logger,
...
// =============================================================================
// PaymentGateway Implementation for domain::UpdateMetadata
// =============================================================================

/// Implementation of PaymentGateway for api::UpdateMetadata flow
#[async_trait]
impl<RCD>
    ...
    > for domain::UpdateMetadata
where
    RCD: Clone
        + Send
        + Sync
        + 'static
        + RouterDataConversion<Self, types::PaymentsUpdateMetadataData, types::PaymentsResponseData>,
{
    async fn execute(
        self: Box<Self>,
        state: &SessionState,
        ...
        RouterData<Self, types::PaymentsUpdateMetadataData, types::PaymentsResponseData>,
        ConnectorError,
    > {
        ...
        let update_metadata_request =
            payments_grpc::PaymentServiceUpdateMetadataRequest::foreign_try_from(router_data)
                .change_context(ConnectorError::RequestEncodingFailed)
                .attach_printable("Failed to construct Payment UpdateMetadata Request")?;
        ...
        let updated_router_data =
            Box::pin(unified_connector_service::ucs_logging_wrapper_granular(
                ...
                |mut router_data, update_metadata_request, grpc_headers| async move {
                    let response = Box::pin(client.update_metadata(
                        update_metadata_request,
                        connector_auth_metadata,
                        grpc_headers,
                    ))
                    .await
                    .attach_printable("Failed to update metadata")?;

                    let update_metadata_response = response.into_inner();

                    let (router_data_response, status_code) =
                        handle_unified_connector_service_response_for_update_metadata(
                            update_metadata_response.clone(),
                        )
                    ...
                    Ok((router_data, (), update_metadata_response))
                },
            ))
            .await
...
/// Implementation of FlowGateway for api::UpdateMetadata
///
