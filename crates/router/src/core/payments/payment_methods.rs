    ...
    let customer_payment_methods = match &payment_intent.customer_id {
        Some(customer_id) => Some(
            payment_methods::list_customer_payment_methods_core(
                ...
                customer_id,
                false, // fetch_new_payment_methods field is false because we want to fetch saved payment methods for the customer, and not fetch new payment methods based on the current request
            )
            .await?,
        ),
        None => None,
