...
#[cfg(feature = "v1")]
/// Get the business details (business_country, business_label) for a profile from the
/// `primary_business_details` configured in the merchant account
///
/// The business details are resolved as follows:
/// 1. The entry whose `{country}_{business}` matches the profile name, since profiles created from
///    `primary_business_details` are named this way
...
pub fn get_business_details_from_primary_business_details(
    primary_business_details: &[api_models::admin::PrimaryBusinessDetails],
    profile_name: &str,
) -> Option<(api_enums::CountryAlpha2, String)> {
    primary_business_details
        .iter()
        .find(|business_details| {
...
#[cfg(feature = "v1")]
/// Get the business details (business_country, business_label) configured in the merchant account
/// for the given business profile
pub fn get_business_details_from_merchant_account(
    merchant_account: &domain::MerchantAccount,
    business_profile: &domain::Profile,
) -> RouterResult<Option<(api_enums::CountryAlpha2, String)>> {
    ...
    Ok(get_business_details_from_primary_business_details(
        &primary_business_details,
...
#[cfg(feature = "v1")]
/// Get the business details (business_country, business_label) to be populated in the payment intent
///
/// The business details are resolved in the following order of precedence:
/// 1. The business details already present in the payment intent (passed in the payment request)
...
///
/// Business details are always resolved as a pair, so that a `connector_label` is never generated
/// from a business country and business label which do not belong together. Hence `None` is
/// returned if either of the business details is already present in the payment intent, or if no
/// business details could be resolved from the merchant connector account or the merchant account.
pub fn get_business_details_for_payment_intent(
    payment_intent: &PaymentIntent,
    merchant_connector_account: Option<&MerchantConnectorAccountType>,
    merchant_account: &domain::MerchantAccount,
    business_profile: &domain::Profile,
) -> RouterResult<Option<(api_enums::CountryAlpha2, String)>> {
    if payment_intent.business_country.is_some() || payment_intent.business_label.is_some() {
        return Ok(None);
    }

    match merchant_connector_account.and_then(MerchantConnectorAccountType::get_business_details) {
        Some(business_details) => Ok(Some(business_details)),
        None => get_business_details_from_merchant_account(merchant_account, business_profile),
    }
}

#[cfg(feature = "v1")]
/// Populate the business details (business_country, business_label) in the payment intent, if they
/// were not passed in the payment request. Refer to [`get_business_details_for_payment_intent`]
/// for the order of precedence
pub fn populate_business_details_in_payment_intent(
    payment_intent: &mut PaymentIntent,
    merchant_connector_account: Option<&MerchantConnectorAccountType>,
    merchant_account: &domain::MerchantAccount,
    business_profile: &domain::Profile,
) -> RouterResult<()> {
    if let Some((business_country, business_label)) = get_business_details_for_payment_intent(
        payment_intent,
...
mod tests {
    ...
    fn primary_business_details(
        details: &[(api_enums::CountryAlpha2, &str)],
    ) -> Vec<api_models::admin::PrimaryBusinessDetails> {
        details
            .iter()
            .map(
                |(country, business)| api_models::admin::PrimaryBusinessDetails {
                    country: *country,
    ...
    #[test]
    fn test_business_details_from_single_primary_business_details() {
        let details = primary_business_details(&[(api_enums::CountryAlpha2::US, "default")]);

        assert_eq!(
            get_business_details_from_primary_business_details(&details, "default"),
            Some((api_enums::CountryAlpha2::US, "default".to_string()))
        );
    }

    #[test]
    fn test_business_details_from_primary_business_details_matching_profile_name() {
        let details = primary_business_details(&[
            (api_enums::CountryAlpha2::US, "food"),
            (api_enums::CountryAlpha2::GB, "shop"),
        ]);

        assert_eq!(
            get_business_details_from_primary_business_details(&details, "GB_shop"),
            Some((api_enums::CountryAlpha2::GB, "shop".to_string()))
        );
    }

    #[test]
    fn test_business_details_from_ambiguous_primary_business_details() {
        let details = primary_business_details(&[
            (api_enums::CountryAlpha2::US, "food"),
            (api_enums::CountryAlpha2::GB, "shop"),
        ]);
    ...
    #[test]
    fn test_business_details_from_empty_primary_business_details() {
        assert_eq!(
            get_business_details_from_primary_business_details(&[], "default"),
            None
        );
    }
...
> {
    ...
    match pm_data {
        ...
        domain::PaymentMethodData::DecryptedWalletTokenDetailsForNetworkTransactionId(
            decrypted_wallet_token_with_ntid,
        ) => Ok(Some(
            api_models::payments::AdditionalPaymentData::NetworkToken(Box::new(
                decrypted_wallet_token_with_ntid.to_owned().into(),
            )),
        )),
    }
}
...
pub fn get_key_params_for_surcharge_details(
    ...
    match payment_method_data {
        ...
        domain::PaymentMethodData::CardWithOptionalCVC(card) => Some((
            common_enums::PaymentMethod::Card,
            common_enums::PaymentMethodType::Credit,
            card.card_network.clone(),
        )),
