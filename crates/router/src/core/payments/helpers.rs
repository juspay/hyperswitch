...
// ==================== ALT-ID INTEGRATION (Guest Checkout Tokenization) ====================

/// Check if the card is a domestic Indian card (required for Alt-ID per RBI guidelines)
fn is_domestic_indian_card(card_issuing_country: Option<&String>) -> bool {
    card_issuing_country
        .map(|country| country.to_lowercase() == "india" || country.to_uppercase() == "IN")
        .unwrap_or(false)
}

/// Check if the merchant is Indian (required for Alt-ID per RBI guidelines)
fn is_indian_merchant(business_profile: &domain::Profile) -> bool {
    business_profile
        .merchant_business_country
        .as_ref()
        .map(|country| *country == api_models::enums::CountryAlpha2::IN)
        // Default to true if country not set (same as Euler behavior)
        .unwrap_or(true)
}
...
/// - Supported card networks (Visa, Mastercard, Amex, Diners, RuPay)
#[cfg(feature = "v1")]
pub async fn try_get_altid_for_guest_checkout(
...
) -> Option<domain::NetworkTokenData> {
    use crate::core::payment_methods::network_tokenization;

    // Check if merchant is Indian (RBI guidelines only apply to Indian merchants)
    if !is_indian_merchant(business_profile) {
        logger::debug!("Non-Indian merchant, skipping Alt-ID (not required)");
        return None;
    }

    // Check if card is domestic Indian card (Alt-ID only for cards issued in India)
    if !is_domestic_indian_card(card.card_issuing_country.as_ref()) {
        logger::debug!(
            "Non-domestic card (issuing_country: {:?}), skipping Alt-ID",
            card.card_issuing_country
        );
        return None;
    }

    // Check if network tokenization is enabled for the profile
    if !business_profile.is_network_tokenization_enabled {
        logger::debug!("Network tokenization not enabled for profile, skipping Alt-ID");
        return None;
    }

    // Check if the card network supports Alt-ID
    let card_network = card.card_network.as_ref()?;
    if !network_tokenization::ALTID_SUPPORTED_NETWORKS.contains(card_network) {
        logger::debug!(
            "Alt-ID not supported for network {:?}, skipping",
            card_network
        );
        return None;
    }

    // Check if Alt-ID URL is configured
    let tokenization_service = state.conf.network_tokenization_service.as_ref()?;
    if tokenization_service.get_inner().fetch_altid_url.is_none() {
        logger::debug!("Alt-ID URL not configured, skipping Alt-ID");
        return None;
    }

    // Convert amount to f64 for the API (amount is in minor units)
    let amount_f64 = amount as f64 / 100.0;

    // Build card detail for Alt-ID
    let card_detail = domain::CardDetail {
    ...
    match network_tokenization::get_altid_for_card(
        state,
        &card_detail,
    ...
    .await
    {
        Ok(Some(network_token_data)) => {
            logger::info!("Successfully fetched Alt-ID for guest checkout");
            Some(network_token_data)
        }
        Ok(None) => {
            logger::debug!("Alt-ID not available, falling back to raw card");
            None
        }
        Err(err) => {
            logger::warn!(
