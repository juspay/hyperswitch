...
pub(crate) fn validate_stripe_hosted_checkout_request(
    ...
    {
        return Err(invalid_request(
            "Stripe hosted checkout does not accept an existing mandate or off-session payment",
        ));
    }
    if request
        .setup_future_usage
        .is_some_and(|usage| usage != api_enums::FutureUsage::OffSession)
    {
        return Err(invalid_request(
            "Stripe hosted checkout only supports `off_session` future usage",
        ));
    }
