...
pub async fn add_ach_pending_promotion_task_if_eligible(
    state: &SessionState,
    payment_attempt: &PaymentAttempt,
    requeue: bool,
        ...
        && payment_attempt.payment_method_type == Some(storage_enums::PaymentMethodType::Ach)
    {
        ...
        // Use the provided schedule_time if available (though logic usually derives it from created_at
        // but for safety/consistency we might double check. The original logic used created_at + window).
        // The original logic was:
        // let ach_schedule_time = payment_attempt.created_at.saturating_add(...)
        // So we should preserve that regardless of the passed 'schedule_time' (which is probably for the main task).
        // Wait, the original code didn't use 'schedule_time' for the ACH task, it calculated it fresh.
        // So I will keep the original logic.

        let ach_schedule_time = payment_attempt
            .created_at
        ...
        .await
        .change_context(errors::ApiErrorResponse::InternalServerError)
        .attach_printable("Failed while adding ACH pending promotion task to process tracker")?;
    }
    Ok(())
}
