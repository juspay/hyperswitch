        ...
        loop {
            ...
            match get_gsm_decision(gsm) {
                api_models::gsm::GsmDecision::Retry => {
                    ...
                    match (payment_data.get_payment_intent().setup_future_usage, payment_data.get_payment_intent().off_session) {
                        (Some(storage_enums::FutureUsage::OffSession), _)
                        | (_, Some(true)) => {
                            let current_connector_routing_data = super::get_connector_data(&mut connector_routing_data)?;
                            let payment_method_info = payment_data
                                .get_payment_method_info()
                                .get_required_value("payment_method_info")?
                                .clone();
                            let mandate_reference_id = payments::get_mandate_reference_id(current_connector_routing_data.action_type.clone(), current_connector_routing_data, payment_data, &payment_method_info)?;
