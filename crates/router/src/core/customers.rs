    ...
    async fn create_domain_model_from_request<'a>(
        ...
        let domain_customer = async {
            Ok(customer_domain::Customer {
                customer_id: merchant_reference_id.to_owned().unwrap_or_default(), // Should we raise an error in case of V1 since this if is supposed to be generated by our application if not provided in request
