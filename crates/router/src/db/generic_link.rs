...
impl GenericLinkInterface for Store {
    ...
    async fn find_generic_link_by_link_id(
        ...
        storage::GenericLink::find_generic_link_by_link_id(&mut conn, link_id)
            .await
    ...
    async fn find_pm_collect_link_by_link_id(
        ...
        let mut conn = connection::pg_connection_read(self).await?;
        storage::GenericLink::find_pm_collect_link_by_link_id(&mut conn, link_id)
            .await
            .map_err(|error| report!(errors::StorageError::from(error)))
    }
    ...
    async fn find_payout_link_by_link_id(
        ...
        let mut conn = connection::pg_connection_read(self).await?;
        storage::GenericLink::find_payout_link_by_link_id(&mut conn, link_id)
            .await
            .map_err(|error| report!(errors::StorageError::from(error)))
    }
    ...
    async fn insert_pm_collect_link(
        ...
        let mut conn = connection::pg_connection_write(self).await?;
        pm_collect_link
            .insert_pm_collect_link(&mut conn)
            .await
            .map_err(|error| report!(errors::StorageError::from(error)))
    }
    ...
    async fn update_payout_link(
        ...
        let mut conn = connection::pg_connection_write(self).await?;
        payout_link
            .update_payout_link(&mut conn, payout_link_update)
            .await
            .map_err(|error| report!(errors::StorageError::from(error)))
    }
}
