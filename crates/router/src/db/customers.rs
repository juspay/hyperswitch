...
where
    domain::Customer: Conversion<DstType = storage_types::Customer, NewDstType = storage_types::CustomerNew>,
{
...
mod storage{
    ...
    // check customer not found in kv and fallback to db
        ...
        storage_scheme: MerchantStorageScheme,
            ...
            let maybe_customer = match storage_scheme {
                ...
                MerchantStorageScheme::RedisKv => {
                    ...
                    Box::pin(db_utils::try_redis_get_else_try_database_get( // check for ValueNotFound
                        async {
                            kv_wrapper(self, KvOperation::<diesel_models::Customer>::HGet(&field),key)
                                .await?
                                .try_into_hget()
                                .map(|ok_res| Some(ok_res))
                        },
                        database_call,
                    ))
                    .await
            ...
            let maybe_result = maybe_customer.async_map(|c| async{
                c.convert(key_store.key.get_inner())
                    .await
                    .change_context(errors::StorageError::DecryptionError)
            }).await.transpose()?;

            maybe_result.map_or(Ok(None), |customer:domain::Customer| {
                match customer.name {
                ...
                MerchantStorageScheme::RedisKv => {
                    ...
                    let redis_entry = kv::TypedSql {
                        op : kv::DBOperation::Update {
                            updatable : kv::Updateable::CustomerUpdate(
        ...
        async fn find_customer_by_customer_id_merchant_id(
            ...
            let result: domain::Customer = customer.convert(key_store.key.get_inner())
                    .await
                    .change_context(errors::StorageError::DecryptionError)?;
                //.await
        ...
        async fn list_customers_by_merchant_id(
            ...
            let encrypted_customers = storage_types::Customer::list_by_merchant_id(&conn, merchant_id)
                .await
                .map_err(|error| report!(errors::StorageError::from(error)))?;
        ...
        async fn insert_customer(
            ...
            let create_customer = match storage_scheme {
                ...
                MerchantStorageScheme::RedisKv => {
                    ...
                    .try_into_hsetnx()
                    {
                        Ok(redis_interface::HsetnxReply::KeyNotSet) => Err(report!(errors::StorageError::DuplicateValue {
                            entity: "customer",
        ...
        async fn delete_customer_by_customer_id_merchant_id(
            ...
            let conn = connection::pg_connection_write(self).await?;
            storage_types::Customer::delete_by_customer_id_merchant_id(&conn, customer_id, merchant_id)
                .await
                .map_err(|error| report!(errors::StorageError::from(error)))
        }
...
mod storage{
    ...
    async fn update_customer_by_customer_id_merchant_id(
        ...
        customer_id: String,
        merchant_id: String,
        customer: storage_types::CustomerUpdate,
        key_store: &domain::MerchantKeyStore,
        _storage_scheme: MerchantStorageScheme,
    ) -> CustomResult<domain::Customer, errors::StorageError> {
        let conn = connection::pg_connection_write(self).await?;
    ...
    async fn find_customer_by_customer_id_merchant_id(
        ...
        let customer: domain::Customer =
            storage_types::Customer::find_by_customer_id_merchant_id(&conn, customer_id, merchant_id)
                .await
                .map_err(|error| report!(errors::StorageError::from(error)))
                .async_and_then(|c| async {
                    c.convert(key_store.key.get_inner())
