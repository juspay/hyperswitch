...
impl BatchSampleDataInterface for KafkaStore {
    async fn insert_payment_intents_batch_for_sample_data(
        ...
        for payment_intent in payment_intents.iter() {
            if let Err(er) = match self.kafka_producer() {
                Ok(kafka) => kafka.log_payment_intent(payment_intent, None).await,
                Err(e) => Err(e),
            } {
                crate::logger::error!(message="Failed to add analytics entry for Payment Intent {payment_intent:?} in sample data", error_message=?er);
            };
        }
        Ok(payment_intents)
    }
    ...
    async fn insert_payment_attempts_batch_for_sample_data(
        ...
        for payment_attempt in payment_attempts.iter() {
            if let Err(er) = match self.kafka_producer() {
                Ok(kafka) => kafka.log_payment_attempt(payment_attempt, None).await,
                Err(e) => Err(e),
            } {
                crate::logger::error!(message="Failed to add analytics entry for Payment Attempt {payment_attempt:?} in sample data", error_message=?er);
            };
        }
        Ok(payment_attempts)
    }
    ...
    async fn insert_refunds_batch_for_sample_data(
        ...
        for refund in refunds.iter() {
            if let Err(er) = match self.kafka_producer() {
                Ok(kafka) => kafka.log_refund(refund, None).await,
                Err(e) => Err(e),
            } {
                crate::logger::error!(message="Failed to add analytics entry for Refund {refund:?} in sample data", error_message=?er);
            };
        }
        Ok(refunds)
    }
    ...
    async fn delete_payment_intents_for_sample_data(
        ...
        for payment_intent in payment_intents.iter() {
            if let Err(er) = match self.kafka_producer() {
                Ok(kafka) => kafka.log_payment_intent_delete(payment_intent).await,
                Err(e) => Err(e),
            } {
                crate::logger::error!(message="Failed to delete analytics entry for Payment Intent {payment_intent:?} in sample data", error_message=?er);
            };
        }
        Ok(payment_intents)
    }
    ...
    async fn delete_payment_attempts_for_sample_data(
        ...
        for payment_attempt in payment_attempts.iter() {
            if let Err(er) = match self.kafka_producer() {
                Ok(kafka) => kafka.log_payment_attempt_delete(payment_attempt).await,
                Err(e) => Err(e),
            } {
                crate::logger::error!(message="Failed to delete analytics entry for Payment Attempt {payment_attempt:?} in sample data", error_message=?er);
            };
        }
    ...
    async fn delete_refunds_for_sample_data(
        ...
        for refund in refunds.iter() {
            if let Err(er) = match self.kafka_producer() {
                Ok(kafka) => kafka.log_refund_delete(refund).await,
                Err(e) => Err(e),
            } {
                crate::logger::error!(message="Failed to delete analytics entry for Refund {refund:?} in sample data", error_message=?er);
            };
        }
