...
pub async fn execute_payment_create_test(
    ...
    match opt_test_request {
        Some(test_request) => {
            let payment_create_resp =
                call_and_read_body_json(&server, test_request.to_request()).await;
            PaymentCreate::verify_success_response(&payment_create_resp, master_data)
                .update_master_data(master_data, &payment_create_resp);
...
impl RequestBuilder for PaymentRetrieve {
    fn make_request_body(data: &MasterData) -> Option<TestRequest> {
        data.payments_retrieve
            .as_ref()
            .map(|_payments_retrieve_request| {
...
pub async fn execute_payment_retrieve_test(
    master_data: &mut MasterData,
    server: &impl Service<
        Request,
        Response = ServiceResponse<impl MessageBody>,
    ...
    match opt_test_request {
        Some(test_request) => {
            let payment_create_resp =
                call_and_read_body_json(&server, test_request.to_request()).await;
            PaymentRetrieve::verify_success_response(&payment_create_resp, master_data)
                .update_master_data(master_data, &payment_create_resp);
            //println!("{:?}",payment_create_resp);
...
impl RequestBuilder for PaymentCapture {
    fn make_request_body(data: &MasterData) -> Option<TestRequest> {
        data.payment_capture.as_ref().map(|_payment_capture_req| {
            ...
            TestRequest::get()
                .uri(&format!(
                    "http://localhost:8080/payments/{}/capture",
                    payment_id
                ))
                .insert_header(("api-key", data.api_key.as_ref().unwrap().as_str()))
        })
...
pub async fn execute_payment_confirm_test(
    master_data: &mut MasterData,
    server: &impl Service<
        Request,
        Response = ServiceResponse<impl MessageBody>,
    ...
    match opt_test_request {
        Some(test_request) => {
            let payment_confirm_resp =
                call_and_read_body_json(&server, test_request.to_request()).await;
            PaymentConfirm::verify_success_response(&payment_confirm_resp, master_data)
                .update_master_data(master_data, &payment_confirm_resp);
