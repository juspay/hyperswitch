...
pub async fn execute_merchant_account_create_test(
    ...
    match opt_test_req {
        Some(test_request) => {
            let merchant_account_create_resp =
                call_and_read_body_json(&server, test_request.to_request()).await;
            MerchantAccount::verify_success_response(&merchant_account_create_resp, master_data)
                .update_master_data(master_data, &merchant_account_create_resp);
...
pub async fn execute_merchant_account_delete_test(
    master_data: &mut MasterData,
    server: &impl Service<
        Request,
        Response = ServiceResponse<impl MessageBody>,
    ...
    match opt_test_req {
        Some(test_request) => {
            let merchant_account_delete_resp =
                call_and_read_body_json(&server, test_request.to_request()).await;
            MerchantAccountDelete::verify_success_response(
                &merchant_account_delete_resp,
                master_data,
            )
            .update_master_data(master_data, &merchant_account_delete_resp);
