...
use router::{configs::settings::Settings, oltp_lib, routes::AppState};
...
async fn spawn_server() -> bool {
    ...
    let (server, _state) = oltp_lib::start_oltp_server(conf)
        .await
...
pub async fn mk_service(
    ...
    actix_web::test::init_service(oltp_lib::mk_app(app_state, request_body_limit)).await
}
