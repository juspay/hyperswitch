...
mod punct {
    ...
    syn::custom_punctuation!(WeakGreaterThan, >?);
    syn::custom_punctuation!(StrongGreaterThanEqual, >==);
    syn::custom_punctuation!(WeakGreaterThanEqual, >=?);
    syn::custom_punctuation!(WeakLessThan, <?);
    syn::custom_punctuation!(StrongLessThanEqual, <==);
    syn::custom_punctuation!(WeakLessThanEqual, <=?);
}
...
enum ComparisonOperator {
    WeakEqual(punct::WeakEqual),
    Equal(Token![=]),
    StrongEqual(Token![==]),
    WeakNotEqual(punct::WeakNotEqual),
    NotEqual(Token![!=]),
    StrongNotEqual(punct::StrongNotEqual),
    WeakGreaterThan(punct::WeakGreaterThan),
    GreaterThan(Token![>]),
    StrongGreaterThan(Token![>>]),
    WeakGreaterThanEqual(punct::WeakGreaterThanEqual),
    GreaterThanEqual(Token![>=]),
    StrongGreaterThanEqual(punct::StrongGreaterThanEqual),
    WeakLessThan(punct::WeakLessThan),
    LessThan(Token![<]),
    StrongLessThan(Token![<<]),
    WeakLessThanEqual(punct::WeakLessThanEqual),
    LessThanEqual(Token![<=]),
    StrongLessThanEqual(punct::StrongLessThanEqual),
}

impl Parse for ComparisonOperator {
    fn parse(input: ParseStream<'_>) -> syn::Result<Self> {
        let lookahead = input.lookahead1();
        if lookahead.peek(punct::WeakGreaterThanEqual) {
            input.parse().map(Self::WeakGreaterThanEqual)
        } else if lookahead.peek(punct::StrongGreaterThanEqual) {
            input.parse().map(Self::StrongGreaterThanEqual)
        } else if lookahead.peek(Token![>=]) {
            input.parse().map(Self::GreaterThanEqual)
        } else if lookahead.peek(punct::WeakLessThanEqual) {
            input.parse().map(Self::WeakLessThanEqual)
        } else if lookahead.peek(punct::StrongLessThanEqual) {
            input.parse().map(Self::StrongLessThanEqual)
        } else if lookahead.peek(Token![<=]) {
            input.parse().map(Self::LessThanEqual)
        } else if lookahead.peek(punct::StrongNotEqual) {
            input.parse().map(Self::StrongNotEqual)
        } else if lookahead.peek(punct::WeakNotEqual) {
            input.parse().map(Self::WeakNotEqual)
        } else if lookahead.peek(Token![!=]) {
            input.parse().map(Self::NotEqual)
        } else if lookahead.peek(punct::WeakEqual) {
            input.parse().map(Self::WeakEqual)
        } else if lookahead.peek(Token![==]) {
            input.parse().map(Self::StrongEqual)
        } else if lookahead.peek(Token![=]) {
            input.parse().map(Self::Equal)
        } else if lookahead.peek(punct::WeakGreaterThan) {
            input.parse().map(Self::WeakGreaterThan)
        } else if lookahead.peek(Token![>>]) {
            input.parse().map(Self::StrongGreaterThan)
        } else if lookahead.peek(Token![>]) {
            input.parse().map(Self::GreaterThan)
        } else if lookahead.peek(punct::WeakLessThan) {
            input.parse().map(Self::WeakLessThan)
        } else if lookahead.peek(Token![<<]) {
            input.parse().map(Self::StrongLessThan)
        } else if lookahead.peek(Token![<]) {
...
impl ToTokens for ComparisonOperator {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        match self {
            Self::StrongGreaterThan(sgt) => sgt.to_tokens(tokens),
            Self::WeakGreaterThan(wgt) => wgt.to_tokens(tokens),
            Self::GreaterThan(gt) => gt.to_tokens(tokens),
            Self::StrongLessThan(slt) => slt.to_tokens(tokens),
            Self::WeakLessThan(wlt) => wlt.to_tokens(tokens),
            Self::LessThan(lt) => lt.to_tokens(tokens),
            Self::WeakEqual(weq) => weq.to_tokens(tokens),
            Self::Equal(eq) => eq.to_tokens(tokens),
            Self::StrongEqual(seq) => seq.to_tokens(tokens),
            Self::WeakNotEqual(wneq) => wneq.to_tokens(tokens),
            Self::NotEqual(neq) => neq.to_tokens(tokens),
            Self::StrongNotEqual(sneq) => sneq.to_tokens(tokens),
            Self::WeakGreaterThanEqual(wgte) => wgte.to_tokens(tokens),
            Self::StrongGreaterThanEqual(sgte) => sgte.to_tokens(tokens),
            Self::GreaterThanEqual(geq) => geq.to_tokens(tokens),
            Self::WeakLessThanEqual(wlte) => wlte.to_tokens(tokens),
            Self::StrongLessThanEqual(slte) => slte.to_tokens(tokens),
...
impl Parse for DomainDef {
    fn parse(input: ParseStream<'_>) -> syn::Result<Self> {
        ...
        let identifier_value: syn::LitStr = params.parse()?;
        let comma: Token![,] = params.parse()?;
        let description_value: syn::LitStr = params.parse()?;
        if !params.is_empty() {
            return Err(params.error("unexpected token found"));
        }
        let semi: Token![;] = input.parse()?;
...
struct Rule {
    rule: kw::rule,
    under: Option<(token::Paren, syn::Ident)>,
    colon: Token![:],
    judgement: AndChain,
    arrow: Token![->],
...
impl Parse for Rule {
    fn parse(input: ParseStream<'_>) -> syn::Result<Self> {
        ...
        let under = if input.peek(token::Paren) {
            let in_paren;
            let paren = syn::parenthesized!(in_paren in input);
            let domain_name: syn::Ident = in_paren.parse()?;
            if !in_paren.is_empty() {
                return Err(in_paren.error("unexpected token found"));
            }
            Some((paren, domain_name))
        } else {
            None
        };
...
impl ToTokens for Rule {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        if let Some((paren, domain_name)) = self.under.as_ref() {
...
impl From<EdgeStrength> for TokenStream {
    fn from(value: EdgeStrength) -> Self {
        match value {
            EdgeStrength::Weak => quote! { cgraph_prelude::Strength::Weak },
            EdgeStrength::Normal => quote! { cgraph_prelude::Strength::Normal },
            EdgeStrength::Strong => quote! { cgraph_prelude::Strength::Strong },
        }
    }
}

impl ToTokens for EdgeStrength {
    fn to_tokens(&self, tokens: &mut TokenStream) {
...
impl From<EdgeRelation> for TokenStream {
    fn from(value: EdgeRelation) -> Self {
        match value {
            EdgeRelation::Positive => quote! { cgraph_prelude::Relation::Positive },
            EdgeRelation::Negative => quote! { cgraph_prelude::Relation::Negative },
        }
    }
}
...
impl CompileCtx {
    ...
    fn compile_comparison(
        &mut self,
        comp: &Comparison,
    ) -> CompileResult<(syn::Ident, EdgeRelation, EdgeStrength, TokenStream)> {
        match &comp.value {
            ...
            ValueKind::Enum(enum_val) => {
                ...
                let code = quote! {
                    let #node_ident = #graph.make_value_node(
                        cgraph_prelude::NodeValue::Value(#val_ty::#key(#key::#enum_val)),
                        None,
                        None::<()>,
                    );
                };

                let strength = match &comp.op {
                    ComparisonOperator::WeakNotEqual(_) | ComparisonOperator::WeakEqual(_) => {
                        EdgeStrength::Weak
                    }

                    ComparisonOperator::NotEqual(_) | ComparisonOperator::Equal(_) => {
                        EdgeStrength::Normal
                    }

                    ComparisonOperator::StrongNotEqual(_) | ComparisonOperator::StrongEqual(_) => {
                        EdgeStrength::Strong
                    }

                    ComparisonOperator::WeakLessThan(_)
                    | ComparisonOperator::LessThan(_)
                    | ComparisonOperator::StrongLessThan(_)
                    | ComparisonOperator::WeakGreaterThan(_)
                    | ComparisonOperator::GreaterThan(_)
                    | ComparisonOperator::StrongGreaterThan(_)
                    | ComparisonOperator::WeakLessThanEqual(_)
                    | ComparisonOperator::LessThanEqual(_)
                    | ComparisonOperator::StrongLessThanEqual(_)
                    | ComparisonOperator::WeakGreaterThanEqual(_)
                    | ComparisonOperator::GreaterThanEqual(_)
                    | ComparisonOperator::StrongGreaterThanEqual(_) => {
                        return Err(syn::Error::new_spanned(
                            &comp.op,
                            "operator not supported for enum comparisons",
                        ));
                    }
                };
                ...
                let relation = if matches!(
                    ...
                    ComparisonOperator::NotEqual(_)
                        | ComparisonOperator::StrongNotEqual(_)
                        | ComparisonOperator::WeakNotEqual(_)
                ) {
                    EdgeRelation::Negative
                } else {
                    EdgeRelation::Positive
                };
                Ok((node_ident, relation, strength, code))
            }
        }
    }
    ...
    fn compile_any_clause(
        ...
        let code = quote! {
            let #node_ident = #graph.make_value_node(
                cgraph_prelude::NodeValue::Key(#key_ty::#key),
                None,
                None::<()>,
            );
        };
    ...
    fn compile_atom(
        ...
        match atom {
            ...
            Atom::Nested(nested) => self
                ...
                .map(|(id, ts)| (id, EdgeRelation::Positive, EdgeStrength::Strong, ts)),
        }
    }
    ...
    fn compile_or_chain(
        ...
        for member in chain.members.iter() {
            let (ident, relation, strength, ts) = self.compile_atom(member, domain)?;
            token_streams.push(ts);
            args.push(quote! { (#ident, #relation, #strength) });
        }

        let node_ident = self.next_ident();
        let domain_tokens = domain.map_or_else(
            || quote! { None::<cgraph_prelude::DomainId> },
            |id| quote! { Some(#id) },
        );
        let graph = &self.graph_ident;
        let code = quote! {
    ...
    fn compile_and_chain(
        ...
        for member in chain.members.iter() {
            ...
            token_streams.push(ts);
            args.push(quote! { (#ident, cgraph_prelude::Relation::Positive, cgraph_prelude::Strength::Strong) });
        }
    ...
    fn compile_conclusion(
        ...
        let (ident, relation, strength, ts) = match conclusion {
            Conclusion::AnyClause(any) => self
                .compile_any_clause(any)
                .map(|(id, rel, _, ts)| (id, rel, EdgeStrength::Normal, ts))?,
        ...
        if !matches!(strength, EdgeStrength::Normal) {
            return Err(syn::Error::new_spanned(
                conclusion,
                "can only use normal strength in the conclusion",
            ));
        }
    ...
    fn compile_rule(&mut self, rule: &Rule) -> CompileResult<TokenStream> {
        ...
        let code = quote! {
            ...
            #graph.make_edge(
                #judgement_ident,
                #conclusion_ident,
                cgraph_prelude::Strength::Normal,
                cgraph_prelude::Relation::Positive,
                #domain_tokens,
    ...
    fn compile(mut self, spec: GraphSpecCtx) -> CompileResult<TokenStream> {
        ...
        Ok(quote! {{
            #(#uses)*
            let mut #graph = cgraph_prelude::ConstraintGraphBuilder::new();
            #(#domain_defs)*
            #(#rule_defs)*
            #graph.build()
        }})
