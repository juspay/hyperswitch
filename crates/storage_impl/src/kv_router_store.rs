...
impl<T: DatabaseStore> KVRouterStore<T> {
    ...
    pub fn update_redis_request_id(&mut self, request_id: String) {
        self.router_store.update_redis_request_id(request_id);
    }
}
...
impl<T: DatabaseStore> KVRouterStore<T> {
        ...
        R: KvStorePartition,
    {
        ...
        // Attach the request id so the drainer stream append emits an
        // external service call event; this path bypasses get_redis_conn().
        let base_redis_conn = &self.router_store.cache_store.redis_conn;
        let redis_conn = match self.request_id.as_ref() {
            Some(request_id) if base_redis_conn.event_emitter.is_enabled() => {
                Arc::new(base_redis_conn.clone_with_request_id(request_id.clone()))
            }
            _ => Arc::clone(base_redis_conn),
        };
        redis_conn
            .stream_append_entry(
                &stream_name.into(),
