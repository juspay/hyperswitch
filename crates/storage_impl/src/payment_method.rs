...
impl<T: DatabaseStore> PaymentMethodInterface for RouterStore<T> {
    ...
    async fn find_payment_method_ids_by_billing_connector_subscription_id(
        ...
        let conn = pg_connection_read(self).await?;
        PaymentMethod::find_payment_method_ids_by_billing_connector_subscription_id(
            &conn,
            subscription_id,
        )
        .await
...
impl PaymentMethodInterface for MockDb {
    ...
    async fn find_payment_method_ids_by_billing_connector_subscription_id(
        ...
        let payment_methods = self.payment_methods.lock().await;
        let payment_method_ids: Vec<String> = payment_methods
            .iter()
            .filter(|pm| {
                pm.billing_connector_subscription_id
                    .as_ref()
