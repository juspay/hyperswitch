...
impl<T: DatabaseStore> PaymentAttemptInterface for RouterStore<T> {
    ...
    async fn find_payment_attempt_by_connector_transaction_id_payment_id_processor_merchant_id(
        ...
        let key_manager_state = self
            .get_keymanager_state()
            .attach_printable("Missing KeyManagerState")?;
        DieselPaymentAttempt::find_by_connector_transaction_id_payment_id_processor_merchant_id(
            &conn,
    ...
    async fn find_payment_attempt_last_successful_attempt_by_payment_id_processor_merchant_id(
        ...
        let key_manager_state = self
            .get_keymanager_state()
            .attach_printable("Missing KeyManagerState")?;
        DieselPaymentAttempt::find_last_successful_attempt_by_payment_id_processor_merchant_id(
            &conn,
            payment_id,
            processor_merchant_id,
        )
    ...
    async fn find_payment_attempt_last_successful_or_partially_captured_attempt_by_payment_id_processor_merchant_id(
        ...
        let key_manager_state = self
            .get_keymanager_state()
            .attach_printable("Missing KeyManagerState")?;
        DieselPaymentAttempt::find_last_successful_or_partially_captured_attempt_by_payment_id_processor_merchant_id(
            &conn,
            payment_id,
            processor_merchant_id,
        )
    ...
    async fn find_payment_attempt_by_processor_merchant_id_connector_txn_id(
        ...
        let key_manager_state = self
            .get_keymanager_state()
            .attach_printable("Missing KeyManagerState")?;
        DieselPaymentAttempt::find_by_processor_merchant_id_connector_txn_id(
            &conn,
            processor_merchant_id,
            connector_txn_id,
        )
    ...
    async fn find_payment_attempt_by_payment_id_processor_merchant_id_attempt_id(
        ...
        DieselPaymentAttempt::find_by_payment_id_processor_merchant_id_attempt_id(
            &conn,
            payment_id,
            processor_merchant_id,
            attempt_id,
    ...
    async fn get_filters_for_payments(
        ...
        DieselPaymentAttempt::get_filters_for_payments(
            &conn,
            intents.as_slice(),
            processor_merchant_id,
        )
    ...
    async fn find_attempts_by_processor_merchant_id_payment_id(
        ...
        let key_manager_state = self
            .get_keymanager_state()
            .attach_printable("Missing KeyManagerState")?;
        DieselPaymentAttempt::find_by_processor_merchant_id_payment_id(
            &conn,
            processor_merchant_id,
            payment_id,
        )
    ...
    async fn find_payment_attempt_by_attempt_id_processor_merchant_id(
        ...
        DieselPaymentAttempt::find_by_processor_merchant_id_attempt_id(
            &conn,
            processor_merchant_id,
            attempt_id,
        )
...
impl<T: DatabaseStore> PaymentAttemptInterface for KVRouterStore<T> {
        ...
        match storage_scheme {
            ...
            MerchantStorageScheme::RedisKv => {
                ...
                let created_attempt = PaymentAttempt {
                    ...
                    encrypted_payment_method_data: payment_attempt
        ...
        match storage_scheme {
            ...
            MerchantStorageScheme::RedisKv => {
                ...
                let updated_attempt = PaymentAttempt::convert_back(
                    key_manager_state,
                    payment_attempt.clone().to_storage_model().apply_changeset(
                        this.clone()
                            .convert()
                            .await
                            .change_context(errors::StorageError::EncryptionError)?,
    ...
    async fn find_payment_attempt_by_connector_transaction_id_payment_id_processor_merchant_id(
        ...
        .await;
        match storage_scheme {
            MerchantStorageScheme::PostgresOnly => self
                .router_store
            ...
            MerchantStorageScheme::RedisKv => {
                ...
                Box::pin(try_redis_get_else_try_database_get(
                    async {
                        let diesel_payment_attempt = Box::pin(kv_wrapper(self, KvOperation::<DieselPaymentAttempt>::HGet(&lookup.sk_id), key)).await?.try_into_hget()?;
                        PaymentAttempt::convert_back(
                            key_manager_state,
                            diesel_payment_attempt,
                            merchant_key_store.key.get_inner(),
                        ...
                        || async {self.router_store.find_payment_attempt_by_connector_transaction_id_payment_id_processor_merchant_id(connector_transaction_id, payment_id, processor_merchant_id, storage_scheme, merchant_key_store).await},
                    ))
                    .await
            }
        }
    ...
    async fn find_payment_attempt_last_successful_attempt_by_payment_id_processor_merchant_id(
        ...
        match storage_scheme {
            ...
            MerchantStorageScheme::RedisKv => {
                ...
                let redis_fut = async {
                    let kv_result = Box::pin(kv_wrapper::<DieselPaymentAttempt, _, _>(
                        self,
...
use common_utils::{
    ...
    types::{keymanager, keymanager::ToEncryptable, ConnectorTransactionIdTrait, CreatedBy},
};
#[cfg(feature = "v2")]
...
use hyperswitch_domain_models::payments::payment_attempt::{
    AttemptAmountDetails, AttemptAmountDetailsSetter,
};
use hyperswitch_domain_models::{
    payments::payment_attempt::EncryptedPaymentAttempt,
...
impl Conversion for PaymentAttempt {
    ...
    async fn convert(self) -> CustomResult<Self::DstType, ValidationError> {
        ...
        let tax_on_surcharge = amount_details.get_tax_on_surcharge();
        let surcharge_amount = amount_details.get_surcharge_amount();
        let order_tax_amount = amount_details.get_order_tax_amount();
        let shipping_cost = amount_details.get_shipping_cost();
        let amount_capturable = amount_details.get_amount_capturable();
        let amount_to_capture = amount_details.get_amount_to_capture();
        let amount_captured = amount_details.get_amount_captured();
        ...
        Self: Sized,
    {
        async {
            ...
            Ok::<Self, error_stack::Report<common_utils::errors::CryptoError>>(Self {
                ...
                id: storage_model.id,
                status: storage_model.status,
                amount_details: amount_details.into(),
                error,
                payment_method_id: storage_model.payment_method_id,
    ...
    fn foreign_from(update: PaymentAttemptUpdate) -> Self {
        match update {
            ...
            PaymentAttemptUpdate::ConfirmIntentResponse(confirm_intent_response_update) => {
                let hyperswitch_domain_models::payments::payment_attempt::ConfirmIntentResponseUpdate {
                    status,
                    connector_payment_id,
                    updated_by,
                    redirection_data,
