...
impl DatabaseStore for ReplicaStore {
    ...
    async fn new(
        ...
        let (master_config, replica_config, accounts_master_config, accounts_replica_config) =
            config;
        let schema_matches_accounts_schema =
            tenant_config.get_schema() == tenant_config.get_accounts_schema();

        let master_pool =
            diesel_make_pg_pool(&master_config, tenant_config.get_schema(), test_transaction)
                .await
                .attach_printable("failed to create master pool")?;
        let accounts_master_pool =
            if schema_matches_accounts_schema && master_config == accounts_master_config {
                master_pool.clone()
            } else {
                diesel_make_pg_pool(
        ...
        let accounts_replica_pool =
            if schema_matches_accounts_schema && replica_config == accounts_replica_config {
                replica_pool.clone()
            } else {
                diesel_make_pg_pool(
