...
mod tests {
    ...
    /// After stripping outer quotes, no lone ' must remain (all must be doubled '').
    /// A lone ' would terminate the string literal and allow SQL injection.
    fn has_no_lone_quote(literal: &str) -> bool {
    ...
    #[test]
    fn sanitize_before_after_contrast() {
    ...
    #[test]
    fn sanitize_escapes_backslash() {
        let lit = sanitize_sql_string_literal(r"foo\bar");
        assert!(lit.contains(r"\\"), "backslash not doubled: {lit}");
    }
    ...
    #[test]
    fn filter_type_equal_escapes_quotes() {
        assert_eq!(
            filter_type_to_sql("error_reason", FilterTypes::Equal, "foo'bar"),
            "error_reason = 'foo''bar'"
        );
    }

    #[test]
    fn filter_type_like_escapes_quotes() {
        assert_eq!(
            filter_type_to_sql("col", FilterTypes::Like, "foo'bar"),
            "col LIKE '%foo''bar%'"
        );
    }
}
