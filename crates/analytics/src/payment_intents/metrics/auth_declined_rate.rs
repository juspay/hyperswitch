...
use crate::{
    enums::AuthInfo,
    query::{
        Aggregate, Filter, FilterCombinator, FilterTypes, GroupByClause, QueryBuilder, QueryFilter,
        SeriesBucket, ToSql, Window,
    },
    types::{AnalyticsCollection, AnalyticsDataSource, MetricsError, MetricsResult},
};

#[derive(Default)]
pub(super) struct AuthDeclinedRate;

#[async_trait::async_trait]
impl<T> super::PaymentIntentMetric<T> for AuthDeclinedRate
where
    ...
    Window<&'static str>: ToSql<T>,
{
    ...
    ) -> MetricsResult<HashSet<(PaymentIntentMetricsBucketIdentifier, PaymentIntentMetricRow)>>
    {
        ...
        let or_clause = Filter::NestedFilter(
            FilterCombinator::Or,
            vec![
                Filter::Plain(
                    "status".into(),
                    FilterTypes::Equal,
                    (IntentStatus::Failed).to_string(),
                ),
                Filter::Plain("attempt_count".into(), FilterTypes::Gt, "1".into()),
            ],
        );

        query_builder.add_nested_filter_clause(or_clause);
