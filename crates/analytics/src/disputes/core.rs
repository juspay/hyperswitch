...
pub async fn get_metrics(
        ...
        .change_context(AnalyticsError::UnknownError)?
    {
        ...
        for (id, value) in data {
            ...
            match metric {
                DisputeMetrics::DisputesChallenged
                | DisputeMetrics::DisputesWon
                | DisputeMetrics::DisputesLost
                | DisputeMetrics::TotalDispute => metrics_builder
        ...
        let values = (
            match pool {
                AnalyticsProvider::Sqlx(pool) => {
                    get_dispute_filter_for_dimension(dim, merchant_id, &req.time_range, pool).await
                }
            ...
            .change_context(AnalyticsError::UnknownError)?
            .into_iter()
            .filter_map(|fil: DisputeFilterRow| {
                match dim {
                    DisputeDimensions::DisputeStage =>
                        fil.dispute_stage.map(|i| i.as_ref().to_string()),
