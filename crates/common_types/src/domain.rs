...
/// Configuration entry for a single merchant advice code stored in config table.
#[derive(Debug, Clone, Deserialize)]
pub struct MerchantAdviceCodeEntry {
    /// Formatted lookup key (e.g., "network:visa|merchant_advice_code:01")
    pub key: String,
    /// Human-readable description of the advice code
    pub description: String,
    /// Recommended action for retry logic (e.g., "do_not_retry", "retry_later")
    pub recommended_action: enums::RecommendedAction,
}

/// Value stored in the HashMap for merchant advice code lookup
#[derive(Debug, Clone)]
pub struct MerchantAdviceCodeValue {
    /// Description of the advice code
    pub description: String,
    /// Recommended action for retry logic
    pub recommended_action: enums::RecommendedAction,
}

/// Merchant advice codes configuration stored as HashMap for O(1) lookup.
#[derive(Debug, Clone, Default)]
pub struct MerchantAdviceCodesConfig(pub HashMap<String, MerchantAdviceCodeValue>);

impl MerchantAdviceCodesConfig {
    /// Find merchant advice code entry by lookup key
    pub fn find_by_lookup_key(&self, lookup_key: &str) -> Option<&MerchantAdviceCodeValue> {
...
impl<'de> Deserialize<'de> for MerchantAdviceCodesConfig {
    ...
    {
        let entries: Vec<MerchantAdviceCodeEntry> = Vec::deserialize(deserializer)?;
        let map = entries
            .into_iter()
            .map(|entry| {
                (
                    entry.key,
                    MerchantAdviceCodeValue {
