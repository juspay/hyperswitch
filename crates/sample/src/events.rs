...
use crate::domain::event as domain;
...
where
    domain::Event:
        behaviour::Conversion<DstType = storage::Event, NewDstType = storage::EventNew>,
{
    type Error;
    ...
    ) -> CustomResult<domain::Event, Self::Error>;
    ...
    async fn list_initial_events_by_merchant_id_primary_object_id(
        ...
        merchant_key_store: &merchant_key_store::MerchantKeyStore,
    ) -> CustomResult<Vec<domain::Event>, Self::Error>;
...
//         let stored_event = behaviour::Conversion::convert(event)
//             .await
