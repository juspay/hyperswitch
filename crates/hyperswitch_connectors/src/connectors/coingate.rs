...
impl
    ...
    fn get_request_body(&self, req: &PaymentsAuthorizeRouterData, _connectors: &Connectors,) -> CustomResult<RequestContent, errors::ConnectorError> {
        ...
        let connector_router_data =
            coingate::CoingateRouterData::from((
                amount,
                req,
            ));
    ...
    fn handle_response(
        ...
        let response: coingate::CoingatePaymentsResponse = res.response.parse_struct("Coingate PaymentsAuthorizeResponse").change_context(errors::ConnectorError::ResponseDeserializationFailed)?;
        event_builder.map(|i| i.set_response_body(&response));
        router_env::logger::info!(connector_response=?response);
...
impl
    ...
    fn handle_response(
        ...
        let response: coingate::RefundResponse = res.response.parse_struct("coingate RefundResponse").change_context(errors::ConnectorError::ResponseDeserializationFailed)?;
        event_builder.map(|i| i.set_response_body(&response));
        router_env::logger::info!(connector_response=?response);
        RouterData::try_from(ResponseRouterData {
            response,
...
impl
    ConnectorIntegration<RSync, RefundsData, RefundsResponseData> for Coingate {
    ...
    fn handle_response(
        ...
        let response: coingate::RefundResponse = res.response.parse_struct("coingate RefundSyncResponse").change_context(errors::ConnectorError::ResponseDeserializationFailed)?;
        event_builder.map(|i| i.set_response_body(&response));
        router_env::logger::info!(connector_response=?response);
        RouterData::try_from(ResponseRouterData {
            response,
