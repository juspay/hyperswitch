...
impl ConnectorSpecifications for Redsys {
    ...
    /// Check if pre-authentication flow is required for Redsys 3DS
    /// This is called by the router to determine if PreAuthenticate should be called before Authorize
    fn is_pre_authentication_flow_required(&self, current_flow: api::CurrentFlowInfo<'_>) -> bool {
        match current_flow {
    ...
    /// Check if authentication flow is required for Redsys 3DS during CompleteAuthorize
    /// This is called after user completes 3DS method to proceed with authentication
    fn is_authentication_flow_required(&self, current_flow: api::CurrentFlowInfo<'_>) -> bool {
        match current_flow {
            api::CurrentFlowInfo::Authorize { auth_type, .. } => {
                // For bloated authentication flow (authentication_bloated_connectors config),
                // authentication step is needed after pre-authentication when 3DS is required.
                // This enables the granular authentication flow: PreAuthenticate -> Authenticate -> (PostAuthenticate if needed)
                *auth_type == common_enums::AuthenticationType::ThreeDs
            }
            api::CurrentFlowInfo::CompleteAuthorize { request_data, .. } => {
                // Authentication is required when:
                // - We have a redirect response (user completed 3DS method or challenge)
                // - The params are empty (3DS method completion, not challenge response)
                // If params contain cres, we need PostAuthenticate instead
                request_data
                    .redirect_response
                    .as_ref()
                    .map(|redirect| {
    ...
    /// Check if post-authentication flow is required for Redsys 3DS during CompleteAuthorize
    /// This is called after user completes ACS challenge to finalize authentication
    fn is_post_authentication_flow_required(&self, current_flow: api::CurrentFlowInfo<'_>) -> bool {
        match current_flow {
            api::CurrentFlowInfo::Authorize { .. } => false,
            api::CurrentFlowInfo::CompleteAuthorize { request_data, .. } => {
                // Post-authentication is required when:
                // - We have a redirect response with non-empty params (contains cres from challenge)
                request_data
                    .redirect_response
                    .as_ref()
                    .map(|redirect| {
...
impl Redsys {
    /// Check if 3DS setup is required for this payment
    pub fn is_3ds_setup_required(
        ...
        router_env::logger::info!(
            router_data_request=?request,
            auth_type=?auth_type,
            "Checking if 3DS setup is required for Redsys"
        );
