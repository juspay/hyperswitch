...
impl ConnectorIntegration<Authorize, PaymentsAuthorizeData, PaymentsResponseData> for Mifinity {
    ...
    fn handle_response(
        ...
        let response_integrity_object = match (response.amount.clone(), response.currency.clone()) {
...
impl ConnectorIntegration<PSync, PaymentsSyncData, PaymentsResponseData> for Mifinity {
    ...
    fn handle_response(
        ...
        let response_integrity_object = match (response.amount.clone(), response.currency.clone()) {
            (Some(amount), Some(currency)) => Some(connector_utils::get_sync_integrity_object(
                self.amount_converter,
                amount,
                currency,
            )?),
