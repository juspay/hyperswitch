...
impl ConnectorCommon for Saferpay {
    ...
    fn get_currency_unit(&self) -> api::CurrencyUnit {
        // Saferpay expects amounts in minor units (`Amount.Value`). Never invoked on
        // the HS side for this UCS-only connector, but must return a valid value
        // (no todo!()).
...
impl ConnectorIntegration<Authorize, PaymentsAuthorizeData, PaymentsResponseData> for Saferpay {
    ...
    fn get_request_body(
        ...
        let connector_router_data = saferpay::SaferpayRouterData::from((amount, req));
        let connector_req = saferpay::SaferpayPaymentsRequest::try_from(&connector_router_data)?;
    ...
    fn handle_response(
        ...
        let response: saferpay::SaferpayPaymentsResponse = res
            .response
            .parse_struct("Saferpay PaymentsAuthorizeResponse")
            .change_context(errors::ConnectorError::ResponseDeserializationFailed)?;
        event_builder.map(|i| i.set_response_body(&response));
        router_env::logger::info!(connector_response=?response);
...
impl ConnectorIntegration<PSync, PaymentsSyncData, PaymentsResponseData> for Saferpay {
    ...
    fn handle_response(
        ...
        let response: saferpay::SaferpayPaymentsResponse = res
            .response
            .parse_struct("saferpay PaymentsSyncResponse")
            .change_context(errors::ConnectorError::ResponseDeserializationFailed)?;
        event_builder.map(|i| i.set_response_body(&response));
        router_env::logger::info!(connector_response=?response);
        RouterData::try_from(ResponseRouterData {
...
impl ConnectorIntegration<Capture, PaymentsCaptureData, PaymentsResponseData> for Saferpay {
    ...
    fn handle_response(
        ...
        let response: saferpay::SaferpayPaymentsResponse = res
            .response
            .parse_struct("Saferpay PaymentsCaptureResponse")
            .change_context(errors::ConnectorError::ResponseDeserializationFailed)?;
        event_builder.map(|i| i.set_response_body(&response));
        router_env::logger::info!(connector_response=?response);
        RouterData::try_from(ResponseRouterData {
...
impl ConnectorIntegration<Execute, RefundsData, RefundsResponseData> for Saferpay {
    ...
    fn get_request_body(
        ...
        let connector_router_data = saferpay::SaferpayRouterData::from((refund_amount, req));
        let connector_req = saferpay::SaferpayRefundRequest::try_from(&connector_router_data)?;
        Ok(RequestContent::Json(Box::new(connector_req)))
    }
    ...
    fn handle_response(
        ...
        let response: saferpay::RefundResponse = res
            .response
            .parse_struct("saferpay RefundResponse")
            .change_context(errors::ConnectorError::ResponseDeserializationFailed)?;
        event_builder.map(|i| i.set_response_body(&response));
        router_env::logger::info!(connector_response=?response);
        RouterData::try_from(ResponseRouterData {
...
impl ConnectorIntegration<RSync, RefundsData, RefundsResponseData> for Saferpay {
    ...
    fn handle_response(
        ...
        let response: saferpay::RefundResponse = res
            .response
            .parse_struct("saferpay RefundSyncResponse")
            .change_context(errors::ConnectorError::ResponseDeserializationFailed)?;
        event_builder.map(|i| i.set_response_body(&response));
        router_env::logger::info!(connector_response=?response);
        RouterData::try_from(ResponseRouterData {
...
static SAFERPAY_CONNECTOR_INFO: ConnectorInfo = ConnectorInfo {
    ...
    description:
        "Saferpay by SIX Payment Services is a payment gateway for online card acceptance in Europe",
