...
impl<T> From<(StringMinorUnit, T)> for FlexitiRouterData<T> {
...
impl From<FlexitiPaymentStatus> for common_enums::AttemptStatus {
    fn from(item: FlexitiPaymentStatus) -> Self {
        match item {
            FlexitiPaymentStatus::Succeeded => Self::Charged,
            FlexitiPaymentStatus::Failed => Self::Failure,
            FlexitiPaymentStatus::Processing => Self::Authorizing,
        }
    }
}
