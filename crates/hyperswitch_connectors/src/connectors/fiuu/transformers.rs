    ...
    fn try_from(bank: BankNames) -> Result<Self, Self::Error> {
        match bank {
            ...
            _ => Err(errors::ConnectorError::NotSupported {
                message: "Invalid BankCode for FPX Refund".to_string(),
                connector: "Fiuu",
            })?,
        }
    }
...
impl TryFrom<&FiuuRouterData<&RefundsRouterData<Execute>>> for FiuuRefundRequest {
    ...
    fn try_from(item: &FiuuRouterData<&RefundsRouterData<Execute>>) -> Result<Self, Self::Error> {
        ...
        Ok(Self {
            ...
            bank_code: item.router_data.request.additional_payment_method_data.as_ref().and_then(|data| {
                if let AdditionalPaymentData::BankRedirect { bank_name, .. } = data {
                    bank_name.and_then(|name| (BankCode::try_from(name).ok()))
                } else {
                    None
                }
            }),
