...
fn use_stripe_checkout_api_version(headers: &mut Vec<(String, Maskable<String>)>) {
    ...
    headers.retain_mut(|(name, value)| {
        ...
        // Hosted Checkout must replace the connector's default version header. Appending a
        // duplicate header would produce a comma-separated value that Stripe rejects.
                ...
                let response_integrity_object = get_sync_integrity_object(
                    ...
                    response
                        ...
                        .ok_or(ConnectorError::MissingRequiredField {
                            field_name: "response.amount_subtotal",
                        })?,
                    response
                        .currency
                        .clone()
                        .ok_or(ConnectorError::MissingRequiredField {
                            field_name: "response.currency",
                        })?,
                )?;
                let new_router_data = RouterData::try_from(ResponseRouterData {
                    response,
            ...
            Ok(x) => {
                ...
                new_router_data.map(|mut router_data| {
                    if x.starts_with(STRIPE_CHECKOUT_SESSION_ID_PREFIX) {
                        // A PaymentIntent webhook can arrive before the Checkout Session event.
                        // Preserve session ownership while accepting the PaymentIntent status,
                        // transaction ID, and paid amount so a missing Checkout event cannot
                        // leave the payment pending indefinitely.
