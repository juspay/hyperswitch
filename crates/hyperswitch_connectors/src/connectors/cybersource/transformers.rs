        ...
        let commerce_indicator = item
            ...
            .and_then(|authn_data| {
                authn_data.eci.clone().map(|eci| {
                    get_commerce_indicator_for_external_authentication(network.clone(), eci)
                })
            })
            .unwrap_or_else(|| {
                solution
...
fn get_commerce_indicator_for_external_authentication(
    ...
    match eci.as_str() {
        "00" | "01" | "02" => {
            if matches!(card_network, Some("mastercard")) {
                "spa"
            } else {
                "internet"
            }
...
fn normalize_cybersource_card_network(card_network: Option<&str>) -> Option<&'static str> {
    card_network.and_then(|network| match network.to_lowercase().as_str() {
        "001" | "visa" => Some("visa"),
        "002" | "master" | "mastercard" | "master card" => Some("mastercard"),
        "003" | "amex" | "americanexpress" | "american express" => Some("amex"),
        "004" | "discover" => Some("discover"),
        "005" | "diners" | "dinersclub" | "diners club" => Some("diners"),
        "007" | "jcb" => Some("jcb"),
        "036" | "cartesbancaires" | "cartes bancaires" => Some("cartesbancaires"),
        "rupay" | "ru pay" => Some("rupay"),
        "062" | "upi" | "unionpay" | "union pay" => Some("upi"),
        _ => None,
    })
}
