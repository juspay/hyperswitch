...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct NexixpayNonRecurringPaymentRequest {
    card: NexixpayCard,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct NexixpayRecurringPaymentRequest {
    contract_id: String,
...
pub enum NexixpayPaymentsRequestData {
    NexixpayNonRecurringPaymentRequest(Box<NexixpayNonRecurringPaymentRequest>),
    NexixpayRecurringPaymentRequest(Box<NexixpayRecurringPaymentRequest>),
}
            ...
            .and_then(|mandate_id| mandate_id.mandate_reference_id)
        {
            None => {
                let recurrence_request_obj = if item.router_data.request.is_mandate_payment() {
                    let contract_id = item.router_data.request.mandate_contract_id.clone().ok_or(
                        errors::ConnectorError::MissingRequiredField {
                            field_name: "mandate_contract_id",
                        },
                    )?;
                ...
                match item.router_data.request.payment_method_data {
                    PaymentMethodData::Card(ref req_card) => {
                        ...
                        Ok(Self::NexixpayNonRecurringPaymentRequest(Box::new(
                            NexixpayNonRecurringPaymentRequest {
                                card,
            ...
            Some(api_models::payments::MandateReferenceId::ConnectorMandateId(mandate_data)) => {
                let contract_id = mandate_data
                    .connector_mandate_id
                    .ok_or(errors::ConnectorError::MissingConnectorMandateID)?;
                let capture_type =
                    get_nexixpay_capture_type(item.router_data.request.capture_method)?;
                Ok(Self::NexixpayRecurringPaymentRequest(Box::new(
                    NexixpayRecurringPaymentRequest {
                        contract_id,
                        capture_type,
                    },
                )))
        ...
        Ok(Self {
            ...
            response: Ok(PaymentsResponseData::TransactionResponse {
                ...
                mandate_reference: Some(MandateReference {
                    connector_mandate_id: item.data.request.mandate_contract_id.clone(),
