...
pub enum FingerprintPresenceIndicator {
    /// Indicates that the payment method was seen on the platform before by this merchant.
    Exists,
    /// Indicates that the payment method was not seen on the platform before by this merchant.
    New,
}
...
pub struct PaymentMethod {
    ...
    ///   merchant.
    pub fingerprint_presence_indicator: Option<FingerprintPresenceIndicator>,
...
pub struct Card {
    ...
    /// The network token,provided by the brands as a more secure alternative to the card number.
    pub network_token: Option<String>,
    /// Masked network token but last 4 digits of the network token are visible.
    pub masked_network_token_last4: Option<String>,
    /// The 2 digit expiry date month of the network token.
    pub network_token_expiry_month: Option<String>,
    /// The 2 digit expiry date year of the network token.
    pub network_token_expiry_year: Option<String>,
}
...
pub struct GlobalpayPaymentMethodsResponse {
    /// Unique Global Payments generated id used to reference a stored payment method on the Global Payments system.
    /// Often referred to as the payment method token. This value can be used instead of payment method details such as a card number and expiry date.
    #[serde(rename = "id")]
    pub payment_method_token_id: Option<String>,
    /// Global Payments time indicating when the object was created in ISO-8601 format
    pub time_created: Option<String>,
    /// The overall status of the payment method.
    pub status: Option<GlobalpayPaymentMethodStatus>,
    /// A unique identifier for the merchant set by Global Payments.
    pub merchant_id: Option<String>,
    /// A meaningful label for the merchant set by Global Payments.
    pub merchant_name: Option<Secret<String>>,
    ...
    pub fingerprint_presence_indicator: Option<FingerprintPresenceIndicator>,
    /// If enabled, this field contains the unique fingerprint signature for that payment method for that merchant.
    /// If the payment method is seen again this same value is generated.
    /// For cards the primary account number is checked only. The expiry date or the CVV is not used for this check.
    pub fingerprint: Option<Secret<String>>,
