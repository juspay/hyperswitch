...
impl
    ...
    fn get_request_body(&self, req: &PaymentsAuthorizeRouterData, _connectors: &Connectors,) -> CustomResult<RequestContent, errors::ConnectorError> {
        ...
        let connector_router_data =
            chargebee::ChargebeeRouterData::from((
                amount,
                req,
            ));
    ...
    fn handle_response(
        ...
        let response: chargebee::ChargebeePaymentsResponse = res.response.parse_struct("Chargebee PaymentsAuthorizeResponse").change_context(errors::ConnectorError::ResponseDeserializationFailed)?;
        event_builder.map(|i| i.set_response_body(&response));
        router_env::logger::info!(connector_response=?response);
...
impl
    ...
    fn handle_response(
        ...
        let response: chargebee::RefundResponse = res.response.parse_struct("chargebee RefundResponse").change_context(errors::ConnectorError::ResponseDeserializationFailed)?;
        event_builder.map(|i| i.set_response_body(&response));
        router_env::logger::info!(connector_response=?response);
        RouterData::try_from(ResponseRouterData {
            response,
...
impl
    ConnectorIntegration<RSync, RefundsData, RefundsResponseData> for Chargebee {
    ...
    fn handle_response(
        ...
        let response: chargebee::RefundResponse = res.response.parse_struct("chargebee RefundSyncResponse").change_context(errors::ConnectorError::ResponseDeserializationFailed)?;
        event_builder.map(|i| i.set_response_body(&response));
        router_env::logger::info!(connector_response=?response);
        RouterData::try_from(ResponseRouterData {
            response,
