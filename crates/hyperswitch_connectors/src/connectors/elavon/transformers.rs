...
impl TryFrom<&ElavonRouterData<&PaymentsAuthorizeRouterData>> for ElavonCreditCardAuthOnlyReq {
    type Error = error_stack::Report<errors::ConnectorError>;
    fn try_from(
        ...
        match item.router_data.request.payment_method_data.clone() {
            PaymentMethodData::Card(req_card) => {
                ...
                "#, 
                    ...
                    card.number,
                    get_exp_date(card.expiry_month,card.expiry_year),
                    item.amount.clone(), 
                    card.cvc, 
                ...
                Ok(Self { 
                    ...
                    ssl_exp_date: get_exp_date(card.expiry_month,card.expiry_year), // MMYY
...
impl TryFrom<&ElavonRouterData<&PaymentsCaptureRouterData>> for ElavonCreditCardCompletionReq {
    type Error = error_stack::Report<errors::ConnectorError>;
    fn try_from(
        ...
        Ok(Self{
            ...
            ssl_pin: auth.ssl_pin,
            ssl_vendor_id: auth.ssl_vendor_id,
            ssl_txn_id: item.router_data.response., // comes from auth response
            ssl_amount: item.amount.clone(), 
        })
    }
}
...
impl<F, T> TryFrom<ResponseRouterData<F, ElavonResponse, T, PaymentsResponseData>>
