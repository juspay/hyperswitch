...
use crate::{
    ...
    utils::{ApplePayDecrypt, PayoutsData},
};
use crate::{
    types::{
...
impl TryFrom<(&WalletData, &PaymentsAuthorizeRouterData)> for AdyenPaymentMethod<'_> {
    ...
    fn try_from(value: (&WalletData, &PaymentsAuthorizeRouterData)) -> Result<Self, Self::Error> {
        ...
        match wallet_data {
            ...
            WalletData::ApplePay(data) => match item.payment_method_token.clone() {
                Some(PaymentMethodToken::ApplePayDecrypt(apple_pay_decrypte)) => {
