...
impl ConnectorCommon for D24 {
    ...
    fn get_currency_unit(&self) -> api::CurrencyUnit {
        // D24 processes amounts in base (major) units. Never invoked on the
...
impl ConnectorIntegration<Capture, PaymentsCaptureData, PaymentsResponseData> for D24 {
    ...
    fn handle_response(
        ...
        let response: d24::D24PaymentsResponse = res
            .response
            .parse_struct("D24 PaymentsCaptureResponse")
            .change_context(errors::ConnectorError::ResponseDeserializationFailed)?;
        event_builder.map(|i| i.set_response_body(&response));
        router_env::logger::info!(connector_response=?response);
        RouterData::try_from(ResponseRouterData {
...
impl ConnectorIntegration<Execute, RefundsData, RefundsResponseData> for D24 {
    ...
    fn get_request_body(
        ...
        let connector_router_data = d24::D24RouterData::from((refund_amount, req));
        let connector_req = d24::D24RefundRequest::try_from(&connector_router_data)?;
        Ok(RequestContent::Json(Box::new(connector_req)))
    }
    ...
    fn handle_response(
        ...
        let response: d24::RefundResponse = res
            .response
            .parse_struct("d24 RefundResponse")
            .change_context(errors::ConnectorError::ResponseDeserializationFailed)?;
        event_builder.map(|i| i.set_response_body(&response));
        router_env::logger::info!(connector_response=?response);
        RouterData::try_from(ResponseRouterData {
...
impl ConnectorIntegration<RSync, RefundsData, RefundsResponseData> for D24 {
    ...
    fn handle_response(
        ...
        let response: d24::RefundResponse = res
            .response
            .parse_struct("d24 RefundSyncResponse")
            .change_context(errors::ConnectorError::ResponseDeserializationFailed)?;
        event_builder.map(|i| i.set_response_body(&response));
        router_env::logger::info!(connector_response=?response);
        RouterData::try_from(ResponseRouterData {
...
static D24_SUPPORTED_PAYMENT_METHODS: LazyLock<SupportedPaymentMethods> = LazyLock::new(|| {
    // Directa24 cards are sale-only: the "webpaycard" product has no separate capture
    // endpoint, so only automatic capture is offered.
    let supported_capture_methods = vec![enums::CaptureMethod::Automatic];

    // Directa24's Brazilian card acquiring scheme coverage. Elo and Hipercard are also
    // acquired by Directa24 in Brazil but have no `CardNetwork` variant in Hyperswitch yet.
    ...
    for payment_method_type in [
        ...
        d24_supported_payment_methods.add(
            ...
            PaymentMethodDetails {
                // One-time payments only - no mandates / MIT.
...
static D24_CONNECTOR_INFO: ConnectorInfo = ConnectorInfo {
    display_name: "Directa24",
    description: "Directa24 (D24) is a payment gateway for emerging markets, integrated through the Unified Connector Service",
