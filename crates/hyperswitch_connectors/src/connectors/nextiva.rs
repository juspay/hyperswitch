...
impl
    ...
    fn get_request_body(&self, req: &PaymentsAuthorizeRouterData, _connectors: &Connectors,) -> CustomResult<RequestContent, errors::ConnectorError> {
        ...
        let connector_router_data =
            nextiva::NextivaRouterData::from((
                amount,
                req,
            ));
    ...
    fn handle_response(
        ...
        let response: nextiva::NextivaPaymentsResponse = res.response.parse_struct("Nextiva PaymentsAuthorizeResponse").change_context(errors::ConnectorError::ResponseDeserializationFailed)?;
        event_builder.map(|i| i.set_response_body(&response));
        router_env::logger::info!(connector_response=?response);
...
impl
    ...
    fn handle_response(
        ...
        let response: nextiva::RefundResponse = res.response.parse_struct("nextiva RefundResponse").change_context(errors::ConnectorError::ResponseDeserializationFailed)?;
        event_builder.map(|i| i.set_response_body(&response));
        router_env::logger::info!(connector_response=?response);
        RouterData::try_from(ResponseRouterData {
            response,
...
impl
    ConnectorIntegration<RSync, RefundsData, RefundsResponseData> for Nextiva {
    ...
    fn handle_response(
        ...
        let response: nextiva::RefundResponse = res.response.parse_struct("nextiva RefundSyncResponse").change_context(errors::ConnectorError::ResponseDeserializationFailed)?;
        event_builder.map(|i| i.set_response_body(&response));
        router_env::logger::info!(connector_response=?response);
        RouterData::try_from(ResponseRouterData {
            response,
