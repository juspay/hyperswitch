...
fn set_stripe_checkout_query_parameter(url: &mut Url, key: &str, value: &str) {
    // The redirect marker is reserved by the integration. Remove any merchant-provided value
    // first so duplicate query keys cannot affect payment-result evaluation.
...
impl StripeCheckoutExpectedPrice for PaymentsCancelData {
    fn expected_price(&self) -> Option<(MinorUnit, api_enums::Currency)> {
        // Cancellation validates only the Stripe Session terminal state because amount validation
        // already happened during creation and synchronization.
        None
    }
    ...
    T: StripeCheckoutExpectedPrice,
{
    ...
    fn try_from(
        ...
        let resource_id = if response.subscription.is_some() && payment_intent.is_none() {
            // Stripe's `checkout.session.completed` subscription webhook carries only `sub_*` and
            // may omit the first invoice PaymentIntent. Preserve `cs_*` for later synchronization.
            response.id.clone()
        } else {
        ...
        Ok(Self {
            ...
            response: Ok(PaymentsResponseData::TransactionResponse {
                ...
                // Expose Stripe's `sub_*` after subscription Checkout so orchestration can bind the
                // native subscription instead of creating a duplicate.
        ...
        let (
                ...
                .and_then(|mandate_ids| mandate_ids.mandate_reference_id)
            {
                Some(mandates::MandateReferenceId::ConnectorMandateId(connector_mandate_ids)) => {
                    // A processor token can also represent an initial customer-present payment.
                    // Forward `setup_future_usage` so Stripe attaches the PaymentMethod to the
                    // connector customer for later subscription MITs. Actual MIT requests omit this
                    // field, leaving existing off-session behavior unchanged.
                    let setup_future_usage = validate_and_get_setup_future_usage(
                        item.request.setup_future_usage,
                        item.request.payment_method_type,
                    )?;
