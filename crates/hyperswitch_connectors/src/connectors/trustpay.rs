...
impl ConnectorIntegration<Authorize, PaymentsAuthorizeData, PaymentsResponseData> for Trustpay {
    ...
    fn handle_response(
        ...
        match &response {
            ...
            trustpay::TrustpayPaymentsResponse::BankRedirectPayments(bank_redirect_resp) => {
                if let Some(payment_info) = bank_redirect_resp.payment_information_response.as_ref()
                {
                    let response_integrity_object =
                        connector_utils::get_authorise_integrity_object(
                            self.amount_converter_to_float_major_unit,
                            payment_info.amount.amount,
                            payment_info.amount.currency.to_string(),
                        )?;
...
impl ConnectorIntegration<Execute, RefundsData, RefundsResponseData> for Trustpay {
    ...
    fn handle_response(
        ...
        // Add integrity object when amount and currency are available in the response
        match &response {
            trustpay::RefundResponse::WebhookRefund(webhook_response) => {
...
impl ConnectorIntegration<RSync, RefundsData, RefundsResponseData> for Trustpay {
    ...
    fn handle_response(
        ...
        // Add integrity object for webhook and bank redirect refund sync responses
        if let trustpay::RefundResponse::WebhookRefund(ref webhook_response) = response {
            ...
            return new_router_data
                ...
                .change_context(errors::ConnectorError::ResponseHandlingFailed);
        } else if let trustpay::RefundResponse::BankRedirectRefundSyncResponse(
            ref bank_redirect_response,
        ) = response
        {
            let response_integrity_object = connector_utils::get_refund_integrity_object(
                self.amount_converter_to_float_major_unit,
                bank_redirect_response.payment_information.amount.amount,
                bank_redirect_response
                    .payment_information
                    .amount
                    .currency
                    .to_string(),
