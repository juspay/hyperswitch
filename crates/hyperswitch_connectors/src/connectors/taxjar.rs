...
impl
    ...
    fn handle_response(
        ...
        let response: taxjar::TaxjarPaymentsResponse = res.response.parse_struct("Taxjar PaymentsAuthorizeResponse").change_context(errors::ConnectorError::ResponseDeserializationFailed)?;
        event_builder.map(|i| i.set_response_body(&response));
        router_env::logger::info!(connector_response=?response);
    ...
    for Taxjar
{
    ...
    fn handle_response(
        ...
        let response: taxjar:: TaxjarPaymentsResponse = res
            .response
            .parse_struct("taxjar PaymentsSyncResponse")
            .change_context(errors::ConnectorError::ResponseDeserializationFailed)?;
        event_builder.map(|i| i.set_response_body(&response));
        router_env::logger::info!(connector_response=?response);
        RouterData::try_from(ResponseRouterData {
    ...
    > for Taxjar
{
    ...
    fn handle_response(
        ...
        let response: taxjar::TaxjarPaymentsResponse = res
            .response
            .parse_struct("Taxjar PaymentsCaptureResponse")
            .change_context(errors::ConnectorError::ResponseDeserializationFailed)?;
        event_builder.map(|i| i.set_response_body(&response));
        router_env::logger::info!(connector_response=?response);
        RouterData::try_from(ResponseRouterData {
...
impl
    ...
    fn get_request_body(&self, req: &RefundsRouterData<Execute>, _connectors: &Connectors,) -> CustomResult<RequestContent, errors::ConnectorError> {
        ...
        let connector_req = taxjar::TaxjarRefundRequest::try_from(&connector_router_data)?;
        Ok(RequestContent::Json(Box::new(connector_req)))
    }
    ...
    fn handle_response(
        ...
        let response: taxjar::RefundResponse = res.response.parse_struct("taxjar RefundResponse").change_context(errors::ConnectorError::ResponseDeserializationFailed)?;
        event_builder.map(|i| i.set_response_body(&response));
        router_env::logger::info!(connector_response=?response);
        RouterData::try_from(ResponseRouterData {
            response,
...
impl
    ConnectorIntegration<RSync, RefundsData, RefundsResponseData> for Taxjar {
    ...
    fn handle_response(
        ...
        let response: taxjar::RefundResponse = res.response.parse_struct("taxjar RefundSyncResponse").change_context(errors::ConnectorError::ResponseDeserializationFailed)?;
        event_builder.map(|i| i.set_response_body(&response));
        router_env::logger::info!(connector_response=?response);
        RouterData::try_from(ResponseRouterData {
            response,
