...
pub struct AciPaymentResultDetails {
    #[serde(default, deserialize_with = "deserialize_acquirer_response")]
    pub acquirer_response: Option<AciAcquirerResponse>,
}

fn deserialize_acquirer_response<'de, D>(
    deserializer: D,
) -> Result<Option<AciAcquirerResponse>, D::Error>
where
    D: serde::Deserializer<'de>,
{
    // ACI returns AcquirerResponse as either a string (e.g., "E0000") or an object
    // Try to deserialize as object, fallback to None if it's a string or fails
...
#[derive(Debug, Default, Clone, Deserialize, PartialEq, Eq, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct AciAcquirerResponse {
    ...
    fn try_from(
        ...
        let response = if status == enums::AttemptStatus::Failure {
            ...
            let network_txn_id = item
                ...
                .and_then(|ar| ar.network_transaction_id.clone());
