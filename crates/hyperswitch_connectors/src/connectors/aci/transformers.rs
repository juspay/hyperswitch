    ...
    fn try_from(
        ...
        let aci_network_token_data = AciNetworkTokenData {
            ...
            payment_brand: get_aci_wallet_payment_brand(
                ...
                "apple_pay.payment_method.network",
            )?,
...
/// generic `APPLEPAYTKN` / `GOOGLEPAYTKN` brand. An unrecognised network is
/// therefore an error rather than a fallback to the wallet brand: sending
/// `APPLEPAYTKN` on the token path produces an opaque rejection from ACI, so fail
/// at request-build time with the offending field instead.
fn get_aci_wallet_payment_brand(
    network: &str,
    ...
    let card_network = match network.to_lowercase().as_str() {
        ...
        "amex" | "americanexpress" | "american express" => {
            Some(common_enums::CardNetwork::AmericanExpress)
        }
        "jcb" => Some(common_enums::CardNetwork::JCB),
        "diners" | "dinersclub" | "diners club" => Some(common_enums::CardNetwork::DinersClub),
    ...
    fn try_from(
        ...
        match wallet_data {
            WalletData::ApplePay(apple_pay_data) => {
                ...
                Ok(Self {
                    recurring_type: get_recurring_type(item),
                    txn_details,
                    payment_method,
                    instruction,
...
mod connector_tx_id_tests {
    ...
    fn absa_layout_uses_scheme_trace_for_citi_and_p38_for_auth() {
        ...
        assert_eq!(
            parsed.original_transaction_id.as_deref(),
            Some("ORIG-ABSA-1")
        );
    }
...
#[cfg(test)]
mod secret_and_webhook_tests {
    use hyperswitch_masking::Secret;

    use super::{
        get_aci_wallet_payment_brand, AciThreeDSecureResponse, AciWebhookAction,
        AciWebhookEventType, AciWebhookNotification, PaymentBrand,
    };
    ...
    #[test]
    fn wallet_payment_brand_maps_known_networks() {
        for (network, expected) in [
            ("visa", PaymentBrand::Visa),
            ("masterCard", PaymentBrand::Mastercard),
            ("american express", PaymentBrand::AmericanExpress),
            ("diners club", PaymentBrand::DinersClub),
        ] {
            let brand = get_aci_wallet_payment_brand(network, "test.network")
                .unwrap_or_else(|_| panic!("{network} should map"));
            assert_eq!(
                serde_json::to_value(brand).ok(),
                serde_json::to_value(expected).ok(),
                "unexpected brand for {network}"
            );
        }
    }

    #[test]
    fn wallet_payment_brand_rejects_unknown_network() {
        // On the decrypted tokenAccount.* path ACI expects the underlying card
        // network, so an unmappable network must fail rather than silently
        // falling back to the generic wallet brand.
        assert!(get_aci_wallet_payment_brand("not-a-network", "test.network").is_err());
    }
}
