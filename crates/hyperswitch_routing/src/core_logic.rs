...
use crate::external_decision_engines::euclid::compare_and_log_result;
use crate::external_decision_engines::euclid::create_de_euclid_routing_algo;
use crate::external_decision_engines::euclid::link_de_euclid_routing_algorithm;
use crate::external_decision_engines::euclid::list_de_euclid_routing_algorithms;
use crate::external_decision_engines::euclid::ActivateRoutingConfigRequest;
use crate::external_decision_engines::euclid::ListRountingAlgorithmsRequest;
use crate::external_decision_engines::euclid::Program;
use crate::external_decision_engines::euclid::RoutingMetadata;
use crate::external_decision_engines::euclid::RoutingRule;
use crate::metrics;
use crate::payment_routing as payments_routing;
use crate::state::RoutingState;
use crate::transformers::ForeignInto;
use crate::transformers::ForeignTryFrom;
use crate::utils as core_utils;
use crate::DYNAMIC_ROUTING_MAX_VOLUME;
use common_utils::ext_traits::OptionExt;
use common_utils::ext_traits::ValueExt;
use storage_impl::errors::StorageErrorExt;

use api_models::routing as api;
...
use common_utils::fp_utils as utils;
...
use hyperswitch_domain_models::api as service_api;
use hyperswitch_domain_models::errors::api_error_response as errors;
use hyperswitch_domain_models::{
    business_profile, mandates, merchant_context, merchant_key_store, payment_address,
};
use hyperswitch_domain_models::{
    payment_method_data,
    payments::{self, payment_attempt},
};
use rustc_hash::FxHashSet;
#[cfg(all(feature = "v1", feature = "dynamic_routing"))]
use storage_impl::redis::cache;
use storage_impl::routing_algorithm::common_enums;
use storage_impl::routing_algorithm::storage_models;
use storage_impl::routing_algorithm::storage_models::RoutingAlgorithm;
...
pub struct PaymentsDslInput<'a> {
    ...
    pub payment_attempt: &'a payment_attempt::PaymentAttempt,
    pub payment_intent: &'a payments::PaymentIntent,
    pub payment_method_data: Option<&'a payment_method_data::PaymentMethodData>,
...
impl<'a> PaymentsDslInput<'a> {
    pub fn new(
        ...
        payment_attempt: &'a payment_attempt::PaymentAttempt,
        payment_intent: &'a payments::PaymentIntent,
        payment_method_data: Option<&'a payment_method_data::PaymentMethodData>,
...
pub enum TransactionData<'a> {
    ...
    #[cfg(feature = "payouts")]
    Payout(&'a types::PayoutData),
}

impl From<&TransactionData<'_>> for storage_enums::TransactionType {
    fn from(value: &TransactionData<'_>) -> Self {
        match value {
            TransactionData::Payment(_) => Self::Payment,
            #[cfg(feature = "payouts")]
            TransactionData::Payout(_) => Self::Payout,
        }
    }
}
...
pub async fn retrieve_merchant_routing_dictionary(
    ...
    transaction_type: &enums::TransactionType,
) -> AppResult<routing_types::RoutingKind> {
    metrics::ROUTING_MERCHANT_DICTIONARY_RETRIEVE.add(1, &[]);

    let routing_metadata: Vec<storage_models::RoutingProfileMetadata> = state
        .store
...
pub async fn create_routing_algorithm_under_profile(
    ...
    request: routing_types::RoutingConfigRequest,
    transaction_type: enums::TransactionType,
) -> RouterResult<routing_types::RoutingDictionaryRecord> {
...
pub async fn create_routing_algorithm_under_profile(
    ...
    if let Some(EuclidAlgorithm::Advanced(program)) = request.algorithm.clone() {
        ...
        decision_engine_routing_id = create_de_euclid_routing_algo(&state, &routing_rule)
            ...
            .map_err(|e| {
                router_env::logger::error!(decision_engine_error=?e, "decision_engine_euclid");
                router_env::logger::debug!(decision_engine_request=?routing_rule, "decision_engine_euclid");
            })
            .ok();
    }
...
pub async fn link_routing_config(
    ...
    match routing_algorithm.kind {
        common_enums::RoutingAlgorithmKind::Dynamic => {
        ...
        common_enums::RoutingAlgorithmKind::Single
        | common_enums::RoutingAlgorithmKind::Priority
        | common_enums::RoutingAlgorithmKind::Advanced
        | common_enums::RoutingAlgorithmKind::VolumeSplit => {
...
pub async fn retrieve_routing_algorithm_from_algorithm_id(
    ...
    authentication_profile_id: Option<common_utils::id_type::ProfileId>,
    algorithm_id: common_utils::id_type::RoutingId,
) -> AppResult<routing_types::MerchantRoutingAlgorithm> {
    metrics::ROUTING_RETRIEVE_CONFIG.add(1, &[]);
    let db = state.store.as_ref();
    let key_manager_state = &(&state).into();
...
pub async fn update_default_routing_config(
    ...
    transaction_type: &enums::TransactionType,
) -> AppResult<Vec<routing_types::RoutableConnectorChoice>> {
    metrics::ROUTING_UPDATE_CONFIG.add(1, &[]);
    let db = state.store.as_ref();
...
pub async fn retrieve_default_routing_config(
    ...
    helpers::get_merchant_default_config(state.store, &id, transaction_type)
        .await
        .map(|conn_choice| {
            metrics::ROUTING_RETRIEVE_DEFAULT_CONFIG_SUCCESS_RESPONSE.add(1, &[]);
...
pub async fn retrieve_routing_config_under_profile(
    ...
    profile_id: common_utils::id_type::ProfileId,
    transaction_type: &enums::TransactionType,
) -> RouterResult<routing_types::LinkedRoutingConfigRetrieveResponse> {
...
pub async fn retrieve_linked_routing_config(
    ...
    transaction_type: &enums::TransactionType,
) -> AppResult<routing_types::LinkedRoutingConfigRetrieveResponse> {
    metrics::ROUTING_RETRIEVE_LINK_CONFIG.add(1, &[]);
    let db = state.store.as_ref();
    let key_manager_state = &(&state).into();
...
pub async fn retrieve_default_routing_config_for_profiles(
    ...
    merchant_context: merchant_context::MerchantContext,
    transaction_type: &enums::TransactionType,
) -> AppResult<Vec<routing_types::ProfileDefaultRoutingConfig>> {
...
pub async fn update_default_routing_config_for_profile(
    ...
    profile_id: common_utils::id_type::ProfileId,
    transaction_type: &enums::TransactionType,
) -> AppResult<routing_types::ProfileDefaultRoutingConfig> {
...
pub async fn toggle_specific_dynamic_routing(
    ...
    let business_profile: business_profile::Profile =
        core_utils::validate_and_get_business_profile(
            db,
            key_manager_state,
            merchant_context.get_merchant_key_store(),
...
pub async fn configure_dynamic_routing_volume_split(
    ...
    utils::when(routing_info.split > DYNAMIC_ROUTING_MAX_VOLUME, || {
        Err(errors::ApiErrorResponse::InvalidRequestData {
...
pub async fn contract_based_dynamic_routing_setup(
    ...
    if feature_to_enable == routing::DynamicRoutingFeatures::None {
        let algorithm = dynamic_routing_algo_ref
            ...
            .change_context(errors::ApiErrorResponse::MissingRequiredField {
                field_name: "dynamic_routing_algo_ref",
            })
    ...
    let config = config
        ...
        .change_context(errors::ApiErrorResponse::MissingRequiredField {
            field_name: "ContractBasedRoutingConfig",
        })
    ...
    let algo = RoutingAlgorithm {
        ...
        description: None,
        kind: common_enums::RoutingAlgorithmKind::Dynamic,
...
impl GetRoutableConnectorsForChoice for StraightThroughAlgorithmTypeSingle {
    async fn get_routable_connectors(
        &self,
        _db: &RoutingState<'_>,
        _business_profile: &business_profile::Profile,
    ) -> RouterResult<RoutableConnectors> {
        ...
        let routable_connector = match straight_through_routing_algorithm {
            api::StraightThroughAlgorithm::Single(connector) => {
                vec![*connector]
            }

            api::StraightThroughAlgorithm::Priority(_)
            | api::StraightThroughAlgorithm::VolumeSplit(_) => {
                Err(RoutingError::DslIncorrectSelectionAlgorithm)
                    .change_context(errors::ApiErrorResponse::InternalServerError)
                    .attach_printable(
...
impl RoutableConnectors {
    ...
    pub async fn construct_dsl_and_perform_eligibility_analysis(
        self,
        state: &RoutingState<'_>,
        ...
        payments_routing::perform_cgraph_filtering(
            state,
            key_store,
            self.0.clone(),
            backend_input,
