...
describe("UCS Comprehensive Testing", () => {
  ...
  const UCS_SUPPORTED_CONNECTORS = [
    {
      ...
      import: () => import("../../configs/Payment/Authorizedotnet").then(module => module.connectorDetails)
    }
  ];
  ...
  before("seed global state", () => {
    ...
    cy.task("cli_log", `UCS Compatible Requests will be determined from connector config`);
  });
    ...
    describe(`UCS Tests for ${connectorInfo.name.toUpperCase()}`, () => {
      ...
      before("load connector config", () => {
        ...
        cy.wrap(connectorInfo.import()).then(config => {
          ...
          if (!connectorConfig || !connectorConfig.card_pm) {
            throw new Error(` Failed to load configuration for connector: ${connectorInfo.name}`);
          }

          const allRequests = Object.keys(connectorConfig.card_pm);
          testResults.totalAvailable = allRequests.length;
          
          cy.task("cli_log", `Total requests available in ${connectorInfo.name}.js: ${allRequests.length}`);

          const ucsRequestNames = ["UCSZeroAuthMandate", "UCSConfirmMandate", "UCSRecurringPayment", "No3DSAutoCapture", "No3DSManualCapture"];
          ...
          if (testableRequests.length === 0) {
            throw new Error(` No UCS-compatible requests found for connector: ${connectorInfo.name}`);
          }

          cy.task("cli_log", ` Setting up merchant account for ${connectorInfo.name}...`);
        });
      });
      ...
      it("should enable UCS configuration", () => {
        ...
        cy.request({
          ...
          } else if (response.status === 400 && response.body.error.message.includes("already exists")) {
            cy.task("cli_log", ` Global UCS configuration already exists - proceeding`);
          } else {
            throw new Error(`Failed to enable global UCS: ${response.status} - ${response.body.error.message}`);
          }
        });
        const merchantId = globalState.get("merchantId");
        const rolloutConfigs = [
          { key: `ucs_rollout_config_${merchantId}_${connectorInfo.name}_card_Authorize`, operation: "Authorize" },
          { key: `ucs_rollout_config_${merchantId}_${connectorInfo.name}_card_SetupMandate`, operation: "SetupMandate" },
          { key: `ucs_rollout_config_${merchantId}_${connectorInfo.name}_card_PaymentSync`, operation: "PaymentSync" },
        ];

        rolloutConfigs.forEach(({ key, operation }) => {
          cy.task("cli_log", ` Creating UCS rollout config for ${operation}: ${key}`);
          ...
          cy.request({
            ...
            if (response.status === 200) {
              ...
              cy.task("cli_log", ` UCS ${operation} rollout config created successfully`);
            } else if (response.status === 400 && response.body.error.message.includes("already exists")) {
              cy.task("cli_log", ` UCS ${operation} rollout config already exists - proceeding`);
            } else {
              throw new Error(`Failed to create UCS rollout config for ${operation}: ${response.status} - ${response.body.error.message}`);
            }
          });
        });
      });
      ...
      it("should verify UCS configuration", () => {
        cy.task("cli_log", ` Verifying UCS configuration for ${connectorInfo.name}...`);
        ...
        cy.request({
          ...
          if (response.status === 200) {
            cy.task("cli_log", ` Global UCS config verified: ${response.body.value}`);
            expect(response.body.value).to.equal("true");
          } else {
            throw new Error(`Global UCS config verification failed: ${response.status}`);
          }
        });
        ...
        rolloutConfigs.forEach((rolloutConfigKey) => {
          cy.request({
            ...
            if (response.status === 200) {
              cy.task("cli_log", ` ${rolloutConfigKey} verified: ${response.body.value}`);
              expect(response.body.value).to.equal("1.0");
            } else {
              throw new Error(`Rollout config verification failed: ${rolloutConfigKey} - ${response.status}`);
            }
          });
        });

        cy.task("cli_log", ` UCS configuration verification completed for ${connectorInfo.name}`);
      });
      ...
      it("should test all UCS-compatible requests", () => {
        const testRequest = (index) => {
          if (index >= testableRequests.length) {
            ...
            cy.task("cli_log", `All ${testableRequests.length} UCS requests completed!`);
            cy.task("cli_log", `\n=== COMPREHENSIVE TEST RESULTS ===`);
            cy.task("cli_log", `Total Available Requests: ${testResults.totalAvailable}`);
            cy.task("cli_log", `Total Enabled for Testing: ${testResults.totalEnabled}`);
            cy.task("cli_log", `Total Actually Tested: ${testResults.totalTested}`);
            cy.task("cli_log", `PASSED (${testResults.passed.length}): ${testResults.passed.join(", ")}`);
            cy.task("cli_log", `FAILED (${testResults.failed.length}): ${testResults.failed.join(", ") || "None"}`);
            cy.task("cli_log", `SKIPPED (${testResults.skipped.length}): ${testResults.skipped.join(", ") || "None"}`);
            cy.task("cli_log", `SUCCESS RATE: ${testResults.totalTested > 0 ? (testResults.passed.length/testResults.totalTested*100).toFixed(1) : 0}%`);
          ...
          cy.task("cli_log", `Progress: ${((index + 1)/testableRequests.length*100).toFixed(1)}% | Passed: ${testResults.passed.length} | Failed: ${testResults.failed.length}`);
          ...
          if (requestType.includes("MIT") || requestType.includes("Repeat")) {
            ...
            if (ucsZeroAuthConfig && ucsConfirmConfig && ucsRecurringConfig) {
              cy.task("cli_log", "Starting UCS Sequential Flow: ZeroAuth → Confirm → Recurring");
              ...
              cy.request({
                ...
                cy.task("cli_log", ` UCS ZeroAuth response status: ${response1.status}`);
                if (response1.status !== ucsZeroAuthConfig.Response.status || response1.body.status !== ucsZeroAuthConfig.Response.body.status) {
                  const errorMsg = `FAILED: UCSZeroAuthMandate - Expected status ${ucsZeroAuthConfig.Response.status}, got ${response1.status}`;
                  cy.task("cli_log", ` Full UCS ZeroAuth response: ${JSON.stringify(response1.body, null, 2)}`);
                ...
                cy.task("cli_log", ` ZeroAuth succeeded - Payment ID: ${paymentId}`);
                ...
                cy.request({
                  ...
                  cy.task("cli_log", ` UCS Confirm response status: ${response2.status}`);
                  if (response2.status !== ucsConfirmConfig.Response.status || response2.body.status !== ucsConfirmConfig.Response.body.status) {
                    const errorMsg = `FAILED: UCSConfirmMandate - Expected status ${ucsConfirmConfig.Response.status}, got ${response2.status}`;
                    cy.task("cli_log", ` Full UCS Confirm response: ${JSON.stringify(response2.body, null, 2)}`);
                    throw new Error(`UCS Sequential Flow Failed - ${errorMsg}`);
                  }
                  
                  const paymentMethodId = response2.body.payment_method_id;
                  globalState.set("paymentMethodId", paymentMethodId);
                  cy.task("cli_log", ` Confirm succeeded - Payment Method ID: ${paymentMethodId}`);
                  ...
                  cy.request({
                    ...
                    cy.task("cli_log", ` UCS Recurring response status: ${response3.status}`);
                    if (response3.status !== ucsRecurringConfig.Response.status || response3.body.status !== ucsRecurringConfig.Response.body.status) {
                      const errorMsg = `FAILED: UCSRecurringPayment - Expected status ${ucsRecurringConfig.Response.status}, got ${response3.status}`;
                      cy.task("cli_log", ` Full UCS Recurring response: ${JSON.stringify(response3.body, null, 2)}`);
                      throw new Error(`UCS Sequential Flow Failed - ${errorMsg}`);
                    }
                    
                    cy.task("cli_log", ` Recurring succeeded - UCS Sequential Flow Completed!`);
              ...
              throw new Error(`UCS Flows not found for connector: ${connectorInfo.name} - ensure all UCS requests are defined in the config`);
            }
          }
           else {
            let ucsRequest;

            cy.task("cli_log", ` Base request structure: ${JSON.stringify(baseRequest, null, 2)}`);

            const isMandateRequest = baseRequest.mandate_data && baseRequest.mandate_data !== null;
            ...
            if (isMandateRequest && baseRequest.mandate_data.mandate_type) {
              cy.task("cli_log", `Mandate data found: ${JSON.stringify(baseRequest.mandate_data)}`);

              if (baseRequest.mandate_data.mandate_type.single_use) {
                requestAmount = baseRequest.mandate_data.mandate_type.single_use.amount;
                cy.task("cli_log", `Using mandate single-use amount: ${requestAmount}`);
                ...
                cy.task("cli_log", `Using mandate multi-use amount: ${requestAmount}`);
              }
            } else if (isMandateRequest) {
              cy.task("cli_log", `Mandate request detected but no mandate_type found`);
            }
            ...
            cy.task("cli_log", `Final UCS request: ${JSON.stringify(ucsRequest, null, 2)}`);
            ...
            cy.request({
              ...
              cy.task("cli_log", ` UCS ${requestType} response status: ${response.status}`);

              // Log full response body for detailed analysis
              cy.task("cli_log", ` Full UCS ${requestType} response: ${JSON.stringify(response.body, null, 2)}`);

              if (response.status === expectedResponse.status && response.body.status == expectedResponse.body.status) {
                cy.task("cli_log", `PASSED: ${requestType} via UCS (Status: ${response.status})`);
                ...
                if (response.body && response.body.status) {
                  ...
                  if (response.body.gateway_system) {
                    cy.task("cli_log", `Gateway System: ${response.body.gateway_system}`);
                  }
                ...
                cy.task("cli_log", `Full error response: ${JSON.stringify(response, null, 2)}`);
                cy.task("cli_log", `Request that failed: ${JSON.stringify(ucsRequest, null, 2)}`);
                throw new Error(`UCS Test Failed - ${errorMsg}`);
              }
            });
          }
      ...
      after(() => {
        ...
        rolloutConfigs.forEach((rolloutConfigKey) => {
          cy.request({
            ...
            if (response.status === 200 || response.status === 404) {
              cy.task("cli_log", ` Deleted UCS rollout config: ${rolloutConfigKey}`);
            } else {
              cy.task("cli_log", `⚠️ Failed to delete UCS rollout config: ${rolloutConfigKey} - ${response.status}`);
            }
          });
        });
        ...
        cy.request({
          ...
          if (response.status === 200 || response.status === 404) {
            ...
            cy.task("cli_log", `⚠️ Failed to delete global UCS configuration - ${response.status}`);
          }
        });

        cy.task("cli_log", ` Cleanup for ${connectorInfo.name} completed`);
        if (testableRequests && testableRequests.length > 0) {
          cy.task("cli_log", ` All ${testableRequests.length} requests passed for ${connectorInfo.name}!`);
        }
      });
    });
  });
  ...
  after(() => {
    ...
    cy.task("cli_log", ` Successfully tested ${UCS_SUPPORTED_CONNECTORS.length} connector(s)`);
