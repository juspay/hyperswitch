    ...
    () => {  
      it("Create Business Profile → Create Connector → Enable POST redirect → Create UPI Payment → Confirm → Handle Redirect → Verify Return URL", () => {  
        ...
        cy.step("Confirm UPI payment", () => {  
          ...
          expect(data, `upi_pm.UpiCollect not found for ${connectorId}`).to.exist;  
        ...
        cy.step("Handle UPI redirect and verify return URL parameters (POST mode)", () => {  
          if (!shouldContinue) {  
          ...
          cy.url().then((url) => {  
            ...
            cy.task("cli_log", "Verified: amount parameter is excluded (POST mode)");  
          });  
        });  
      });  
    }  
  ...
  context(  
    ...
    () => {  
      it("Create Business Profile → Enable GET redirect → Create UPI Payment → Confirm → Handle Redirect → Verify Return URL", () => {  
        ...
        cy.step("Handle UPI redirect and verify return URL parameters (GET mode)", () => {  
          if (!shouldContinue) {  
            cy.task("cli_log", "Skipping step: Handle Redirect");  
            return;  
          }  
          ...
          cy.url().then((url) => {  
            ...
            cy.task("cli_log", "Verified: amount parameter is included (GET mode)");  
          });  
        });  
      });  
    }  
