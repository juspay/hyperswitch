...
describe("Crypto Payment", () => {
  ...
  context("Crypto Currency Payment flow", () => {
    it(
      "Create Payment Intent -> Payment Methods Call Test -> Confirm Crypto Currency Payment -> Handle redirection -> Retrieve Payment Call Test",
      () => {
        ...
        cy.step("Confirm Crypto Currency Payment", () => {
          if (!shouldContinue) {
            cy.task(
              "cli_log",
              "Skipping step: Confirm Crypto Currency Payment"
            );
            return;
          }
          const data = getConnectorDetails(globalState.get("connectorId"))[
