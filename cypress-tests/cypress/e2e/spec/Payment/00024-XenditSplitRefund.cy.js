...
describe("Xendit - Split Refund Tests", () => {
  ...
  context(
    "Split Refund - Full refund with for_user_id when charges data is unavailable",
    () => {
      it("Create Payment Intent -> Confirm Payment -> Full Refund with split_refunds containing for_user_id -> Sync Refund", () => {
        ...
        cy.step("Confirm Payment Intent", () => {
          ...
          )["card_pm"]["SplitRefundWithNoChargesData"];
          cy.confirmCallTest(
        ...
        cy.step("Full Refund Payment with split_refunds", () => {
          if (!shouldContinue) {
          ...
          const refundData = getConnectorDetails(
            globalState.get("connectorId")
          )["card_pm"]["SplitRefundFullWithForUserId"];
  ...
  context(
    "Split Refund - Partial refund with for_user_id when charges data is unavailable",
    () => {
      it("Create Payment Intent -> Confirm Payment -> Partial Refund with split_refunds containing for_user_id -> Sync Refund", () => {
        let shouldContinue = true;
        ...
        cy.step("Partial Refund Payment with split_refunds", () => {
          if (!shouldContinue) {
          ...
          const refundData = getConnectorDetails(
            globalState.get("connectorId")
          )["card_pm"]["SplitRefundPartialWithForUserId"];
          cy.refundCallTest(fixtures.refundBody, refundData, globalState);
          if (!utils.should_continue_further(refundData)) {
            shouldContinue = false;
          }
