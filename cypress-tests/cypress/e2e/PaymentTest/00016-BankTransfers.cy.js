...
describe("Bank Transfers", () => {
  ...
  context("Bank transfer - Pix forward flow", () => {
    ...
    it("create-payment-call-test", () => {
      let data = getConnectorDetails(globalState.get("connectorId"))["bank_transfer_pm"]["PaymentIntent"];
    ...
    it("Confirm bank transfer", () => {
      let data = getConnectorDetails(globalState.get("connectorId"))["bank_transfer_pm"]["Pix"];
      let req_data = data["Request"];
      let res_data = data["Response"];
      cy.confirmBankTransferCallTest(fixtures.confirmBody, req_data, res_data, true, globalState);
      if (should_continue) should_continue = utils.should_continue_further(res_data);
    });
