...
export const connectorDetails = {
  card_pm: {
    ...
    Capture: getCustomExchange({
      Configs: {
        TRIGGER_SKIP: true, // Skip until MPGS mock server configuration is fixed
      },
      Request: {
    ...
    SaveCardUseNo3DSAutoCapture: getCustomExchange({
      Configs: {
        TRIGGER_SKIP: true, // Skip if MPGS doesn't support save card
      },
      Request: {
        payment_method: "card",
        payment_method_data: {
    ...
    // Note: MPGS doesn't support 3DS or save card, marked as TRIGGER_SKIP
    ...
    // Note: MPGS doesn't support off-session or save card, marked as TRIGGER_SKIP

    SaveCardUseNo3DSAutoCaptureOffSession: getCustomExchange({
      Configs: {
        TRIGGER_SKIP: true, // Skip if MPGS doesn't support off-session save card
      },
      Request: {
        payment_method: "card",
        payment_method_data: {
    ...
    MITAutoCapture: getCustomExchange({
      Configs: {
        TRIGGER_SKIP: true, // Skip if MPGS doesn't support MIT
      },
      Request: {},
      Response: {
        status: 200,
    ...
    // Note: MPGS doesn't support 3DS or mandates, marked as TRIGGER_SKIP
    ...
    // Note: These may not be supported by MPGS, marked as TRIGGER_SKIP

    PaymentMethodIdMandateNo3DSAutoCapture: getCustomExchange({
      Configs: {
        TRIGGER_SKIP: true, // Skip if MPGS doesn't support payment method ID mandates
      },
      Request: {
        payment_method: "card",
        payment_method_data: {
    ...
    // ===== VOID SCENARIOS =====
    // Note: Adding void scenarios to match Silverflow
