...
describe("Rule Based Routing Test", () => {
  ...
  context("Rule based routing,Card->Stripe,Bank_redirect->adyen", () => {
    ...
    it("add-routing-config", () => {
      ...
      cy.addRoutingConfig(fixtures.routingConfigBody, req_data, res_data, "advanced", routing_data, globalState);
    });
    ...
    it("Confirm bank redirect", () => {
      let data = utils.getConnectorDetails("adyen")["bank_redirect_pm"]["ideal"];
      let req_data = data["Request"];
      let res_data = data["Response"];
  ...
  context("Rule based routing,Currency->is->USD->Stripe->else->adyen", () => {
    ...
    it("create-payment-call-test-with-USD", () => {
      let data = utils.getConnectorDetails("stripe")["card_pm"]["PaymentIntent"];
      let req_data = data["Request"];
      let res_data = data["Response"];
  ...
  context("Rule based routing,amount->isGreaterThan->100->adyen->else->stripe", () => {
    before("seed global state", () => {
      cy.task("getGlobalState").then((state) => {
        globalState = new State(state);
      });
