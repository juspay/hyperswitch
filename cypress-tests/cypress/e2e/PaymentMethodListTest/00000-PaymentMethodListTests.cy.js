...
describe("Payment Method list using Constraint Graph flow tests", () => {
  context(
    ...
    () => {
      ...
      it("payment-method-list-call-test", () => {
        let data = getConnectorDetails("stripe")["pm_list"]["PmListResponse"]["PmListWithStripeForIdeal"];
  ...
  context(
    ...
    () => {
      ...
      // payment method list which should only have ideal with stripe
      it("payment-method-list-call-test", () => {
        let data = getConnectorDetails("stripe")["pm_list"]["PmListResponse"]["PmListNull"];
        cy.paymentMethodListTestLessThanEqualToOnePaymentMethod(data, globalState);
      });
    }
  );
  ...
  context(
   ...
   The resultant Payment Method list should have both Stripe and Cybersource\n
    ...
    () => {
      ...
      it("payment-method-list-call-test", () => {
        let data = getConnectorDetails("stripe")["pm_list"]["PmListResponse"]["PmListWithCreditTwoConnector"];
        cy.paymentMethodListTestTwoConnectorsForOnePaymentMethodCredit(data, globalState);
      });
    }
  );
