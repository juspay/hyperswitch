...
Cypress.Commands.add("merchantListCall", (globalState) => {
  ...
  cy.request({
    ...
    for (const key in response.body) {
      ...
      expect(response.body[key]).to.have.property("organization_id").and.not.empty;
      expect(response.body[key]).to.have.property("default_profile").and.not.empty;
    }
  });
});
  ...
  cy.request({
    ...
    for (const key in response.body) {
      ...
      if (base_url.includes("sandbox") || base_url.includes("integ")) {
        expect(response.body[key]).to.have.property("key_id").include("snd_").and.not.empty;
      } else if (base_url.includes("localhost")) {
        expect(response.body[key]).to.have.property("key_id").include("dev_").and.not.empty;
      }
...
Cypress.Commands.add(
  ...
  (connectorType, createConnectorBody, payment_methods_enabled, globalState, connectorName, connectorLabel) => {
    ...
    cy.readFile(globalState.get("connectorAuthFilePath")).then((jsonContent) => {
      const authDetails = getValueByKey(JSON.stringify(jsonContent), connectorName);
      ...
      cy.request({
        ...
        if (response.status === 200) {
          ...
          globalState.set("merchantConnectorId", response.body.merchant_connector_id);
        } else {
          cy.task("cli_log", "response status -> " + JSON.stringify(response.status));

          throw new Error(`Connector Create Call Failed ${response.body.error.message}`);
        }
      });
    });
  }
...
Cypress.Commands.add(
  ...
  (connectorType, createConnectorBody, payment_methods_enabled, globalState) => {
    const merchantId = globalState.get("merchantId");
    createConnectorBody.connector_type = connectorType;
...
Cypress.Commands.add("createPayoutConnectorCallTest", (connectorType, createConnectorBody, globalState) => {
  const merchantId = globalState.get("merchantId");
  ...
  // it is best to use then() to handle the response within the same block of code
  cy.readFile(globalState.get("connectorAuthFilePath")).then((jsonContent) => {
    let authDetails = getValueByKey(JSON.stringify(jsonContent), `${connectorName}_payout`);
...
Cypress.Commands.add("connectorUpdateCall", (connectorType, updateConnectorBody, globalState) => {
  const merchant_id = globalState.get("merchantId");
  const connector_id = globalState.get("connectorId");
  const merchant_connector_id = globalState.get("merchantConnectorId");
...
Cypress.Commands.add("customerUpdateCall", (customerUpdateBody, globalState) => {
  const customer_id = globalState.get("customerId");
...
Cypress.Commands.add("paymentMethodListTestLessThanEqualToOnePaymentMethod", (res_data, globalState) => {
  cy.request({
    method: "GET",
    ...
    if (response.status === 200) {
      ...
      if (res_data["payment_methods"].length == 1) {
        function getPaymentMethodType(obj) {
          return obj["payment_methods"][0]["payment_method_types"][0]["payment_method_type"];
        }
        expect(getPaymentMethodType(res_data)).to.equal(getPaymentMethodType(response.body));
      } else {
...
Cypress.Commands.add("paymentMethodListTestTwoConnectorsForOnePaymentMethodCredit", (res_data, globalState) => {
  cy.request({
    method: "GET",
    url: `${globalState.get("baseUrl")}/account/payment_methods?client_secret=${globalState.get("clientSecret")}`,
    headers: {
    ...
    if (response.status === 200) {
      ...
      if (res_data["payment_methods"].length > 0) {
        function getPaymentMethodType(obj) {
          return obj["payment_methods"][0]["payment_method_types"][0]["card_networks"][0]["eligible_connectors"]
            .slice()
            .sort();
        }
        ...
        for (let i = 0; i < response_payment_method_type.length; i++) {
          expect(config_payment_method_type[i]).to.equal(response_payment_method_type[i]);
        }
      } else {
        expect(0).to.equal(response.body["payment_methods"].length);
      }
...
Cypress.Commands.add(
  ...
  (createPaymentBody, req_data, res_data, authentication_type, capture_method, globalState) => {
    if (!createPaymentBody || typeof createPaymentBody !== "object" || !req_data.currency) {
    ...
    cy.request({
      ...
      if (res_data.status === 200) {
        ...
        expect(createPaymentBody.amount, "amount").to.equal(response.body.amount);
        expect(createPaymentBody.currency, "currency").to.equal(response.body.currency);
        expect(createPaymentBody.capture_method, "capture_method").to.equal(response.body.capture_method);
        expect(createPaymentBody.authentication_type, "authentication_type").to.equal(
          response.body.authentication_type
        );
        expect(createPaymentBody.description, "description").to.equal(response.body.description);
        expect(createPaymentBody.email, "email").to.equal(response.body.email);
        expect(createPaymentBody.email, "customer.email").to.equal(response.body.customer.email);
        expect(createPaymentBody.customer_id, "customer.id").to.equal(response.body.customer.id);
        expect(createPaymentBody.metadata, "metadata").to.deep.equal(response.body.metadata);
        expect(createPaymentBody.setup_future_usage, "setup_future_usage").to.equal(response.body.setup_future_usage);
        expect(createPaymentBody.amount, "amount_capturable").to.equal(response.body.amount_capturable);
        expect(response.body.amount_received, "amount_received").to.be.oneOf([0, null]);
        expect(response.body.connector, "connector").to.be.null;
        expect(createPaymentBody.capture_method, "capture_method").to.equal(response.body.capture_method);
        expect(response.body.payment_method, "payment_method").to.be.null;
        expect(response.body.payment_method_data, "payment_method_data").to.be.null;
        expect(response.body.merchant_connector_id, "merchant_connector_id").to.be.null;
        expect(response.body.payment_method_id, "payment_method_id").to.be.null;
        expect(response.body.payment_method_id, "payment_method_status").to.be.null;
        expect(response.body.profile_id, "profile_id").to.not.be.null;
        expect(response.body.merchant_order_reference_id, "merchant_order_reference_id").to.be.null;
        expect(response.body.connector_mandate_id, "connector_mandate_id").to.be.null;
      } else {
        defaultErrorHandler(response, res_data);
      }
    });
...
Cypress.Commands.add("createPaymentMethodTest", (globalState, req_data, res_data) => {
...
Cypress.Commands.add("confirmCallTest", (confirmBody, req_data, res_data, confirm, globalState) => {
  ...
  cy.request({
    ...
    if (response.status === 200) {
      ...
      expect(response.body.connector, "connector").to.equal(globalState.get("connectorId"));
      expect(response.body.payment_id, "payment_id").to.equal(paymentIntentID);
      expect(response.body.payment_method_data, "payment_method_data").to.not.be.empty;
      expect(response.body.merchant_connector_id, "connectorId").to.equal(globalState.get("merchantConnectorId"));
      expect(response.body.customer, "customer").to.not.be.empty;
      expect(response.body.billing, "billingAddress").to.not.be.empty;
      expect(response.body.profile_id, "profile_id").to.not.be.null;
      expect(response.body.connector_transaction_id, "connector_transaction_id").to.not.be.null;
      if (response.body.capture_method === "automatic") {
        if (response.body.authentication_type === "three_ds") {
          expect(response.body).to.have.property("next_action").to.have.property("redirect_to_url");
          globalState.set("nextActionUrl", response.body.next_action.redirect_to_url);
          for (const key in res_data.body) {
            expect(res_data.body[key], [key]).to.deep.equal(response.body[key]);
          }
...
Cypress.Commands.add("confirmBankRedirectCallTest", (confirmBody, req_data, res_data, confirm, globalState) => {
  const paymentIntentId = globalState.get("paymentID");
...
Cypress.Commands.add("confirmBankTransferCallTest", (confirmBody, req_data, res_data, confirm, globalState) => {
  const paymentIntentID = globalState.get("paymentID");
  for (const key in req_data) {
    confirmBody[key] = req_data[key];
  }
    ...
    if (response.status === 200) {
      ...
      if (response.body.capture_method === "automatic" || response.body.capture_method === "manual") {
        switch (response.body.payment_method_type) {
          case "pix":
            expect(response.body).to.have.property("next_action").to.have.property("qr_code_url");
...
Cypress.Commands.add("confirmUpiCall", (confirmBody, req_data, res_data, confirm, globalState) => {
    ...
    if (response.status === 200) {
      if (response.body.capture_method === "automatic" || response.body.capture_method === "manual") {
        if (response.body.payment_method_type === "upi_collect") {
          ...
          expect(response.body).to.have.property("next_action").to.have.property("qr_code_fetch_url");
          globalState.set("nextActionUrl", response.body.next_action.qr_code_fetch_url);
        }
      } else {
        throw new Error(`Invalid capture method ${response.body.capture_method}`);
      }
...
Cypress.Commands.add(
  ...
  (createConfirmPaymentBody, req_data, res_data, authentication_type, capture_method, globalState) => {
    ...
    cy.request({
      ...
      if (response.status === 200) {
        ...
        globalState.set("paymentID", response.body.payment_id);
        expect(response.body.connector, "connector").to.equal(globalState.get("connectorId"));
        expect(response.body.payment_id, "payment_id").to.equal(globalState.get("paymentID"));
        expect(response.body.payment_method_data, "payment_method_data").to.not.be.empty;
        expect(response.body.merchant_connector_id, "connectorId").to.equal(globalState.get("merchantConnectorId"));
        expect(response.body.customer, "customer").to.not.be.empty;
        expect(response.body.billing, "billingAddress").to.not.be.empty;
...
Cypress.Commands.add("retrievePaymentCallTest", (globalState, autoretries = false, attempt = 1) => {
  const payment_id = globalState.get("paymentID");
  cy.request({
    method: "GET",
    ...
    expect(response.body.billing, "billingAddress").to.not.be.empty;
    expect(response.body.customer, "customer").to.not.be.empty;
    if (["succeeded", "processing", "requires_customer_action"].includes(response.body.status)) {
      expect(response.body.connector, "connector").to.equal(globalState.get("connectorId"));
      expect(response.body.payment_method_data, "payment_method_data").to.not.be.empty;
    ...
    if (autoretries) {
      ...
      expect(response.body.attempts[0].attempt_id).to.include(`${payment_id}_`);
      for (const key in response.body.attempts) {
        if (
          response.body.attempts[key].attempt_id === `${payment_id}_${attempt}` &&
          ...
          expect(response.body.attempts[key].status).to.equal("authentication_pending");
        } else {
...
Cypress.Commands.add(
  ...
  (requestBody, req_data, res_data, amount, confirm, capture_method, payment_type, globalState) => {
    for (const key in req_data) {
  ...
  cy.request({
    ...
    if (response.status === 200) {
      ...
      if (response.body.error.message === "Mandate Validation Failed") {
        ...
        expect(response.body.error.message).to.equal("Mandate Validation Failed");
        expect(response.body.error.reason).to.equal("request amount is greater than mandate amount");
      }
    } else {
      throw new Error(
...
Cypress.Commands.add("mitUsingPMId", (requestBody, amount, confirm, capture_method, globalState) => {
  requestBody.amount = amount;
  requestBody.confirm = confirm;
  requestBody.capture_method = capture_method;
...
Cypress.Commands.add("handleRedirection", (globalState, expected_redirection) => {
  ...
  handleRedirection("three_ds", { redirection_url, expected_url }, connectorId, null);
});

Cypress.Commands.add("handleBankRedirectRedirection", (globalState, payment_method_type, expected_redirection) => {
  let connectorId = globalState.get("connectorId");
  let expected_url = new URL(expected_redirection);
  let redirection_url = new URL(globalState.get("nextActionUrl"));
  ...
  if (!(connectorId == "adyen" && payment_method_type == "sofort")) {
    handleRedirection("bank_redirect", { redirection_url, expected_url }, connectorId, payment_method_type);
  }
});

Cypress.Commands.add("handleBankTransferRedirection", (globalState, payment_method_type, expected_redirection) => {
  let connectorId = globalState.get("connectorId");
  let expected_url = new URL(expected_redirection);
  let redirection_url = new URL(globalState.get("nextActionUrl"));
  let next_action_type = globalState.get("nextActionType");
  cy.log(payment_method_type);
  handleRedirection("bank_transfer", { redirection_url, expected_url }, connectorId, payment_method_type, {
    next_action_type,
  });
});

Cypress.Commands.add("handleUpiRedirection", (globalState, payment_method_type, expected_redirection) => {
  let connectorId = globalState.get("connectorId");
  let expected_url = new URL(expected_redirection);
  let redirection_url = new URL(globalState.get("nextActionUrl"));
  handleRedirection("upi", { redirection_url, expected_url }, connectorId, payment_method_type);
});
...
Cypress.Commands.add("listCustomerPMCallTest", (globalState) => {
  ...
  cy.request({
    ...
    if (response.body.customer_payment_methods[0]?.payment_token) {
      ...
      expect(response.body).to.have.property("customer_payment_methods").to.be.an("array").and.empty;
    }
  });
});
...
Cypress.Commands.add(
  ...
  (createConfirmPayoutBody, req_data, res_data, confirm, auto_fulfill, globalState) => {
    for (const key in req_data) {
...
Cypress.Commands.add("terminate2Fa", (globalState) => {
  ...
  cy.request({
    ...
    if (response.status === 200) {
      ...
      throw new Error(`2FA terminate call failed with status ${response.status} and message ${response.body.message}`);
    }
  });
});
...
Cypress.Commands.add("addRoutingConfig", (routingBody, req_data, res_data, type, data, globalState) => {
  for (const key in req_data) {
...
Cypress.Commands.add("activateRoutingConfig", (req_data, res_data, globalState) => {
...
Cypress.Commands.add("retrieveRoutingConfig", (req_data, res_data, globalState) => {
  let routing_config_id = globalState.get("routingConfigId");
  cy.request({
    method: "GET",
...
Cypress.Commands.add("autoRetryConfig", (autoRetryGsmBody, globalState, value) => {
...
Cypress.Commands.add("setMaxAutoRetries", (maxAutoRetryBody, globalState, value) => {
...
Cypress.Commands.add("updateGsmConfig", (gsmBody, globalState, step_up_possible) => {
  ...
  cy.request({
    ...
    logRequestId(response.headers["x-request-id"]);
    if (response.status === 200) {
      expect(response.body).to.have.property("message").to.equal("card_declined");
      expect(response.body).to.have.property("connector").to.equal("stripe");
      expect(response.body).to.have.property("step_up_possible").to.equal(step_up_possible);
    }
  });
});
