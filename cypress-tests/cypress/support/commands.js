...
Cypress.Commands.add(
  "computeAndVerifyRedirectSignature",
  (globalState) => {
    const merchantId = globalState.get("merchantId");
    const apiKey = globalState.get("adminApiKey");
    const baseUrl = globalState.get("baseUrl");
    ...
    cy.request({
      ...
      logRequestId(accountResponse.headers["x-request-id"]);

      expect(accountResponse.status).to.equal(200);
      const hashKey = accountResponse.body.payment_response_hash_key;
      expect(hashKey, "payment_response_hash_key should exist").to.be
        .a("string")
        .and.not.be.empty;
      ...
      cy.request({
        ...
        const redirectUrl =
          paymentResponse.body?.next_action?.redirect_to_url;
        ...
        expect(
          signature,
          "signature should exist in redirect URL"
        ).to.be.a("string").and.not.be.empty;
        expect(
          signatureAlgorithm,
          "signature_algorithm should be HMAC-SHA512"
...
Cypress.Commands.add(
  ...
  (globalState) => {
    ...
    const tamperedPayload = signingPayload.replace(/status=\w+/, "status=tampered_status");
    ...
    cy.request({
      ...
      cy.request({
        ...
        cy.task("computeHmacSha512", {
          ...
          cy.task(
            "cli_log",
            `Webhook signature verified - signature match: YES`
          );
        });
      });
    });
