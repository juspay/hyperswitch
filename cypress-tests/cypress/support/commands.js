...
Cypress.Commands.add("createConnectorWithNameCallTest", (createConnectorBody, connector_name, connector_type, globalState) => {
  const merchantId = globalState.get("merchantId");
  ...
  cy.readFile(globalState.get("connectorAuthFilePath")).then((jsonContent) => {
    const authDetails = getValueByKey(JSON.stringify(jsonContent), connector_name);
    createConnectorBody.connector_account_details = authDetails;
    cy.request({
      method: "POST",
      url: `${globalState.get("baseUrl")}/account/${merchantId}/connectors`,
      ...
      if (response.status === 200) {
        globalState.set(`${connector_name}_mc_id`, response.body.merchant_connector_id);
      } else {
        cy.task('cli_log', "response status -> " + JSON.stringify(response.status));
      }
    });
