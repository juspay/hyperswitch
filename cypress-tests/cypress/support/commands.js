...
Cypress.Commands.add("confirmCallTestBankRedirect", (confirmBody, details, confirm, globalState) => {
  const paymentIntentID = globalState.get("paymentID");
  ...
  confirmBody.confirm = confirm;
  confirmBody.customer_acceptance = details.customer_acceptance;
  confirmBody.setup_future_usage = details.setup_future_usage;
  confirmBody.client_secret = globalState.get("clientSecret");
  globalState.set("payment_method_type", details.payment_method_type);
  cy.task('cli_log', "global_state_payment_method_type ->> " + globalState.get("payment_method_type"));
  cy.request({
    method: "POST",
    url: `${globalState.get("baseUrl")}/payments/${paymentIntentID}/confirm`,
    headers: {
    ...
    const xRequestId = response.headers['x-request-id'];
    cy.log(JSON.stringify(confirmBody));
    if (xRequestId) {
      cy.task('cli_log', "x-request-id ->> " + xRequestId);
    } else {
      cy.task('cli_log', "x-request-id is not available in the response headers");
    ...
    if (response.body.capture_method === "automatic" || response.body.capture_method === "manual") {
      if (response.body.authentication_type === "three_ds") {
        expect(response.body).to.have.property("next_action")
          .to.have.property("redirect_to_url");
        console.log("response.body.next_action.redirect_to_url abhishek ->> " + JSON.stringify(response.body.next_action.redirect_to_url));
        globalState.set("nextActionUrl", response.body.next_action.redirect_to_url);
        cy.task('cli_log', "global_state_redirect_url ->> " + globalState.get("nextActionUrl"));
        cy.task('cli_log', "response-body ->> " + JSON.stringify(response.body));
      } else {
        // Handle other authentication types as needed
        throw new Error(`Unsupported authentication type: ${authentication_type}`);
      }
...
Cypress.Commands.add("handleRedirectionBankRedirect", (globalState, expected_redirection) => {
  let expected_url = new URL(expected_redirection);
  let redirection_url = new URL(globalState.get("nextActionUrl"));
  cy.task('cli_log', "global_state_redirect_url ->> " + redirection_url.href);
  cy.visit(redirection_url.href);
  if (globalState.get("connectorId") === "trustpay") {
    if (globalState.get("payment_method_type") === "ideal") {
      cy.get('p').should('contain.text', 'Choose your iDeal Issuer Bank please')
      cy.get('#issuerSearchInput').click()
      cy.get('#issuerSearchInput').type('ING{enter}')
      cy.get('#trustpay__selectIssuer_submit').click()
    }
    else if (globalState.get("payment_method_type") === "giropay") {
    ...
    else if (globalState.get("payment_method_type") === "eps") {
    ...
    if (globalState.get("payment_method_type") === "ideal") {
      cy.get(':nth-child(4) > td > p').should('contain.text', 'Your Payment was Authorised/Refused/Cancelled (It may take up to five minutes to show on the Payment List)')
    ...
    else if (globalState.get("payment_method_type") === "sofort") {
      cy.task('cli_log', "print here ->> " + JSON.stringify(globalState.get("payment_method_type")));
    }
    else if (globalState.get("payment_method_type") === "eps") {
