...
Cypress.Commands.add(
  ...
  (merchantCreateBody, globalState) => {
    ...
    cy.request({
      ...
      responseHandler.validateExistenceOfMerchantId(
      ...
      (jsonContent) => {
        ...
        cy.request({
          ...
          if (response.status === 200) {
            responseHandler.validateConnectorName(
...
Cypress.Commands.add(
  ...
  (request, det, authentication_type, capture_method, globalState) => {
    if (
      !request ||
    ...
    cy.request({
      ...
      cy.log(clientSecret);
      responseHandler.validateResponseStatus(
...
Cypress.Commands.add(
  "confirmCallTest",
  (confirmBody, details, confirm, globalState) => {
...
Cypress.Commands.add(
  "captureCallTest",
  (request, amount_to_capture, paymentSuccessfulStatus, globalState) => {
    ...
    cy.request({
      ...
      if (amount_to_capture == amount && response.body.status == "succeeded") {
        ...
        responseHandler.validatePaymentId(response, payment_id);
        responseHandler.validateAmountToCapture(
          response.body.amount,
          amount_to_capture
        );
        responseHandler.validateCapturableAmount(request, response);
        responseHandler.validateReceivedAmount(amount, request, response);
        responseHandler.validatePaymentStatus(
          paymentSuccessfulStatus,
          response.body.status
        );
...
Cypress.Commands.add("voidCallTest", (request, globalState) => {
  ...
  cy.request({
    ...
    responseHandler.validateReceivedAmount(
...
Cypress.Commands.add(
  "refundCallTest",
  (request, refund_amount, det, globalState) => {
    const payment_id = globalState.get("paymentID");
...
Cypress.Commands.add(
  ...
  (request, amount, confirm, capture_method, globalState) => {
    request.amount = amount;
    request.confirm = confirm;
    request.capture_method = capture_method;
    ...
    cy.request({
      ...
      responseHandler.handleAuthType(response, globalState, true, {
  ...
  cy.request({
    ...
    for (i in response.body) {
      if (response.body[i].mandate_id === globalState.get("mandateId")) {
        responseHandler.validateMandateStatus(
...
Cypress.Commands.add("revokeMandateCallTest", (globalState) => {
  ...
  cy.request({
    ...
    if (response.body.status === 200) {
      ...
      responseHandler.validateMandateReason(
      ...
      cy.get("iframe", { timeout: 100000 })
        ...
        .within((body) => {
          cy.get("iframe", { timeout: 10000 })
            .its("0.contentDocument.body")
            .within((body) => {
              cy.get('form[name="cardholderInput"]', { timeout: 10000 })
                .should("exist")
                .then((form) => {
...
Cypress.Commands.add("listCustomerPMCallTest", (globalState) => {
  ...
  cy.request({
    ...
    if (response.body.customer_payment_methods[0]?.payment_token) {
      ...
      responseHandler.validatePaymentToken(
        globalState.get("paymentToken"),
        paymentToken
      ); // Verify paymentToken
    } else {
