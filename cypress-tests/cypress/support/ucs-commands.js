...
Cypress.Commands.add("verifyUCSConfigs", (globalState, connector) => {
  ...
  cy.task("cli_log", `🎉 UCS configuration verification completed for ${connector}`);
});
  ...
  rolloutConfigs.forEach((rolloutConfigKey) => {
    cy.request({
      ...
      if (response.status === 200 || response.status === 404) {
        cy.task("cli_log", `🗑️ Deleted UCS rollout config: ${rolloutConfigKey}`);
      } else {
        cy.task(
          "cli_log",
 ...
 */
Cypress.Commands.add("createUCSPayment", (requestType, connector, globalState, baseRequest) => {
  ...
  if (isMandateRequest && baseRequest.mandate_data.mandate_type) {
    ...
    cy.task("cli_log", `⚠️ Mandate request detected but no mandate_type found`);
  }
  ...
  cy.task("cli_log", `📄 Final UCS request: ${JSON.stringify(ucsPaymentBody, null, 2)}`);
 ...
 * @param {string} requestType - Type of UCS request
 */
Cypress.Commands.add("validateUCSResponse", (response, expectedResponse, requestType) => {
...
Cypress.Commands.add("logTestProgress", (current, total, passed, failed) => {
  ...
  cy.task("cli_log", `📊 Progress: ${progress}% | Passed: ${passed.length} | Failed: ${failed.length}`);
});
...
Cypress.Commands.add("logTestResults", (testResults) => {
  ...
  cy.task("cli_log", `🏁 All ${testResults.totalTested} UCS requests completed!`);
  ...
  cy.task("cli_log", `⏭️ SKIPPED (${testResults.skipped.length}): ${testResults.skipped.join(", ") || "None"}`);

  const successRate = testResults.totalTested > 0 ? ((testResults.passed.length / testResults.totalTested) * 100).toFixed(1) : 0;
 ...
 * @param {string} connector - Connector name
 */
Cypress.Commands.add("setupUCSEnvironment", (fixtures, globalState, connector) => {
 ...
 */
Cypress.Commands.add("loadUCSConfig", (connector, ucsRequestNames, getConnectorDetails) => {
  const config = getConnectorDetails(connector).card_pm;
  const connectorConfig = { card_pm: config };

  if (!connectorConfig?.card_pm) {
    throw new Error(`Failed to load configuration for connector: ${connector}`);
  }
  ...
  cy.task("cli_log", `📊 Total requests available in ${connector}.js: ${allRequests.length}`);
  ...
  if (testableRequests.length === 0) {
    throw new Error(`No UCS-compatible requests found for connector: ${connector}`);
  }
 ...
 * @param {Object} globalState - Global state object
 */
Cypress.Commands.add("executeUCSSequentialFlow", (connectorConfig, testResults, currentConnector, globalState) => {
  ...
  if (!ucsZeroAuthConfig || !ucsConfirmConfig || !ucsRecurringConfig) {
    throw new Error(`❌ UCS Sequential Flow configs not found for connector: ${currentConnector}`);
  }
  ...
  cy.request({
    ...
    cy.validateUCSResponse(response1, ucsZeroAuthConfig.Response, "UCSZeroAuthMandate")
      .then((result1) => {
        if (!result1.success) {
        ...
        cy.request({
          ...
          cy.validateUCSResponse(response2, ucsConfirmConfig.Response, "UCSConfirmMandate")
            .then((result2) => {
              if (!result2.success) {
              ...
              cy.request({
                ...
                cy.validateUCSResponse(response3, ucsRecurringConfig.Response, "UCSRecurringPayment")
                  .then((result3) => {
                    if (!result3.success) {
                      throw new Error(`UCS Sequential Flow Failed - ${result3.error}`);
                    }

                    if (testResults.passed) {
                      testResults.passed.push("UCSRecurringPayment");
                    }
                    cy.task("cli_log", "🎉 UCS Sequential Flow Completed Successfully!");
                  });
              });
            });
        });
