...
const validateContentType = function (response) {
  expect(response.headers["content-type"]).to.include("application/json");
};
const validateExistenceOfMerchantId = function (
  merchantId,
  responseMerchantId
) {
  expect(responseMerchantId).to.equal(merchantId).and.not.empty;
};
const validateCustomerId = function (customerId, responseCustomerId) {
  expect(responseCustomerId).to.equal(customerId).and.not.empty;
};
const validateConnectorName = function (
  connectorName,
  connectorNameFromGlobalState
) {
  expect(connectorName).to.equal(connectorNameFromGlobalState).and.not.empty;
};
const validatePaymentId = function (response, paymentId) {
  expect(response.body).to.have.property("payment_id").equal(paymentId);
};
const validateAmount = function (amount, response) {
  expect(amount).to.equal(response.body.amount);
};
const validateAmountToCapture = function (amount, amount_to_capture) {
  expect(amount).to.equal(amount_to_capture);
};
const validateExistenceOfClientSecret = function (body) {
  expect(body).to.have.property("client_secret");
};
const validateExistenceOfRedirectUrl = function (response) {
  expect(response.body)
    .to.have.property("next_action")
    .to.have.property("redirect_to_url");
};
const validateExistenceOfPMRedirectUrl = function (response) {
  expect(response.body).to.have.property("redirect_url");
};
const validateExistenceOfPaymentMethods = function (response) {
  expect(response.body).to.have.property("payment_methods");
};
const validatePaymentToken = function (token, paymentToken) {
  expect(token).to.equal(paymentToken);
};
const validateExistenceOfStatus = function (response) {
  expect(response.body).to.have.property("status");
};
const validateExistenceOfMandateId = function (response) {
  expect(response.body).to.have.property("mandate_id");
};
const validateMandateStatus = function (status, expectedStatus) {
  expect(status).to.equal(expectedStatus);
};
const validateMandateReason = function (reason, expectedReason) {
  expect(reason).to.equal(expectedReason);
};
const validateArrayResponse = function (data) {
    ...
    case "no_three_ds":
      if (response.body.capture_method === "automatic") {
        validatePaymentStatus(
          details.paymentSuccessfulStatus,
