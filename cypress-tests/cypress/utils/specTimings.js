 ...
 */
function buildReportLines(timings, label) {
  ...
  const lines = [];
  lines.push("");
  lines.push(`  (Spec Timings — ${titleSuffix}${timings.length} specs)`);
  lines.push("");
  lines.push(
 ...
 *
 * `appendFileSync` here can race when `CYPRESS_SPEC_SHARDS` runs several of
 * these processes concurrently against the same step, each with their own
 * `label` — POSIX `O_APPEND` writes are atomic per `write()` call for a
 * regular file, and this is one `fs.appendFileSync` call per report, so
 * concurrent shards interleave whole blocks rather than corrupting a line.
 */
function writeStepSummary(lines, label) {
  const summaryPath = process.env.GITHUB_STEP_SUMMARY;
  if (!summaryPath) return;

  const heading = label ? `## Spec Timings — ${label}` : "## Spec Timings";
  const markdown = ["", heading, "", "```", ...lines, "```", ""].join("\n");
  fs.appendFileSync(summaryPath, markdown);
}

function printReport(timings, label) {
  if (timings.length === 0) return;

  const lines = buildReportLines(timings, label);
  for (const line of lines) {
    console.log(line);
  }

  try {
    writeStepSummary(lines, label);
  } catch (error) {
 ...
 *
 * @param {string} [label] - Identifies which run this report is for, e.g. the
 *   `REPORT_NAME` cypress.config.js already computes. Required to tell shards
 *   apart when `CYPRESS_SPEC_SHARDS` runs several of these concurrently — each
 *   is its own Node process reporting into the same
 *   `$GITHUB_STEP_SUMMARY`/stdout, and without a label the resulting blocks
 *   would be indistinguishable.
 */
export function registerSpecTimings(on, label) {
  const timings = [];
