...
<Steps>
  ...
  <Step title="Self-Deploy flow" icon="map-pin">
   Use the admin api key and the [Merchant Account - Create](http://localhost:3000/api-reference/merchant-account/merchant-account--create) endpoint to create your Merchant Account
  </Step>  
</Steps>
  </Step>
 ...
 <Steps>
  ...
  <Step title="Set up Connectors">
    Connect the payment [connector](http://localhost:3000/api-reference/merchant-connector-account/merchant-connector--create) using and the mode of payments that your organization will accept. could be a payment processor/facilitator/acquirer or a provider of specialized services like Fraud/Accounting
  <Steps>
  <Step title="Hyperswitch Dashboard flow" icon= "box-open">
  ...
  <Step title="Self-Deploy flow" icon="map-pin">
   Use the admin api key and the [Connector Account ](http://localhost:3000/api-reference/merchant-connector-account/merchant-connector--create)endpoints to set up a connector
  </Step>  
</Steps>
  </Step>
  <Step title="Try your first payment">
    You are all set! go ahead and try the [Payments API](http://localhost:3001/api-reference/payments/payments--create), be sure to try the different use cases provided
  </Step>
