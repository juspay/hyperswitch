...
if (jsonData?.status) {
pm.test("[POST]::/payments - Content check if value for 'status' matches 'failed'", function() {
  pm.expect(jsonData.status).to.eql("failed");
})};

// Response body should have value "requires_confirmation" for "status"
if (jsonData?.status) {
pm.test("[POST]::/payments - Content check if value for 'error_code' matches 'UNPROCESSABLE_ENTITY'", function() {
  pm.expect(jsonData.error_code).to.eql("UNPROCESSABLE_ENTITY");
})};

// Response body should have value "requires_confirmation" for "status"
if (jsonData?.status) {
pm.test("[POST]::/payments - Content check if value for 'error_message' matches 'failed'", function() {
  pm.expect(jsonData.error_message).to.eql("description - Invalid card number, field - number;");
})};
