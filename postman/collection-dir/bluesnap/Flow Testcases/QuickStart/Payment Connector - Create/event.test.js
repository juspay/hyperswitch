...
pm.test("[POST]::/account/:account_id/connectors - Content check if auth_type is BodyKey", function() {
   pm.expect((typeof jsonData.connector_account_details.auth_type !== "BodyKey")).to.be.true;
});

pm.test("[POST]::/account/:account_id/connectors - Content check if 'api_key' exists", function() {
   pm.expect((typeof jsonData.connector_account_details.api_key !== "undefined")).to.be.true;
});

pm.test("[POST]::/account/:account_id/connectors - Content check if 'key1' exists", function() {
   pm.expect((typeof jsonData.connector_account_details.key1 !== "undefined")).to.be.true;
});
