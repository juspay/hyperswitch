...
if (jsonData?.status) {
pm.test("[POST]::/refunds - Content check if value for 'status' matches 'failed'", function() {
  pm.expect(jsonData.status).to.eql("failed");
})};

// Response body should have value "1000" for "amount"
if (jsonData?.status) {
pm.test("[POST]::/refunds - Content check if value for 'amount' matches '500'", function() {
