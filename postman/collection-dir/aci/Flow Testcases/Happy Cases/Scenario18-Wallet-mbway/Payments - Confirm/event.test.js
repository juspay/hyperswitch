...
// Response body should have value "mb_way" for "payment_method_type"
if (jsonData?.payment_method_type) {
pm.test("[POST]::/payments/:id/confirm - Content check if value for 'payment_method_type' matches 'mb_way'", function() {
  pm.expect(jsonData.payment_method_type).to.eql("mb_way");
})};


// Response body should have value "aci" for "connector"
if (jsonData?.connector) {
pm.test("[POST]::/payments/:id/confirm - Content check if value for 'connector' matches 'aci'", function() {
  pm.expect(jsonData.connector).to.eql("aci");
})};