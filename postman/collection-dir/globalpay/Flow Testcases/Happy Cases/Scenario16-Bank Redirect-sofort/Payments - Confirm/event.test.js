...
pm.test("[POST]::/payments/:id/confirm - Content check if value for 'payment_method_type' matches 'sofort'", function() {
  pm.expect(jsonData.payment_method_type).to.eql("sofort");
})};


// Response body should have value "globalpay" for "connector"
if (jsonData?.connector) {
pm.test("[POST]::/payments/:id/confirm - Content check if value for 'connector' matches 'globalpay'", function() {
  pm.expect(jsonData.connector).to.eql("globalpay");
})};