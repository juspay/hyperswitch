...
pm.test("[POST]::/payments - Content check if value for 'error.type' matches 'connector_error'", function() {
  pm.expect(jsonData.error.type).to.eql("connector");
})};

if (jsonData?.error?.type) {
pm.test("[POST]::/payments - Content check if value for 'error.type' matches 'connector_error'", function() {
  pm.expect(jsonData.error.message).to.eql("INVALID_REQUEST_DATA: Transaction not supported Please contact support ");
})};