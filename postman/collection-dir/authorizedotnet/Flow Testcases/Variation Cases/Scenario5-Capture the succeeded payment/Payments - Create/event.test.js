...
if (jsonData?.status) {
    pm.test("[POST]::/payments - Content check if value for 'status' matches 'processing' or 'requires_capture'", function () {
pm.collectionVariables.set("variable_key", "variable_value");
  pm.expect(jsonData.status).to.be.oneOf(["processing", "requires_capture"]);
})};
