...
if (jsonData?.error) {
  pm.test("[POST]::/payments - Content check if error type is 'invalid_request'", function() {
  pm.expect(jsonData.error.message).to.be.oneOf(["You cannot cancelled this payment because it has status processing", "You cannot cancelled this payment because it has status failed"] );
})};
