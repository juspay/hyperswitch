...
if (jsonData?.status) {
pm.test("[POST]::/payments - Content check if value for 'status' matches 'processing' or 'failed' or 'requires_confirmation'", function() {
  pm.expect(jsonData.status).to.be.oneOf(["processing", "requires_confirmation", "failed"]);
})};
