...
// Response body should have value "Json deserialize error" for "error type"
if (jsonData?.error?.type) {
pm.test("[POST]::/payments - Content check if value for 'error.type' matches 'Json deserialize error'", function() {
  pm.expect(jsonData.error.type).to.eql("Json deserialize error");
})};