...
pm.test("Save Payment ID", function () {
    ...
    // Assuming the payment ID is in the 'paymentId' field of the response
    const payment_id = responseBody.payment_id;

    // Save the payment ID as an environment variable
    pm.environment.set("payment_id", payment_id);
});

pm.test("Save client_secret", function () {
    // Parse the response body as JSON
    const responseBody = pm.response.json();

    // Assuming the payment ID is in the 'paymentId' field of the response
    const client_secret = responseBody.client_secret;
