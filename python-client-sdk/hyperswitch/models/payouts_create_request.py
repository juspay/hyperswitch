...
class PayoutsCreateRequest(BaseModel):
    """
    PayoutsCreateRequest
    """ # noqa: E501
    amount: Annotated[int, Field(strict=True, ge=0)] = Field(description="The payout amount. Amount for the payout in lowest denomination of the currency. (i.e) in cents for USD denomination, in paisa for INR denomination etc.,")
    currency: Currency
    ...
    confirm: Optional[StrictBool] = Field(default=False, description="This field is used when merchant wants to confirm the payout, thus useful for the payout _Confirm_ request. Ideally merchants should _Create_ a payout, _Update_ it (if required), then _Confirm_ it.")
    ...
    __properties: ClassVar[List[str]] = ["amount", "currency", "routing", "connector", "confirm", "payout_type", "payout_method_data", "billing", "auto_fulfill", "customer_id", "customer", "return_url", "business_country", "business_label", "description", "entity_type", "recurring", "metadata", "payout_token", "profile_id", "priority", "payout_link", "payout_link_config", "session_expiry", "email", "name", "phone", "phone_country_code", "payout_method_id"]
