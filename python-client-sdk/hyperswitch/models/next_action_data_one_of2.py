...
class NextActionDataOneOf2(BaseModel):
    """
    Contains third party sdk session token response
    """ # noqa: E501
    session_token: Optional[SessionToken] = None
    type: StrictStr
    __properties: ClassVar[List[str]] = ["session_token", "type"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['third_party_sdk_session_token']):
            raise ValueError("must be one of enum values ('third_party_sdk_session_token')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "session_token": SessionToken.from_dict(obj["session_token"]) if obj.get("session_token") is not None else None,
            "type": obj.get("type")
        })
        return _obj
