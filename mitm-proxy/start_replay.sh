...
echo "    HIT-server= server-UUID cassette matched via test name fallback"
echo "    MISS      = no cassette found, forwarded live"
echo "    WARN      = connector call arrived with no x-request-id"
echo ""
echo "Set this in the Cypress terminal for server-UUID cassette fallback matching:"
