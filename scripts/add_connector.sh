...
function find_prev_connector() {
    ...
    connectors=(aci adyen airwallex applepay authorizedotnet bambora bitpay bluesnap braintree checkout coinbase cybersource dlocal dummyconnector fiserv forte globalpay iatapay klarna mollie multisafepay nexinets noon nuvei opennode payeezy paypal payu rapyd shift4 stripe trustpay wise worldline worldpay "$1")
...
case "$connector_type" of
    payout) sed -i'' -e "s/pub enum PayoutRoutableConnectors {/pub enum PayoutRoutableConnectors {\n\t${pgc},/" crates/api_models/src/enums.rs   ;;
    *)      sed -i'' -e "s/pub enum PaymentRoutableConnectors {/pub enum PaymentRoutableConnectors {\n\t${pgc},/" crates/api_models/src/enums.rs ;;
esac
